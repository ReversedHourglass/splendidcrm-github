	...
	public class CreditCards : SplendidControl
	{
			...
			try
			{
				switch ( e.CommandName )
				{
					case "CreditCards.Create":
						Response.Redirect("~/CreditCards/edit.aspx?ACCOUNT_ID=" + gID.ToString());
						break;
					...
					{
						// 05/13/2008 Paul.  Add ability to delete credit card. 
						Guid gCREDIT_CARD_ID = Sql.ToGuid(e.CommandArgument);
		...
		protected void BindGrid()
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				m_sVIEW_NAME = "vwACCOUNTS_CREDIT_CARDS";
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwACCOUNTS_CREDIT_CARDS", Sql.ClientScriptBlock(cmd));
