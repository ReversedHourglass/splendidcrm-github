...
namespace SplendidCRM.Accounts.HubSpot
{
	/// <summary>
	...
	public class DetailView : SplendidControl
	{
		...
		{
			this.Visible = Spring.Social.HubSpot.HubSpotSync.HubSpotEnabled(Application) && (SplendidCRM.Security.GetUserAccess(m_sMODULE, "view") >= 0) && (SplendidCRM.Security.GetUserAccess("HubSpot", "view") >= 0);
			if ( !this.Visible )
				return;
			...
			try
			{
				...
				nHID = Sql.ToInteger(Request["HID"]);
				if ( !IsPostBack )
				{
					if ( !Sql.IsEmptyGuid(gID) )
					{
						...
						{
							string sSQL ;
							sSQL = "select (select SYNC_REMOTE_KEY from vwACCOUNTS_SYNC where SYNC_SERVICE_NAME = N'HubSpot' and SYNC_LOCAL_ID = vwACCOUNTS.ID) as SYNC_REMOTE_KEY" + ControlChars.CrLf
								...
								using ( DbDataAdapter da = dbf.CreateDataAdapter() )
								{
									...
									using ( DataTable dtCurrent = new DataTable() )
									{
										...
										if ( dtCurrent.Rows.Count > 0 )
										{
											...
											nHID = Sql.ToInteger(rdr["SYNC_REMOTE_KEY"]);
										}
										else
										{
										}
					...
					if ( nHID > 0 )
					{
						...
						{
							Spring.Social.HubSpot.Api.IHubSpot hubSpot = Spring.Social.HubSpot.HubSpotSync.CreateApi(Context.Application);
							Spring.Social.HubSpot.Api.Company obj = hubSpot.CompanyOperations.GetById(nHID);
							DataRow rdr = Spring.Social.HubSpot.Api.Company.ConvertToRow(obj);
				...
				else
				{
					...
					nHID = Sql.ToInteger(ViewState["HID"]);
					if ( nHID > 0 )
					{
		...
		{    
			...
			m_sMODULE = "Accounts";
			this.LayoutDetailView = "DetailView.HubSpot";
			if ( IsPostBack )
			{
				// 04/29/2015 Paul.  We need to make sure not to populate the DetailView even if there was an error or if disabled. Otherwise the other subpanels might fail. 
				ctlDynamicButtons.AppendButtons(m_sMODULE + "." + LayoutDetailView, Guid.Empty, null);
