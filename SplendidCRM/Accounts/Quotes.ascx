...
<SplendidCRM:SubPanelButtons ID="ctlDynamicButtons" Module="Quotes" SubPanel="divAccountsQuotes" Title="Quotes.LBL_MODULE_NAME" Runat="Server" />

<div id="divAccountsQuotes" style='<%= "display:" + (CookieValue("divAccountsQuotes") != "1" ? "inline" : "none") %>'>
			...
			<asp:TemplateColumn HeaderText="" ItemStyle-Width="1%" ItemStyle-HorizontalAlign="Left" ItemStyle-Wrap="false">
				<ItemTemplate>
					...
					<span onclick="return confirm('<%= L10n.TermJavaScript(".NTC_DELETE_CONFIRMATION") %>')">
						<asp:ImageButton Visible='<%# SplendidCRM.Security.GetRecordAccess(Container, "Accounts", "remove", "BILLING_ACCOUNT_ASSIGNED_ID") >= 0 && !ArchiveViewExists() %>' CommandName="Quotes.Remove" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "QUOTE_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" AlternateText='<%# L10n.Term(".LNK_REMOVE") %>' SkinID="delete_inline" Runat="server" />
						<asp:LinkButton  Visible='<%# SplendidCRM.Security.GetRecordAccess(Container, "Accounts", "remove", "BILLING_ACCOUNT_ASSIGNED_ID") >= 0 && !ArchiveViewExists() %>' CommandName="Quotes.Remove" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "QUOTE_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" Text='<%# L10n.Term(".LNK_REMOVE") %>' Runat="server" />
					</span>
				</ItemTemplate>
			</asp:TemplateColumn>
		</Columns>
