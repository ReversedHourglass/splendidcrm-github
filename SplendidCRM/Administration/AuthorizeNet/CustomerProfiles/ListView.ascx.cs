	...
	public class ListView : SplendidControl
	{
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				...
				{
					ctlCheckAll.SelectAll(vwMain, "customerProfileId");
					grdMain.DataBind();
				}
		...
		private void Bind(bool bBind)
		{
			...
			{
				DataSet ds = Cache.Get("AuthorizeNet.CustomerProfiles") as DataSet;
				if ( ds == null )
				{
					ds = AuthorizeNetUtils.CustomerProfiles(Application);
					Cache.Insert("AuthorizeNet.CustomerProfiles", ds, null, DateTime.Now.AddMinutes(1), System.Web.Caching.Cache.NoSlidingExpiration);
				}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				if ( !IsPostBack )
				{
					Cache.Remove("AuthorizeNet.CustomerProfiles");
				}
				Bind(!IsPostBack);
			}
			catch(Exception ex)
		...
		private void InitializeComponent()
		{
			...
			LayoutListView = "CustomerProfiles.ListView";
			SetMenu(m_sMODULE);
