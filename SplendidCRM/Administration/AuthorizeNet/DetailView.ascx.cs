...
namespace SplendidCRM.Administration.AuthorizeNet
{
	...
	public class DetailView : SplendidControl
	{
			...
			try
			{
				...
				else if ( e.CommandName == "Refund" )
				{
					if ( Sql.ToBoolean(Context.Application["CONFIG.AuthorizeNet.Enabled"]) && !Sql.IsEmptyString(Context.Application["CONFIG.AuthorizeNet.UserName"]) )
					{
						...
						Response.Redirect("view.aspx?transId=" + sRefundTransactionId);
					}
				}
			}
			catch(Exception ex)
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				sTransactionID = Sql.ToString(Request["transId"]);
				ctlDynamicButtons.Title = sTransactionID;
				SetPageTitle(L10n.Term(".moduleList." + m_sMODULE) + " - " + ctlDynamicButtons.Title);

				if ( !IsPostBack )
				{
					if ( Sql.ToBoolean(Context.Application["CONFIG.AuthorizeNet.Enabled"]) && !Sql.IsEmptyString(Context.Application["CONFIG.AuthorizeNet.UserName"]) && !Sql.IsEmptyString(sTransactionID) )
					{
						DataSet ds = AuthorizeNetUtils.Transaction(Application, sTransactionID);
						Page.Items["AuthorizeNet.Transaction"] = ds;
