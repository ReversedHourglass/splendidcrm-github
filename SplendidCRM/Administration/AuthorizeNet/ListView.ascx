<%@ Control CodeBehind="ListView.ascx.cs" Language="c#" AutoEventWireup="false" Inherits="SplendidCRM.Administration.AuthorizeNet.ListView" %>
<script runat="server">
/**********************************************************************************************************************
...
<div id="divListView">
	...
	<SplendidCRM:ListHeader Module="AuthorizeNet" Title="AuthorizeNet.LBL_LIST_FORM_TITLE" Runat="Server" />
	...
	<SplendidCRM:SplendidGrid id="grdMain" Width="100%" CssClass="listView" MassUpdateHoverColumn="0"
		...
		<Columns>
			<asp:TemplateColumn HeaderText="" ItemStyle-Width="1%">
				<ItemTemplate><%# grdMain.InputCheckbox(!PrintView, ctlCheckAll.FieldName, Sql.ToString(Eval("transId")), ctlCheckAll.SelectedItems) %></ItemTemplate>
			</asp:TemplateColumn>
		</Columns>
	</SplendidCRM:SplendidGrid>
	<%@ Register TagPrefix="SplendidCRM" Tagname="CheckAll" Src="~/_controls/CheckAll.ascx" %>
	<SplendidCRM:CheckAll ID="ctlCheckAll" Visible="<%# !PrintView %>" FieldName="transId" Runat="Server" />
