...
namespace SplendidCRM.Administration.ContractTypes
{
	...
	public class AutoComplete : System.Web.Services.WebService
	{
		...
		public ContractType CONTRACT_TYPES_CONTRACT_TYPE_NAME_Get(string sNAME)
		{
			...
			//try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						Security.Filter(cmd, "ContractTypes", "list");
				...
				if ( Sql.IsEmptyGuid(item.ID) )
				{
					...
					throw(new Exception(L10n.Term("ContractTypes.ERR_CONTRACT_TYPE_NOT_FOUND")));
				}
			}
			//catch
			{
		...
		[WebMethod(EnableSession=true)]
		public string[] CONTRACT_TYPES_CONTRACT_TYPE_NAME_List(string prefixText, int count)
		{
		...
		// so the field name can be NAME or CONTRACT_TYPE_NAME. 
		[WebMethod(EnableSession=true)]
		public string[] CONTRACT_TYPES_NAME_List(string prefixText, int count)
		{
			return CONTRACT_TYPES_CONTRACT_TYPE_NAME_List(prefixText, count);
		}
	}
}
