...
namespace SplendidCRM.Discounts
{
	...
	public class AutoComplete : System.Web.Services.WebService
	{
		...
		public Discount DISCOUNTS_DISCOUNT_NAME_Get(string sNAME)
		{
			...
			//try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						Security.Filter(cmd, "Discounts", "list");
				...
				if ( Sql.IsEmptyGuid(item.ID) )
				{
					...
					throw(new Exception(L10n.Term("Discounts.ERR_DISCOUNT_NOT_FOUND")));
				}
			}
			//catch
			{
		...
		[WebMethod(EnableSession=true)]
		public string[] DISCOUNTS_DISCOUNT_NAME_List(string prefixText, int count)
		{
		...
		// so the field name can be NAME or Discount_NAME. 
		[WebMethod(EnableSession=true)]
		public string[] DISCOUNTS_NAME_List(string prefixText, int count)
		{
			return DISCOUNTS_DISCOUNT_NAME_List(prefixText, count);
		}
	}
}
