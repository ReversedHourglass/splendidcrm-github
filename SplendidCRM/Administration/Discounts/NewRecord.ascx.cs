...
namespace SplendidCRM.Administration.Discounts
{
	/// <summary>
	...
	public class NewRecord : NewRecordControl
	{
		...
		protected DropDownList    lstPRICING_FORMULA              ;
		protected Label           lblPRICING_REQUIRED             ;
		protected Label           lblPRICING_FACTOR               ;
		protected TextBox         txtPRICING_FACTOR               ;
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			SqlProcs.spDISCOUNTS_Update(
				ref gID
				, txtNAME.Text
				, lstPRICING_FORMULA.SelectedValue
				, Sql.ToInteger(txtPRICING_FACTOR.Text)
				, trn
				);
			Cache.Remove("vwDISCOUNTS_LISTBOX");
		}

		protected void lstPRICING_FORMULA_SelectedIndexChanged(object sender, EventArgs e)
		{
			switch ( lstPRICING_FORMULA.SelectedValue )
			{
				case "Fixed"             :  txtPRICING_FACTOR.Visible = false;  txtPRICING_FACTOR.Text = "1.0";  break;
				case "ProfitMargin"      :  txtPRICING_FACTOR.Visible = true ;  break;
				case "PercentageMarkup"  :  txtPRICING_FACTOR.Visible = true ;  break;
				case "PercentageDiscount":  txtPRICING_FACTOR.Visible = true ;  break;
				case "FixedDiscount"     :  txtPRICING_FACTOR.Visible = true ;  break;
				case "IsList"            :  txtPRICING_FACTOR.Visible = false;  txtPRICING_FACTOR.Text = "1.0";  break;
			}
			lblPRICING_REQUIRED.Visible = txtPRICING_FACTOR.Visible;
			lblPRICING_FACTOR  .Visible = txtPRICING_FACTOR.Visible;
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				if ( !bIsPostBack )
				{
					lstPRICING_FORMULA.DataSource = SplendidCache.List("pricing_formula_dom");
					lstPRICING_FORMULA.DataBind();
					lstPRICING_FORMULA_SelectedIndexChanged(null, null);
					if ( NotPostBack )
						this.DataBind();
		...
		private void InitializeComponent()
		{
			...
			lstPRICING_FORMULA.SelectedIndexChanged += new EventHandler(lstPRICING_FORMULA_SelectedIndexChanged);
