<%@ Control CodeBehind="PopupView.ascx.cs" Language="c#" AutoEventWireup="false" Inherits="SplendidCRM.Administration.Discounts.PopupView" %>
<script runat="server">
/**********************************************************************************************************************
...
<div id="divPopupView">
	...
	<SplendidCRM:SearchView ID="ctlSearchView" Module="Discounts" IsPopupSearch="true" ShowSearchTabs="false" Visible="<%# !PrintView %>" Runat="Server" />

	<script type="text/javascript">
	function SelectDiscount(sPARENT_ID, sPARENT_NAME)
	{
		if ( window.opener != null && window.opener.ChangeDiscount != null )
		{
			window.opener.ChangeDiscount(sPARENT_ID, sPARENT_NAME);
			window.close();
		}
		else
		{
			alert('Original window has closed.  Discount cannot be assigned.' + '\n' + sPARENT_ID + '\n' + sPARENT_NAME);
		}
	}
	function SelectChecked()
	{
		if ( window.opener != null && window.opener.ChangeDiscount != null )
		{
			...
			window.opener.ChangeDiscount(sSelectedItems, '');
			window.close();
		}
		else
		{
	...
	<SplendidCRM:ListHeader Title="Discounts.LBL_LIST_FORM_TITLE" Runat="Server" />
	...
	<asp:UpdatePanel UpdateMode="Conditional" Visible='<%# !Sql.ToBoolean(Application["CONFIG.disable_popupview_inline"]) %>' runat="server">
		<ContentTemplate>
			<asp:Button ID="btnCreateInline" CommandName="NewRecord.Show" OnCommand="Page_Command" Text='<%# L10n.Term(m_sMODULE + ".LNK_NEW_DISCOUNT") %>' CssClass="button" style="margin-bottom: 4px;" Visible="<%# !this.IsMobile %>" runat="server" />
