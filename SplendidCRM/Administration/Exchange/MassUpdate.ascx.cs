	...
	public class MassUpdate : SplendidCRM.MassUpdate
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			try
			{
				if ( !IsPostBack )
				{
					//int nACLACCESS_Import = Security.GetUserAccess(m_sMODULE, "import");
					int nACLACCESS_Edit   = Security.GetUserAccess(m_sMODULE, "edit"  );
					int nACLACCESS_Delete = Security.GetUserAccess(m_sMODULE, "delete");
					ctlDynamicButtons.ShowButton("MassEnable" , nACLACCESS_Edit   >= 0);
					ctlDynamicButtons.ShowButton("MassDisable", nACLACCESS_Delete >= 0);
					
					this.Visible = !PrintView && ((nACLACCESS_Edit   >= 0) || (nACLACCESS_Delete >= 0));
				}
			}
			catch(Exception ex)
			{
