...
namespace SplendidCRM.ForumTopics
{
	...
	public class AutoComplete : System.Web.Services.WebService
	{
		...
		public ForumTopic FORUM_TOPICS_FORUM_TOPIC_NAME_Get(string sNAME)
		{
			...
			//try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						Security.Filter(cmd, "ForumTopics", "list");
				...
				if ( Sql.IsEmptyGuid(item.ID) )
				{
					...
					throw(new Exception(L10n.Term("ForumTopics.ERR_FORUM_TOPIC_NOT_FOUND")));
				}
			}
			//catch
			{
		...
		[WebMethod(EnableSession=true)]
		public string[] FORUM_TOPICS_FORUM_TOPIC_NAME_List(string prefixText, int count)
		{
		...
		// so the field name can be NAME or FORUM_TOPIC_NAME. 
		[WebMethod(EnableSession=true)]
		public string[] FORUM_TOPICS_NAME_List(string prefixText, int count)
		{
			return FORUM_TOPICS_FORUM_TOPIC_NAME_List(prefixText, count);
		}
	}
}
