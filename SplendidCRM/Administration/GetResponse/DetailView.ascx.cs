...
namespace SplendidCRM.Administration.GetResponse
{
	...
	public class DetailView : SplendidControl
	{
		...
		protected void Page_Command(Object sender, CommandEventArgs e)
		{
			if ( e.CommandName == "Test" )
			{
				try
				{
					...
					Spring.Social.GetResponse.GetResponseSync.ValidateGetResponse(Application, sbErrors);
			...
			{
#if false
				Spring.Social.GetResponse.GetResponseSync.Sync(Context);
#else
				System.Threading.Thread t = new System.Threading.Thread(Spring.Social.GetResponse.GetResponseSync.Sync);
				t.Start(Context);
				ctlDynamicButtons.ErrorText = L10n.Term("GetResponse.LBL_SYNC_BACKGROUND");
#endif
...
#else
				System.Threading.Thread t = new System.Threading.Thread(Spring.Social.GetResponse.GetResponseSync.SyncAll);
				t.Start(Context);
				ctlDynamicButtons.ErrorText = L10n.Term("GetResponse.LBL_SYNC_BACKGROUND");
#endif
				Response.Redirect("default.aspx");
		...
		{
			SetPageTitle(L10n.Term("GetResponse.LBL_GetResponse_SETTINGS"));
			...
			try
			{
				if ( !IsPostBack )
				{
					...
					DIRECTION          .Text    = Sql.ToString (Application["CONFIG.GetResponse.Direction"         ]);
					CONFLICT_RESOLUTION.Text    = Sql.ToString (Application["CONFIG.GetResponse.ConflictResolution"]);
					string sSYNC_MODULES = Sql.ToString(Application["CONFIG.GetResponse.SyncModules"]);
					if ( !Sql.IsEmptyString(sSYNC_MODULES) )
						SYNC_MODULES.Text = Sql.ToString(L10n.Term(".GetResponse_sync_module.", sSYNC_MODULES));
				}
			}
			catch(Exception ex)
			{
