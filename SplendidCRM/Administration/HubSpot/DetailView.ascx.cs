...
namespace SplendidCRM.Administration.HubSpot
{
	...
	public class DetailView : SplendidControl
	{
		...
		protected void Page_Command(Object sender, CommandEventArgs e)
		{
			if ( e.CommandName == "Test" )
			{
				try
				{
					...
					Spring.Social.HubSpot.HubSpotSync.ValidateHubSpot(Application, sbErrors);
			...
			else if ( e.CommandName == "Sync" )
			{
				...
				t.Start(Context);
				ctlDynamicButtons.ErrorText = L10n.Term("HubSpot.LBL_SYNC_BACKGROUND");
#endif
		...
		{
			SetPageTitle(L10n.Term("HubSpot.LBL_HUBSPOT_SETTINGS"));
			...
			try
			{
				if ( !IsPostBack )
				{
					...
					DIRECTION          .Text    = Sql.ToString (Application["CONFIG.HubSpot.Direction"         ]);
					CONFLICT_RESOLUTION.Text    = Sql.ToString (Application["CONFIG.HubSpot.ConflictResolution"]);
					string sSYNC_MODULES = Sql.ToString(Application["CONFIG.HubSpot.SyncModules"]);
					if ( !Sql.IsEmptyString(sSYNC_MODULES) )
						SYNC_MODULES.Text = Sql.ToString(L10n.Term(".hubspot_sync_module.", sSYNC_MODULES));
				}
			}
			catch(Exception ex)
			{
