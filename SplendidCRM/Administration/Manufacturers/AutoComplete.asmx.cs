...
namespace SplendidCRM.Manufacturers
{
	...
	public class AutoComplete : System.Web.Services.WebService
	{
		...
		public Manufacturer MANUFACTURERS_MANUFACTURER_NAME_Get(string sNAME)
		{
			...
			//try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						Security.Filter(cmd, "Manufacturers", "list");
				...
				if ( Sql.IsEmptyGuid(item.ID) )
				{
					...
					throw(new Exception(L10n.Term("Manufacturers.ERR_MANUFACTURER_NOT_FOUND")));
				}
			}
			//catch
			{
		...
		[WebMethod(EnableSession=true)]
		public string[] MANUFACTURERS_MANUFACTURER_NAME_List(string prefixText, int count)
		{
		...
		// so the field name can be NAME or MANUFACTURER_NAME. 
		[WebMethod(EnableSession=true)]
		public string[] MANUFACTURERS_NAME_List(string prefixText, int count)
		{
			return MANUFACTURERS_MANUFACTURER_NAME_List(prefixText, count);
		}
	}
}
