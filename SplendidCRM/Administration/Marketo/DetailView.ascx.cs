...
namespace SplendidCRM.Administration.Marketo
{
	...
	public class DetailView : SplendidControl
	{
		...
		protected void Page_Command(Object sender, CommandEventArgs e)
		{
			if ( e.CommandName == "Test" )
			{
				try
				{
					...
					Spring.Social.Marketo.MarketoSync.ValidateMarketo(Application, sbErrors);
			...
			else if ( e.CommandName == "Sync" )
			{
				...
				t.Start(Context);
				ctlDynamicButtons.ErrorText = L10n.Term("Marketo.LBL_SYNC_BACKGROUND");
#endif
		...
		{
			SetPageTitle(L10n.Term("Marketo.LBL_MARKETO_SETTINGS"));
			...
			try
			{
				if ( !IsPostBack )
				{
					...
					DIRECTION          .Text    = Sql.ToString (Application["CONFIG.Marketo.Direction"         ]);
					CONFLICT_RESOLUTION.Text    = Sql.ToString (Application["CONFIG.Marketo.ConflictResolution"]);
					string sSYNC_MODULES = Sql.ToString(Application["CONFIG.Marketo.SyncModules"]);
					if ( !Sql.IsEmptyString(sSYNC_MODULES) )
						SYNC_MODULES.Text = Sql.ToString(L10n.Term(".marketo_sync_module.", sSYNC_MODULES));
				}
			}
			catch(Exception ex)
			{
