...
namespace SplendidCRM.ProductCategories
{
	...
	public class AutoComplete : System.Web.Services.WebService
	{
		...
		public ProductCategory PRODUCT_CATEGORIES_PRODUCT_CATEGORY_NAME_Get(string sNAME)
		{
			...
			//try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						Security.Filter(cmd, "ProductCategories", "list");
				...
				if ( Sql.IsEmptyGuid(item.ID) )
				{
					...
					throw(new Exception(L10n.Term("ProductCategories.ERR_PRODUCT_CATEGORY_NOT_FOUND")));
				}
			}
			//catch
			{
		...
		[WebMethod(EnableSession=true)]
		public string[] PRODUCT_CATEGORIES_PRODUCT_CATEGORY_NAME_List(string prefixText, int count)
		{
		...
		// so the field name can be NAME or PRODUCT_CATEGORY_NAME. 
		[WebMethod(EnableSession=true)]
		public string[] PRODUCT_CATEGORIES_NAME_List(string prefixText, int count)
		{
			return PRODUCT_CATEGORIES_PRODUCT_CATEGORY_NAME_List(prefixText, count);
		}
	}
}
