							...
							using ( IDbTransaction trn = Sql.BeginTransaction(con) )
							{
								try
								{
									...
									SqlProcs.spPRODUCT_CATEGORIES_Update
										...
										, new DynamicControl(this, "DESCRIPTION").SelectedValue
									...
									// 09/26/2010 Paul.  Fix clear so that it clears vwPRODUCT_CATEGORIES_LISTBOX. 
					...
					if ( !Sql.IsEmptyGuid(gID) || !Sql.IsEmptyGuid(gDuplicateID) )
					{
						...
						using ( IDbConnection con = dbf.CreateConnection() )
						{
							...
							sSQL = "select *                        " + ControlChars.CrLf
							     + "  from vwPRODUCT_CATEGORIES_Edit" + ControlChars.CrLf
