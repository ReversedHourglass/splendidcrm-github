			...
			try
			{
				...
				else if ( e.CommandName == "NewRecord" )
				{
					...
					if ( mgrAjax != null )
					{
						...
						ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "NewRecord", "SelectProductCategory('" + gID.ToString() + "', '" + Sql.EscapeJavaScript(sNAME) + "');", true);
					}
					else
					{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					sSQL = "  from vwPRODUCT_CATEGORIES_List" + ControlChars.CrLf
