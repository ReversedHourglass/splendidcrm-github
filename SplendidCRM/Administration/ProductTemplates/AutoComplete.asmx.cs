...
namespace SplendidCRM.ProductTemplates
{
	...
	public class AutoComplete : System.Web.Services.WebService
	{
		...
		public ProductTemplate PRODUCT_TEMPLATES_PRODUCT_TEMPLATE_NAME_Get(string sNAME)
		{
			...
			//try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					sSQL = "select ID        " + ControlChars.CrLf
					     ...
					     + "  from vwPRODUCT_TEMPLATES" + ControlChars.CrLf;
				...
				if ( Sql.IsEmptyGuid(item.ID) )
				{
					...
					throw(new Exception(L10n.Term("ProductTemplates.ERR_PRODUCT_TEMPLATE_NOT_FOUND")));
				}
			}
			//catch
			{
		...
		[WebMethod(EnableSession=true)]
		public string[] PRODUCT_TEMPLATES_PRODUCT_TEMPLATE_NAME_List(string prefixText, int count)
		{
		...
		// so the field name can be NAME or PRODUCT_TEMPLATE_NAME. 
		[WebMethod(EnableSession=true)]
		public string[] PRODUCT_TEMPLATES_NAME_List(string prefixText, int count)
		{
			return PRODUCT_TEMPLATES_PRODUCT_TEMPLATE_NAME_List(prefixText, count);
		}
	}
}
