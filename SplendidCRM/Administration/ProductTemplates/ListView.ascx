...
<div id="divListView">
	...
	<SplendidCRM:HeaderButtons ID="ctlModuleHeader" Module="ProductTemplates" Title=".moduleList.Home" EnablePrint="true" HelpName="index" EnableHelp="true" Runat="Server" />

	<%@ Register TagPrefix="SplendidCRM" Tagname="SearchView" Src="~/_controls/SearchView.ascx" %>
	<SplendidCRM:SearchView ID="ctlSearchView" Module="ProductTemplates" ShowDuplicateSearch="true" Visible="<%# !PrintView %>" Runat="Server" />
		...
		<Columns>
			...
			<asp:TemplateColumn HeaderText="" ItemStyle-Width="1%" ItemStyle-HorizontalAlign="Center" ItemStyle-Wrap="false">
				<ItemTemplate>
					...
					<asp:HyperLink Visible='<%# !Sql.IsProcessPending(Container) %>' NavigateUrl='<%# "edit.aspx?id=" + Eval("ID") %>' ToolTip='<%# L10n.Term(".LNK_EDIT") %>' Runat="server">
	...
	<asp:Panel ID="pnlMassUpdateSeven" runat="server">
		<%@ Register TagPrefix="SplendidCRM" Tagname="MassUpdate" Src="~/Administration/ProductTemplates/MassUpdate.ascx" %>
		...
		<SplendidCRM:DynamicMassUpdate ID="ctlDynamicMassUpdate" Module="ProductTemplates" Visible="<%# SplendidCRM.Crm.Config.enable_dynamic_mass_update() && !PrintView && !IsMobile && SplendidCRM.Crm.Modules.MassUpdate(m_sMODULE) %>" runat="Server" />
	</asp:Panel>
