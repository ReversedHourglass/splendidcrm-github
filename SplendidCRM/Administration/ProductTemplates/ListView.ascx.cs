			...
			try
			{
								...
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									try
									{
										while ( stk.Count > 0 )
										{
											...
											// 04/20/2009 Paul.  Finally got around to adding support for MassUpdate. 
											...
											SqlProcs.spPRODUCT_TEMPLATES_MassUpdate(sIDs, ctlMassUpdate.DATE_AVAILABLE, ctlMassUpdate.DATE_COST_PRICE, ctlMassUpdate.STATUS, ctlMassUpdate.TAX_CLASS, ctlMassUpdate.SUPPORT_TERM, ctlMassUpdate.ACCOUNT_ID, ctlMassUpdate.PRIMARY_TEAM_ID, ctlMassUpdate.TEAM_SET_LIST, ctlMassUpdate.ADD_TEAM_SET, Guid.Empty, ctlMassUpdate.TYPE_ID, ctlMassUpdate.QUICKBOOKS_ACCOUNT, trn);
										}
										trn.Commit();
									}
									catch(Exception ex)
							...
							using ( IDbConnection con = dbf.CreateConnection() )
							{
								...
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									try
									{
										...
										{
											string sIDs = Utils.BuildMassIDs(stk);
											// 04/20/2009 Paul.  Finally got around to adding support for MassDelete. 
				...
				else if ( e.CommandName == "Export" )
				{
					...
					if ( nACLACCESS  >= 0 )
					{
						...
						if ( vwMain != null )
						{
							// 10/02/2008 Paul.  Products are not assigned to anyone. 
