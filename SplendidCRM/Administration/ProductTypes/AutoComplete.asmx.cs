...
namespace SplendidCRM.ProductTypes
{
	...
	public class AutoComplete : System.Web.Services.WebService
	{
		...
		public ProductType PRODUCT_TYPES_PRODUCT_TYPE_NAME_Get(string sNAME)
		{
			...
			//try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						Security.Filter(cmd, "ProductTypes", "list");
				...
				if ( Sql.IsEmptyGuid(item.ID) )
				{
					...
					throw(new Exception(L10n.Term("ProductTypes.ERR_PRODUCT_TYPE_NOT_FOUND")));
				}
			}
			//catch
			{
		...
		[WebMethod(EnableSession=true)]
		public string[] PRODUCT_TYPES_PRODUCT_TYPE_NAME_List(string prefixText, int count)
		{
		...
		// so the field name can be NAME or PRODUCT_TYPE_NAME. 
		[WebMethod(EnableSession=true)]
		public string[] PRODUCT_TYPES_NAME_List(string prefixText, int count)
		{
			return PRODUCT_TYPES_PRODUCT_TYPE_NAME_List(prefixText, count);
		}
	}
}
