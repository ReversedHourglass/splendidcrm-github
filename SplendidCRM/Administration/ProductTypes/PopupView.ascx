<%@ Control CodeBehind="PopupView.ascx.cs" Language="c#" AutoEventWireup="false" Inherits="SplendidCRM.Administration.ProductTypes.PopupView" %>
<script runat="server">
/**********************************************************************************************************************
...
<div id="divPopupView">
	...
	<SplendidCRM:SearchView ID="ctlSearchView" Module="ProductTypes" IsPopupSearch="true" ShowSearchTabs="false" Visible="<%# !PrintView %>" Runat="Server" />

	<script type="text/javascript">
	function SelectProductType(sPARENT_ID, sPARENT_NAME)
	{
		if ( window.opener != null && window.opener.ChangeProductType != null )
		{
			window.opener.ChangeProductType(sPARENT_ID, sPARENT_NAME);
			window.close();
		}
		else
		{
	...
	function SelectChecked()
	{
		if ( window.opener != null && window.opener.ChangeProductType != null )
		{
			...
			window.opener.ChangeProductType(sSelectedItems, '');
			window.close();
		}
		else
		{
	...
	function Clear()
	{
		if ( window.opener != null && window.opener.ChangeProductType != null )
		{
			window.opener.ChangeProductType('', '');
			window.close();
		}
		else
		{
	...
	<asp:UpdatePanel UpdateMode="Conditional" Visible='<%# !Sql.ToBoolean(Application["CONFIG.disable_popupview_inline"]) %>' runat="server">
		<ContentTemplate>
			<asp:Button ID="btnCreateInline" CommandName="NewRecord.Show" OnCommand="Page_Command" Text='<%# L10n.Term(m_sMODULE + ".LNK_NEW_PRODUCT_TYPE") %>' CssClass="button" style="margin-bottom: 4px;" Visible="<%# !this.IsMobile %>" runat="server" />
