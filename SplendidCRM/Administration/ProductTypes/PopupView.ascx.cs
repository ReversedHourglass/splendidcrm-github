...
namespace SplendidCRM.Administration.ProductTypes
{
	...
	public class PopupView : SplendidControl
	{
		...
		// 10/21/2010 Paul.  Controls to manage inline create. 
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				...
				// 10/21/2010 Paul.  Handle new events that hide and show the NewRecord panel. 
				...
				else if ( e.CommandName == "NewRecord" )
				{
					...
					if ( mgrAjax != null )
					{
						...
						ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "NewRecord", "SelectProductType('" + gID.ToString() + "', '" + Sql.EscapeJavaScript(sNAME) + "');", true);
					}
					else
					{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					// 10/03/2010 Paul.  Fix view to refer to Product Types. 
					sSQL = "  from vwPRODUCT_TYPES" + ControlChars.CrLf
