...
namespace SplendidCRM.SurveyThemes
{
	...
	public class AutoComplete : System.Web.Services.WebService
	{
		...
		public SurveyTheme SURVEY_THEMES_SURVEY_THEME_NAME_Get(string sNAME)
		{
			SurveyTheme item = new SurveyTheme();
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						Security.Filter(cmd, "SurveyThemes", "list");
				...
				if ( Sql.IsEmptyGuid(item.ID) )
				{
					...
					throw(new Exception(L10n.Term("SurveyThemes.ERR_SURVEY_THEME_NOT_FOUND")));
				}
			}
			return item;
		}
		...
		[WebMethod(EnableSession=true)]
		public string[] SURVEY_THEMES_SURVEY_THEME_NAME_List(string prefixText, int count)
		{
		...
		[WebMethod(EnableSession=true)]
		public string[] SURVEY_THEMES_NAME_List(string prefixText, int count)
		{
			return SURVEY_THEMES_SURVEY_THEME_NAME_List(prefixText, count);
		}
	}
}
