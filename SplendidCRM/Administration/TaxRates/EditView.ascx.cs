			...
			if ( e.CommandName == "Save" || e.CommandName == "SaveNew" )
			{
				try
				{
					...
					if ( Page.IsValid )
					{
							...
							using ( IDbTransaction trn = Sql.BeginTransaction(con) )
							{
								try
								{
									...
									SqlProcs.spTAX_RATES_Update
										...
										, new DynamicControl(this, "VALUE"                ).DecimalValue
										, new DynamicControl(this, "LIST_ORDER"           ).IntegerValue
										, new DynamicControl(this, "QUICKBOOKS_TAX_VENDOR").Text
						...
						// 05/26/2019 Paul.  Clear ReactClient. 
			...
			try
			{
				...
				if ( !IsPostBack )
				{
					...
					if ( !Sql.IsEmptyGuid(gID) || !Sql.IsEmptyGuid(gDuplicateID) )
					{
						...
						{
							string sSQL ;
							// 12/10/2012 Paul.  Use the Edit view. 
							sSQL = "select *               " + ControlChars.CrLf
							     + "  from vwTAX_RATES_Edit" + ControlChars.CrLf
								...
								using ( DbDataAdapter da = dbf.CreateDataAdapter() )
								{
									...
									using ( DataTable dtCurrent = new DataTable() )
									{
										...
										if ( dtCurrent.Rows.Count > 0 )
										{
											...
											// 04/07/2016 Paul.  Tax rates per team. 
											bool bEnableTaxRateTeams = Sql.ToBoolean(HttpContext.Current.Application["CONFIG.Orders.EnableTaxRateTeams"]);
											new DynamicControl(this, "TEAM_NAME"          ).Visible = bEnableTaxRateTeams;
											new DynamicControl(this, "TEAM_NAME_LABEL"    ).Visible = bEnableTaxRateTeams;
											new DynamicControl(this, "TEAM_SET_NAME"      ).Visible = bEnableTaxRateTeams;
											new DynamicControl(this, "TEAM_SET_NAME_LABEL").Visible = bEnableTaxRateTeams;
					...
					else
					{
						...
						using ( IDbConnection con = dbf.CreateConnection() )
						{
							...
							using ( IDbCommand cmd = con.CreateCommand() )
							{
								cmd.CommandText = sSQL;
								con.Open();
								new DynamicControl(this, "LIST_ORDER").Text = (Sql.ToInteger(cmd.ExecuteScalar())+1).ToString();
							}
						}
					}
				}
