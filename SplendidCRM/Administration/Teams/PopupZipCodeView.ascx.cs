...
{
	/// <summary>
	///		Summary description for PopupZipCodeView.
	/// </summary>
	public class PopupZipCodeView : SplendidControl
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					{
						sSQL = "  from vwTEAMS_ZIPCODES_List" + ControlChars.CrLf
						     + " where 1 = 1                " + ControlChars.CrLf;
					}
					else
					{
						sSQL = "  from vwTEAMS_ZIPCODES_MyList                 " + ControlChars.CrLf
		...
		{    
			...
			this.AppendGridColumns(grdMain, m_sMODULE + ".PopupZipCodeView", arrSelectFields);
			ctlDynamicButtons.AppendButtons(m_sMODULE + ".PopupZipCode" + (bMultiSelect ? "MultiSelect" : "View"), Guid.Empty, Guid.Empty);
