...
namespace SplendidCRM.Administration.iContact
{
	...
	public class DetailView : SplendidControl
	{
		...
		protected void Page_Command(Object sender, CommandEventArgs e)
		{
			if ( e.CommandName == "Test" )
			{
				try
				{
					...
					Spring.Social.iContact.iContactSync.ValidateiContact(Application, sbErrors);
			...
			else if ( e.CommandName == "Sync" )
			{
				...
				t.Start(Context);
				ctlDynamicButtons.ErrorText = L10n.Term("iContact.LBL_SYNC_BACKGROUND");
#endif
		...
		{
			SetPageTitle(L10n.Term("iContact.LBL_iContact_SETTINGS"));
			...
			try
			{
				if ( !IsPostBack )
				{
					...
					DIRECTION          .Text    = Sql.ToString (Application["CONFIG.iContact.Direction"         ]);
					CONFLICT_RESOLUTION.Text    = Sql.ToString (Application["CONFIG.iContact.ConflictResolution"]);
					string sSYNC_MODULES = Sql.ToString(Application["CONFIG.iContact.SyncModules"]);
					if ( !Sql.IsEmptyString(sSYNC_MODULES) )
						SYNC_MODULES.Text = Sql.ToString(L10n.Term(".iContact_sync_module.", sSYNC_MODULES));
				}
			}
			catch(Exception ex)
			{
