...
{
	/// <summary>
	///		Summary description for KBDocuments.
	/// </summary>
	public class KBDocuments : SubPanelControl
	{
		...
		protected HtmlInputHidden txtKBDOCUMENT_ID  ;
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				...
				{
					case "KBDocuments.Create":
						Response.Redirect("~/KBDocuments/edit.aspx?CASE_ID=" + gID.ToString());
						break;
					case "KBDocuments.Edit":
					{
						Guid gKBDOCUMENT_ID = Sql.ToGuid(e.CommandArgument);
						Response.Redirect("~/KBDocuments/edit.aspx?ID=" + gKBDOCUMENT_ID.ToString());
						break;
					}
					case "KBDocuments.Remove":
					{
						Guid gKBDOCUMENT_ID = Sql.ToGuid(e.CommandArgument);
						if ( bEditView )
						{
							this.DeleteEditViewRelationship(gKBDOCUMENT_ID);
						}
						else
						{
							SqlProcs.spKBDOCUMENTS_CASES_Delete(gKBDOCUMENT_ID, gID);
						}
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bEditView && IsPostBack && arrUPDATED.Count > 0 )
					{
						...
						sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
						     + "     , ID                        as KBDOCUMENT_ID        " + ControlChars.CrLf
						     + "     , NAME                      as KBDOCUMENT_NAME      " + ControlChars.CrLf
						     ...
						     + "  from vwKBDOCUMENTS" + ControlChars.CrLf;
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwCASES_KBDOCUMENTS", Sql.ClientScriptBlock(cmd));
					...
					try
					{
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								...
								{
									this.CreateEditViewRelationships(dt, "KBDOCUMENT_ID");
								}
							}
						}
					}
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach ( Guid gDELETE_ID in arrDELETED )
			{
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spKBDOCUMENTS_CASES_Delete(gDELETE_ID, gPARENT_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spKBDOCUMENTS_CASES_Update(gUPDATE_ID, gPARENT_ID, trn);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			Guid gKBDOCUMENT_ID = Sql.ToGuid(txtKBDOCUMENT_ID.Value);
			if ( !Sql.IsEmptyGuid(gKBDOCUMENT_ID) )
			{
				try
				{
					if ( bEditView )
					{
						this.UpdateEditViewRelationship(gKBDOCUMENT_ID);
					}
					else
					{
						SqlProcs.spKBDOCUMENTS_CASES_Update(gKBDOCUMENT_ID, gID);
					}
					// 05/16/2008 Paul.  Instead of redirecting, just rebind the grid and AJAX will repaint. 
					//Response.Redirect("view.aspx?ID=" + gID.ToString());
					// 05/16/2008 Paul.  If we are not going to redirect,then we must clear the value. 
					txtKBDOCUMENT_ID.Value = String.Empty;
				}
				catch(Exception ex)
				{
					SplendidError.SystemError(new StackTrace(true).GetFrame(0), ex);
		...
		private void InitializeComponent()
		{
			...
			arrSelectFields.Add("KBDOCUMENT_ID"        );
			...
			// 10/05/2017 Paul.  Add Archive relationship view. 
			m_sVIEW_NAME = "vwCASES_KBDOCUMENTS";
