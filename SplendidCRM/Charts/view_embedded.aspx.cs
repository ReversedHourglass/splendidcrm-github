				...
				if ( !IsPostBack )
				{
					if ( !Sql.IsEmptyGuid(gID) )
					{
						...
						if ( dtChart != null && dtChart.Rows.Count > 0 )
						{
							...
							// 05/22/2021 Paul.  Embedded view should not have Edit, Duplicate, Delete buttons. 
							ctlDynamicButtons.AppendButtons(m_sMODULE + ".EmbeddedView", gASSIGNED_USER_ID, rdr);
					...
					{
						// 05/22/2021 Paul.  Embedded view should not have Edit, Duplicate, Delete buttons. 
						ctlDynamicButtons.AppendButtons(m_sMODULE + ".EmbeddedView", Guid.Empty, null);
