	...
	public class Contracts : SplendidControl
	{
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				switch ( e.CommandName )
				{
					...
					// 11/04/2010 Paul.  Delete icon existed, but the command did not. 
					case "Contracts.Delete":
					{
						Guid gCONTRACT_ID = Sql.ToGuid(e.CommandArgument);
						SqlProcs.spCONTRACTS_Delete(gCONTRACT_ID);
						BindGrid();
						break;
					}
					...
					case "Contracts.Create":
						...
						if ( this.IsMobile || Sql.ToBoolean(Application["CONFIG.disable_editview_inline"]) )
							Response.Redirect("~/" + m_sMODULE + "/edit.aspx?B2C_CONTACT_ID=" + gID.ToString());
						else
						{
		...
		protected void BindGrid()
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					Sql.AppendParameter(cmd, gID, "B2C_CONTACT_ID");
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwCONTACTS_CONTRACTS", Sql.ClientScriptBlock(cmd));
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !IsPostBack )
			{
				...
				// 06/03/2010 Paul.  Allow a contract to be created from an Contact. 
		...
		private void InitializeComponent()
		{
			...
			// 10/05/2017 Paul.  Add Archive relationship view. 
			m_sVIEW_NAME = "vwCONTACTS_CONTRACTS";
