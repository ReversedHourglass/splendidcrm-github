...
namespace SplendidCRM.Contacts.HubSpot
{
	/// <summary>
			...
			try
			{
				...
				if ( !IsPostBack )
				{
					if ( !Sql.IsEmptyGuid(gID) )
					{
						...
						{
							string sSQL ;
							sSQL = "select (select SYNC_REMOTE_KEY from vwCONTACTS_SYNC where SYNC_SERVICE_NAME = N'HubSpot' and SYNC_LOCAL_ID = vwCONTACTS.ID) as SYNC_REMOTE_KEY" + ControlChars.CrLf
					...
					if ( nHID > 0 )
					{
						...
						if ( sbErrors.Length == 0 )
						{
							...
							Spring.Social.HubSpot.Api.Contact obj = hubSpot.ContactOperations.GetById(nHID);
							DataRow rdr = Spring.Social.HubSpot.Api.Contact.ConvertToRow(obj);
