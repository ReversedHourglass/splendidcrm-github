...
namespace SplendidCRM.Contacts
{
	...
	public class Invoices : SplendidControl
	{
		...
		protected void BindGrid()
		{
			...
			{
				string sSQL;
				// 02/09/2008 Paul.  where 1 = 1 cannot be used when Security.Filter is used. 
				// 04/26/2008 Paul.  Build the list of fields to use in the select clause.
				// 10/05/2017 Paul.  Add Archive relationship view. 
				// 10/18/2017 Paul.  The Archive view does not separate billing and shipping, so use distinct on the file result set. 
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					// 02/08/2008 Paul.  Make sure to filter relationship data based on team access rights. 
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwCONTACTS_INVOICES", Sql.ClientScriptBlock(cmd));
		...
		private void InitializeComponent()
		{
			...
			// 10/05/2017 Paul.  Add Archive relationship view. 
			m_sVIEW_NAME = "vwCONTACTS_INVOICES";
