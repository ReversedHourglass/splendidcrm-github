...
namespace SplendidCRM.Contacts.Marketo
{
	/// <summary>
			...
			try
			{
				...
				if ( !IsPostBack )
				{
					if ( !Sql.IsEmptyGuid(gID) )
					{
						...
						{
							string sSQL ;
							sSQL = "select (select SYNC_REMOTE_KEY from vwCONTACTS_SYNC where SYNC_SERVICE_NAME = N'Marketo' and SYNC_LOCAL_ID = vwCONTACTS.ID) as SYNC_REMOTE_KEY" + ControlChars.CrLf
