<%@ Control CodeBehind="Orders.ascx.cs" Language="c#" AutoEventWireup="false" Inherits="SplendidCRM.Contacts.Orders" %>
<script runat="server">
/**********************************************************************************************************************
...
<SplendidCRM:SubPanelButtons ID="ctlDynamicButtons" Module="Orders" SubPanel="divContactsOrders" Title="Orders.LBL_MODULE_NAME" Runat="Server" />

<div id="divContactsOrders" style='<%= "display:" + (CookieValue("divContactsOrders") != "1" ? "inline" : "none") %>'>
			...
			<asp:TemplateColumn HeaderText="" ItemStyle-Width="1%" ItemStyle-HorizontalAlign="Left" ItemStyle-Wrap="false">
				<ItemTemplate>
					...
					<span onclick="return confirm('<%= L10n.TermJavaScript(".NTC_DELETE_CONFIRMATION") %>')">
						<asp:ImageButton Visible='<%# SplendidCRM.Security.GetRecordAccess(Container, "Contacts", "remove", "CONTACT_ASSIGNED_USER_ID") >= 0 && !ArchiveViewExists() %>' CommandName="Orders.Remove" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "ORDER_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" AlternateText='<%# L10n.Term(".LNK_REMOVE") %>' SkinID="delete_inline" Runat="server" />
						<asp:LinkButton  Visible='<%# SplendidCRM.Security.GetRecordAccess(Container, "Contacts", "remove", "CONTACT_ASSIGNED_USER_ID") >= 0 && !ArchiveViewExists() %>' CommandName="Orders.Remove" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "ORDER_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" Text='<%# L10n.Term(".LNK_REMOVE") %>' Runat="server" />
					</span>
				</ItemTemplate>
			</asp:TemplateColumn>
		</Columns>
