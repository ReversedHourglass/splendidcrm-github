...
<SplendidCRM:SubPanelButtons ID="ctlDynamicButtons" Module="Quotes" SubPanel="divContactsQuotes" Title="Quotes.LBL_MODULE_NAME" Runat="Server" />

<div id="divContactsQuotes" style='<%= "display:" + (CookieValue("divContactsQuotes") != "1" ? "inline" : "none") %>'>
			...
			<asp:TemplateColumn HeaderText="" ItemStyle-Width="1%" ItemStyle-HorizontalAlign="Left" ItemStyle-Wrap="false">
				<ItemTemplate>
					...
					<span onclick="return confirm('<%= L10n.TermJavaScript(".NTC_DELETE_CONFIRMATION") %>')">
						<asp:ImageButton Visible='<%# SplendidCRM.Security.GetRecordAccess(Container, "Contacts", "remove", "CONTACT_ASSIGNED_USER_ID") >= 0 && !ArchiveViewExists() %>' CommandName="Quotes.Remove" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "QUOTE_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" AlternateText='<%# L10n.Term(".LNK_REMOVE") %>' SkinID="delete_inline" Runat="server" />
						<asp:LinkButton  Visible='<%# SplendidCRM.Security.GetRecordAccess(Container, "Contacts", "remove", "CONTACT_ASSIGNED_USER_ID") >= 0 && !ArchiveViewExists() %>' CommandName="Quotes.Remove" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "QUOTE_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" Text='<%# L10n.Term(".LNK_REMOVE") %>' Runat="server" />
					</span>
				</ItemTemplate>
			</asp:TemplateColumn>
		</Columns>
