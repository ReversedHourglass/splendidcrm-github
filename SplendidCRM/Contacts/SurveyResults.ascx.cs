	...
	public class SurveyResults : SplendidControl
	{
		...
		protected void BindGrid()
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				m_sVIEW_NAME = "vwCONTACTS_SURVEY_RESULTS";
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwCONTACTS_SURVEY_RESULTS", Sql.ClientScriptBlock(cmd));
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !IsPostBack )
			{
				ctlDynamicButtons.AppendButtons("Contacts." + m_sMODULE, Guid.Empty, gID);
			}
		}
		...
		private void InitializeComponent()
		{
			...
			this.AppendGridColumns(grdMain, "Contacts." + m_sMODULE, arrSelectFields, Page_Command);
			if ( IsPostBack )
