...
namespace SplendidCRM.Contracts
{
	...
	public class AutoComplete : System.Web.Services.WebService
	{
		...
		public Contract CONTRACTS_CONTRACT_NAME_Get(string sNAME)
		{
			...
			//try
			{
				...
				if ( Sql.IsEmptyGuid(item.ID) )
				{
					...
					throw(new Exception(L10n.Term("Contracts.ERR_CONTRACT_NOT_FOUND")));
				}
			}
			//catch
			{
		...
		[WebMethod(EnableSession=true)]
		public string[] CONTRACTS_CONTRACT_NAME_List(string prefixText, int count)
		{
		...
		// so the field name can be NAME or CONTRACT_NAME. 
		[WebMethod(EnableSession=true)]
		public string[] CONTRACTS_NAME_List(string prefixText, int count)
		{
			return CONTRACTS_CONTRACT_NAME_List(prefixText, count);
		}
	}
}
