...
<SplendidCRM:SubPanelButtons ID="ctlDynamicButtons" Module="Documents" SubPanel="divContractsDocuments" Title="Documents.LBL_MODULE_NAME" Runat="Server" />

<div id="divContractsDocuments" style='<%= "display:" + (CookieValue("divContractsDocuments") != "1" ? "inline" : "none") %>'>
			...
			<asp:TemplateColumn HeaderText="" ItemStyle-Width="8%" ItemStyle-HorizontalAlign="Left" ItemStyle-Wrap="false">
				<ItemTemplate>
					...
					<div style="DISPLAY: <%# Sql.ToString(DataBinder.Eval(Container.DataItem, "REVISION")) != Sql.ToString(DataBinder.Eval(Container.DataItem, "SELECTED_REVISION")) ? "inline" : "none" %>">
						<asp:ImageButton Visible='<%# SplendidCRM.Security.GetRecordAccess(Container, "Contracts", "edit", "CONTRACT_ASSIGNED_USER_ID") >= 0 && !ArchiveViewExists() %>' CommandName="Documents.GetLatest" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "DOCUMENT_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" AlternateText='<%# L10n.Term(".LNK_GET_LATEST") %>' SkinID="getLatestDocument" Runat="server" />
						<asp:LinkButton  Visible='<%# SplendidCRM.Security.GetRecordAccess(Container, "Contracts", "edit", "CONTRACT_ASSIGNED_USER_ID") >= 0 && !ArchiveViewExists() %>' CommandName="Documents.GetLatest" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "DOCUMENT_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" Text='<%# L10n.Term(".LNK_GET_LATEST") %>' Runat="server" />
					</div>
