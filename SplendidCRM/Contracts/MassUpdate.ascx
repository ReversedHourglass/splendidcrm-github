...
<SplendidCRM:MassUpdateButtons ID="ctlDynamicButtons" SubPanel="divContractsMassUpdate" Title=".LBL_MASS_UPDATE_TITLE" Runat="Server" />

<%-- 03/26/2018 Paul.  Hide mass update fields in ArchiveView. --%>
<div id="divContractsMassUpdate" style='<%= "display:" + (CookieValue("divContractsMassUpdate") != "1" && !ArchiveView() ? "inline" : "none") %>'>
				...
				<asp:Table Width="100%" CellPadding="0" CellSpacing="0" runat="server">
					<asp:TableRow>
						<asp:TableCell Width="15%" CssClass="dataLabel">
							<asp:Label Text='<%# L10n.Term("Contracts.LBL_ACCOUNT_NAME") %>' Visible='<%# Sql.ToString(Application["CONFIG.BusinessMode"]) != "B2C" %>' runat="server" />
						</asp:TableCell>
