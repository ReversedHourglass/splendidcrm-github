	...
	public class Products : SplendidControl
	{
		...
		protected void BindGrid()
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				m_sVIEW_NAME = "vwCONTRACTS_PRODUCTS";
					...
					try
					{
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								...
								this.ApplyGridViewRules("Contracts." + m_sMODULE, dt);
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gPRODUCT_ID) )
			{
				try
				{
					...
					txtPRODUCT_ID.Value = String.Empty;
				}
				catch(Exception ex)
				{
					SplendidError.SystemError(new StackTrace(true).GetFrame(0), ex);
			...
			if ( !IsPostBack )
			{
				...
				ctlDynamicButtons.AppendButtons("Contracts." + m_sMODULE, gASSIGNED_USER_ID, gID);
			}
		}
		...
		private void InitializeComponent()
		{
			...
			arrSelectFields.Add("PRODUCT_ID"               );
			...
			this.AppendGridColumns(grdMain, "Contracts." + m_sMODULE, arrSelectFields, Page_Command);
			// 04/28/2008 Paul.  Make use of dynamic buttons. 
			if ( IsPostBack )
				ctlDynamicButtons.AppendButtons("Contracts." + m_sMODULE, Guid.Empty, Guid.Empty);
		}
		#endregion
	}
}
