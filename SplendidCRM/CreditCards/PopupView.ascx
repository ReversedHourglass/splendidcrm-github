<%@ Control CodeBehind="PopupView.ascx.cs" Language="c#" AutoEventWireup="false" Inherits="SplendidCRM.CreditCards.PopupView" %>
<script runat="server">
/**********************************************************************************************************************
...
<div id="divPopupView">
	...
	<SplendidCRM:SearchView ID="ctlSearchView" Module="CreditCards" IsPopupSearch="true" ShowSearchTabs="false" Visible="<%# !PrintView %>" Runat="Server" />

	<script type="text/javascript">
	function SelectCreditCard(sPARENT_ID, sPARENT_NAME)
	{
		if ( window.opener != null && window.opener.ChangeCreditCard != null )
		{
			window.opener.ChangeCreditCard(sPARENT_ID, sPARENT_NAME);
			window.close();
		}
		else
		{
			alert('Original window has closed.  CreditCard cannot be assigned.' + '\n' + sPARENT_ID + '\n' + sPARENT_NAME);
		}
	}
	function SelectChecked()
	{
		if ( window.opener != null && window.opener.ChangeCreditCard != null )
		{
			...
			window.opener.ChangeCreditCard(sSelectedItems, '');
			window.close();
		}
		else
		{
	...
	function Clear()
	{
		if ( window.opener != null && window.opener.ChangeCreditCard != null )
		{
			window.opener.ChangeCreditCard('', '');
			window.close();
		}
		else
		{
	...
	<SplendidCRM:ListHeader Title="CreditCards.LBL_LIST_FORM_TITLE" Runat="Server" />
	...
	<asp:UpdatePanel UpdateMode="Conditional" Visible='<%# !Sql.ToBoolean(Application["CONFIG.disable_popupview_inline"]) %>' runat="server">
		<ContentTemplate>
			<asp:Button ID="btnCreateInline" CommandName="NewRecord.Show" OnCommand="Page_Command" Text='<%# L10n.Term(m_sMODULE + ".LNK_NEW_CREDIT_CARD") %>' CssClass="button" style="margin-bottom: 4px;" Visible="<%# !this.IsMobile %>" runat="server" />
