...
namespace SplendidCRM.CreditCards
{
	...
	public class PopupView : SplendidControl
	{
			...
			try
			{
				...
				else if ( e.CommandName == "NewRecord" )
				{
					...
					if ( mgrAjax != null )
					{
						...
						ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "NewRecord", "SelectCreditCard('" + gID.ToString() + "', '" + Sql.EscapeJavaScript(sNAME) + "');", true);
					}
					else
					{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				// 02/10/2008 Paul.  We are not going to allow browsing all credit cards, require the account ID. 
				Guid gACCOUNT_ID = Sql.ToGuid(Request["ACCOUNT_ID"]);
				// 01/11/2011 Paul.  Need to allow credit cards to be associated with a contact. 
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					sSQL = "  from vwCREDIT_CARDS_List" + ControlChars.CrLf;
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						{
							Sql.AppendParameter(cmd, gCONTACT_ID, "CONTACT_ID");
							new DynamicControl(ctlSearchView, "CONTACT_NAME").Visible = false;
						}
						else if ( !Sql.IsEmptyGuid(gACCOUNT_ID) )
						{
							Sql.AppendParameter(cmd, gACCOUNT_ID, "ACCOUNT_ID");
							new DynamicControl(ctlSearchView, "ACCOUNT_NAME").Visible = false;
						}
						else
						{
							// 01/11/2011 Paul.  We are not going to allow browsing all credit cards, require the account ID. 
							cmd.CommandText += "   and 1 = 0" + ControlChars.CrLf;
							// 08/05/2010 Paul.  The Account is required for Inline Editing. 
							btnCreateInline   .Visible = false;
							pnlNewRecordInline.Visible = false;
						}
				...
				if ( !IsPostBack )
				{
					...
					ctlNewRecord.CONTACT_ID = gCONTACT_ID;
				}
			}
			catch(Exception ex)
			{
