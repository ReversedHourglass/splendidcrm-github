...
namespace SplendidCRM.Documents
{
	...
	public class Quotes : SubPanelControl
	{
		...
		protected SplendidCRM.Quotes.NewRecord ctlNewRecord   ;
			...
			try
			{
				switch ( e.CommandName )
				{
					case "Documents.GetLatest":
					{
						...
						SqlProcs.spDOCUMENTS_QUOTES_GetLatest(gQUOTE_ID, gID);
						BindGrid();
						break;
					}
					case "Quotes.Edit":
		...
		protected void BindGrid()
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwDOCUMENTS_QUOTES", Sql.ClientScriptBlock(cmd));
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach ( Guid gDELETE_ID in arrDELETED )
			{
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spDOCUMENTS_QUOTES_Delete(gDELETE_ID, gPARENT_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spDOCUMENTS_QUOTES_Update(gUPDATE_ID, gPARENT_ID, trn);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gQUOTE_ID) )
			{
				try
				{
					...
					{
						SqlProcs.spDOCUMENTS_QUOTES_Update(gQUOTE_ID, gID);
					}
		...
		private void InitializeComponent()
		{
			...
			// 10/05/2017 Paul.  Add Archive relationship view. 
			m_sVIEW_NAME = "vwDOCUMENTS_QUOTES";
