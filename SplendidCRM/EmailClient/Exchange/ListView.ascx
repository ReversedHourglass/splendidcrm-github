<%@ Control Language="c#" AutoEventWireup="false" Codebehind="ListView.ascx.cs" Inherits="SplendidCRM.EmailClient.Exchange.ListView" TargetSchema="http://schemas.microsoft.com/intellisense/ie5" %>
<script runat="server">
/**********************************************************************************************************************
					...
					<asp:TableCell BorderWidth="1" VerticalAlign="Top">
						<div id="divMain" style="overflow-x: auto; overflow-y: scroll; height: 350px; border-bottom: 1px solid black;" runat="server">
							...
							<div style="width: 97%;">
								<asp:UpdatePanel UpdateMode="Conditional" runat="server">
									<ContentTemplate>
										<SplendidCRM:SplendidGrid id="grdMain" SkinID="grdListView" AllowCustomPaging="true" AllowPaging="true" EnableViewState="true" runat="server">
											<Columns>
												<asp:TemplateColumn>
													<ItemTemplate><asp:Button Text="Select" CommandName="Select" CommandArgument='<%# Eval("UNIQUE_ID") %>' style="display: none;" runat="server" />
												...
												<asp:TemplateColumn     HeaderText="EmailClient.LBL_LIST_FROM"      SortExpression="FROM" ItemStyle-Width="20%" ItemStyle-Wrap="true">
													<ItemTemplate><asp:Label Font-Bold='<%# !Sql.ToBoolean(Eval("IS_READ")) %>' Text='<%# HttpUtility.HtmlEncode(Sql.ToString(Eval("FROM"))) %>' runat="server" /></ItemTemplate>
												</asp:TemplateColumn>
