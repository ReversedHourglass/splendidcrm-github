...
{
	/// <summary>
	/// Summary description for ExchangeService2007
	/// </summary>
	[WebServiceBinding(ConformsTo = WsiProfiles.BasicProfile1_1)]
	[ToolboxItem(false)]
	public class ExchangeService2007 : System.Web.Services.WebService, INotificationServiceBinding
	{
			...
			try
			{
				foreach ( ResponseMessageType rmt in SendNotification1.ResponseMessages.Items )
				{
					if ( rmt.ResponseCode != ResponseCodeType.NoError )
					{
						...
						SyncError.SystemMessage(Context, "Error", new StackTrace(true).GetFrame(0), "ExchangeService2007.SendNotification: " + rmt.ResponseCode.ToString() + " - " + rmt.MessageText);
						SplendidError.SystemMessage(Context, "Error", new StackTrace(true).GetFrame(0), "ExchangeService2007.SendNotification: " + rmt.ResponseCode.ToString() + " - " + rmt.MessageText);
					}
					...
					if ( rmt is SendNotificationResponseMessageType )
					{
						...
						{
							result.SubscriptionStatus = SubscriptionStatusType.Unsubscribe;
							SyncError.SystemMessage(Context, "Error", new StackTrace(true).GetFrame(0), "ExchangeService2007.SendNotification: Subscription not found for " + notification.SubscriptionId);
						}
						else
						{
							...
							Microsoft.Exchange.WebServices.Data.ExchangeService service = ExchangeUtils.CreateExchangeService(Application, sEXCHANGE_ALIAS, sEXCHANGE_EMAIL, sMAIL_SMTPUSER, sMAIL_SMTPPASS, gEXCHANGE_ID);
							...
							using ( IDbConnection con = dbf.CreateConnection() )
							{
								...
								foreach ( BaseNotificationEventType evtBase in notification.Items )
								{
									...
									if ( evtBase is MovedCopiedEventType )
									{
										...
										if ( bVERBOSE_EVENTS )
											SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "ExchangeService2007.SendNotification: MovedCopiedEventType - " + sParentFolderId + ", " + sItemID);
									}
									else if ( evtBase is ModifiedEventType )
									{
										...
										if ( evt.Item is ItemIdType )
											sItemID = (evt.Item as ItemIdType).Id;
										if ( bVERBOSE_EVENTS )
											SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "ExchangeService2007.SendNotification: ModifiedEventType - " + sParentFolderId + ", " + sItemID);
									}
									...
									else if ( evtBase is BaseObjectChangedEventType )
									{
										...
										if ( evt.Item is ItemIdType )
											sItemID = (evt.Item as ItemIdType).Id;
										if ( bVERBOSE_EVENTS )
											SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "ExchangeService2007.SendNotification: BaseObjectChangedEventType - " + sParentFolderId + ", " + sItemID);
									}
									else
									{
										if ( bVERBOSE_EVENTS )
											SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "ExchangeService2007.SendNotification: " + evtBase.GetType().FullName);
									}
									if ( !Sql.IsEmptyString(sItemID) && !Sql.IsEmptyString(sParentFolderId) )
									{
										...
										if ( vwUserFolders.Count > 0 )
										{
											...
											if ( bVERBOSE_EVENTS )
												SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "ExchangeService2007.SendNotification: " + sEXCHANGE_ALIAS + " event in " + sMODULE_NAME + " " + sPARENT_NAME);
