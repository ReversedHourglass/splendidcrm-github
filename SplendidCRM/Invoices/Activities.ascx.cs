...
namespace SplendidCRM.Invoices
{
	...
	public class Activities : SplendidControl
	{
		...
		protected void BindGrid()
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				else
				{
					...
					m_sVIEW_NAME = "vwINVOICES_ACTIVITIES_OPEN";
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						if ( bDebug )
							RegisterClientScriptBlock("vwINVOICES_ACTIVITIES_OPEN", Sql.ClientScriptBlock(cmd));
				...
				// 10/05/2017 Paul.  Add Archive relationship view. 
				// 11/01/2017 Paul.  Use a module-based flag so that Record Level Security is only enabled when needed. 
				m_sVIEW_NAME = "vwINVOICES_ACTIVITIES" + (ArchiveViewEnabled() ? "_ARCHIVE" : "_HISTORY");
				sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
				     + Sql.AppendRecordLevelSecurityField("Activities", "edit", m_sVIEW_NAME)
				     + "  from " + m_sVIEW_NAME + ControlChars.CrLf;
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwINVOICES_ACTIVITIES_HISTORY", Sql.ClientScriptBlock(cmd));
		...
		private void InitializeComponent()
		{
			...
			// 10/05/2017 Paul.  Add Archive relationship view. 
			m_sVIEW_NAME = "vwINVOICES_ACTIVITIES";
			if ( !ArchiveViewEnabled() )
