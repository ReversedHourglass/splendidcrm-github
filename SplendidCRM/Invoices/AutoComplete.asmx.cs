...
namespace SplendidCRM.Invoices
{
	...
	public class AutoComplete : System.Web.Services.WebService
	{
		...
		[WebMethod(EnableSession=true)]
		public Invoice INVOICES_INVOICE_NAME_Get(string sNAME)
		{
			Invoice item = new Invoice();
			//try
			{
				...
				if ( Sql.IsEmptyGuid(item.ID) )
				{
					...
					throw(new Exception(L10n.Term("Invoices.ERR_INVOICE_NOT_FOUND")));
				}
			}
			//catch
			{
		...
		// 03/29/2007 Paul.  In order for AutoComplete to work, the parameter names must be "prefixText" and "count". 
		[WebMethod(EnableSession=true)]
		public string[] INVOICES_INVOICE_NAME_List(string prefixText, int count)
		{
			string[] arrItems = new string[0];
			try
			{
		...
		// so the field name can be NAME or INVOICE_NAME. 
		[WebMethod(EnableSession=true)]
		public string[] INVOICES_NAME_List(string prefixText, int count)
		{
			return INVOICES_INVOICE_NAME_List(prefixText, count);
		}

		[WebMethod(EnableSession=true)]
		public Invoice INVOICES_INVOICE_NUM_Get(string sNAME)
		{
			Invoice item = new Invoice();
			//try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						Sql.AppendParameter(cmd, sNAME, Sql.SqlFilterMode.StartsWith, "INVOICE_NUM");
						// 07/02/2007 Paul.  Sort is important so that the first match is selected. 
						cmd.CommandText += " order by INVOICE_NUM" + ControlChars.CrLf;
						using ( IDataReader rdr = cmd.ExecuteReader(CommandBehavior.SingleRow) )
						{
							if ( rdr.Read() )
							{
								item.ID   = Sql.ToGuid   (rdr["ID"  ]);
								item.NAME = Sql.ToString (rdr["INVOICE_NUM"]);
							}
						}
					}
				}
		...
		// 03/29/2007 Paul.  In order for AutoComplete to work, the parameter names must be "prefixText" and "count". 
		[WebMethod(EnableSession=true)]
		public string[] INVOICES_INVOICE_NUM_List(string prefixText, int count)
		{
			string[] arrItems = new string[0];
			try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						Security.Filter(cmd, "Invoices", "list");
						// 07/12/2010 Paul.  It does not make sense to allow a fuzzy search for a number. 
						Sql.AppendParameter(cmd, prefixText, Sql.SqlFilterMode.StartsWith, "INVOICE_NUM");
						cmd.CommandText += " order by INVOICE_NUM" + ControlChars.CrLf;
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							((IDbDataAdapter)da).SelectCommand = cmd;
							using ( DataTable dt = new DataTable() )
							{
								...
								arrItems = new string[dt.Rows.Count];
								for ( int i=0; i < dt.Rows.Count; i++ )
									arrItems[i] = Sql.ToString(dt.Rows[i]["INVOICE_NUM"]);
							}
						}
					}
				}
