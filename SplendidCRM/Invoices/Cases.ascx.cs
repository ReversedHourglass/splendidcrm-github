...
namespace SplendidCRM.Invoices
{
	...
	public class Cases : SubPanelControl
	{
			...
			try
			{
				switch ( e.CommandName )
				{
					...
					case "Cases.Remove":
					{
						...
						{
							SqlProcs.spINVOICES_CASES_Delete(gID, gCASE_ID);
						}
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bEditView && IsPostBack && arrUPDATED.Count > 0 )
					{
						...
						arrSelectFields.Remove("INVOICE_ID"              );
						arrSelectFields.Remove("INVOICE_NAME"            );
						arrSelectFields.Remove("INVOICE_ASSIGNED_USER_ID");
						sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
						     + "     , ID                        as CASE_ID                 " + ControlChars.CrLf
						     + "     , NAME                      as CASE_NAME               " + ControlChars.CrLf
						     + "     , @INVOICE_ID               as INVOICE_ID              " + ControlChars.CrLf
						     + "     , @INVOICE_NAME             as INVOICE_NAME            " + ControlChars.CrLf
						     + "     , @INVOICE_ASSIGNED_USER_ID as INVOICE_ASSIGNED_USER_ID" + ControlChars.CrLf
						     + "  from vwCASES" + ControlChars.CrLf;
						cmd.CommandText = sSQL;
						Sql.AddParameter(cmd, "@INVOICE_ID"              , gID);
						Sql.AddParameter(cmd, "@INVOICE_NAME"            , Sql.ToString(Page.Items["NAME"            ]));
						Sql.AddParameter(cmd, "@INVOICE_ASSIGNED_USER_ID", Sql.ToGuid  (Page.Items["ASSIGNED_USER_ID"]));
					...
					else
					{
						...
						Sql.AppendParameter(cmd, gID, "INVOICE_ID");
					}
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwINVOICES_CASES", Sql.ClientScriptBlock(cmd));
					...
					try
					{
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								...
								// 10/05/2017 Paul.  Add Archive relationship view. 
								this.ApplyGridViewRules("Invoices." + this.LayoutListView, dt);
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach ( Guid gDELETE_ID in arrDELETED )
			{
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spINVOICES_CASES_Delete(gPARENT_ID, gDELETE_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spINVOICES_CASES_Update(gPARENT_ID, gUPDATE_ID, trn);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gCASE_ID) )
			{
				try
				{
					...
					{
						SqlProcs.spINVOICES_CASES_Update(gID, gCASE_ID);
					}
					// 05/16/2008 Paul.  Instead of redirecting, just rebind the grid and AJAX will repaint. 
					//Response.Redirect("view.aspx?ID=" + gID.ToString());
			...
			if ( !IsPostBack )
			{
				...
				// 10/05/2017 Paul.  Add Archive relationship view. 
				ctlDynamicButtons.AppendButtons("Invoices." + this.LayoutListView, gASSIGNED_USER_ID, gID);
		...
		private void InitializeComponent()
		{
			...
			// 10/05/2017 Paul.  Add Archive relationship view. 
			m_sVIEW_NAME = "vwINVOICES_CASES";
			if ( ArchiveViewExists() )
				m_sVIEW_NAME = m_sVIEW_NAME + "_ARCHIVE";
			this.LayoutListView = m_sMODULE + (ArchiveView() ? ".ArchiveView" : String.Empty);
			this.AppendGridColumns(grdMain, "Invoices." + this.LayoutListView, arrSelectFields, Page_Command);
			// 04/28/2008 Paul.  Make use of dynamic buttons. 
			if ( IsPostBack )
				ctlDynamicButtons.AppendButtons("Invoices." + this.LayoutListView, Guid.Empty, Guid.Empty);
		}
		#endregion
	}
}
