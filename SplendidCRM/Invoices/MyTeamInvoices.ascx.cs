...
{
	/// <summary>
	///		Summary description for MyTeamInvoices.
	/// </summary>
	public class MyTeamInvoices : DashletControl
	{
		...
		protected void Bind(bool bBind)
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				m_sVIEW_NAME = "vwINVOICES_MyList";
				sSQL = "  from " + m_sVIEW_NAME + ControlChars.CrLf;
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					cmd.CommandText = sSQL;
					Security.Filter(cmd, m_sMODULE, "list");
					grdMain.OrderByClause("DUE_DATE", "desc");
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwINVOICES_MyList", Sql.ClientScriptBlock(cmd));
					...
					try
					{
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								...
								this.ApplyGridViewRules(m_sMODULE + ".MyInvoices", dt);
		...
		private void InitializeComponent()
		{
			...
			arrSelectFields.Add("DUE_DATE"        );
			arrSelectFields.Add("ASSIGNED_USER_ID");
			arrSelectFields.Add("TEAM_ID"         );
			this.AppendGridColumns(grdMain, m_sMODULE + ".MyInvoices", arrSelectFields);
		}
		#endregion
	}
}
