...
namespace SplendidCRM.KBDocuments
{
	...
	public class AutoComplete : System.Web.Services.WebService
	{
		...
		public KBDocument KBDOCUMENTS_KBDOCUMENT_NAME_Get(string sNAME)
		{
			...
			//try
			{
				...
				if ( Sql.IsEmptyGuid(item.ID) )
				{
					...
					throw(new Exception(L10n.Term("KBDocuments.ERR_KBDOCUMENT_NOT_FOUND")));
				}
			}
			//catch
			{
		...
		[WebMethod(EnableSession=true)]
		public string[] KBDOCUMENTS_KBDOCUMENT_NAME_List(string prefixText, int count)
		{
		...
		// so the field name can be NAME or KBDOCUMENT_NAME. 
		[WebMethod(EnableSession=true)]
		public string[] KBDOCUMENTS_NAME_List(string prefixText, int count)
		{
			return KBDOCUMENTS_KBDOCUMENT_NAME_List(prefixText, count);
		}
	}
}
