...
namespace SplendidCRM.KBDocuments
{
	...
	public class Cases : SubPanelControl
	{
		...
		// 02/21/2010 Paul.  Make sure to point to the KBDocuments.NewRecord. 
			...
			try
			{
				switch ( e.CommandName )
				{
					...
					case "Cases.Remove":
					{
						...
						{
							SqlProcs.spKBDOCUMENTS_CASES_Delete(gID, gCASE_ID);
						}
						BindGrid();
						break;
					}
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bEditView && IsPostBack && arrUPDATED.Count > 0 )
					{
						...
						arrSelectFields.Remove("KBDOCUMENT_ID"              );
						arrSelectFields.Remove("KBDOCUMENT_NAME"            );
						arrSelectFields.Remove("KBDOCUMENT_ASSIGNED_USER_ID");
						sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
						     + "     , ID                           as CASE_ID                    " + ControlChars.CrLf
						     + "     , NAME                         as CASE_NAME                  " + ControlChars.CrLf
						     + "     , @KBDOCUMENT_ID               as KBDOCUMENT_ID              " + ControlChars.CrLf
						     + "     , @KBDOCUMENT_NAME             as KBDOCUMENT_NAME            " + ControlChars.CrLf
						     + "     , @KBDOCUMENT_ASSIGNED_USER_ID as KBDOCUMENT_ASSIGNED_USER_ID" + ControlChars.CrLf
						     + "  from vwCASES" + ControlChars.CrLf;
						cmd.CommandText = sSQL;
						Sql.AddParameter(cmd, "@KBDOCUMENT_ID"              , gID);
						Sql.AddParameter(cmd, "@KBDOCUMENT_NAME"            , Sql.ToString(Page.Items["NAME"            ]));
						Sql.AddParameter(cmd, "@KBDOCUMENT_ASSIGNED_USER_ID", Sql.ToGuid  (Page.Items["ASSIGNED_USER_ID"]));
					...
					else
					{
						...
						Sql.AppendParameter(cmd, gID, "KBDOCUMENT_ID");
					}
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwKBDOCUMENTS_CASES", Sql.ClientScriptBlock(cmd));
					...
					try
					{
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								...
								// 10/05/2017 Paul.  Add Archive relationship view. 
								this.ApplyGridViewRules("KBDocuments." + this.LayoutListView, dt);
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach ( Guid gDELETE_ID in arrDELETED )
			{
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spKBDOCUMENTS_CASES_Delete(gPARENT_ID, gDELETE_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spKBDOCUMENTS_CASES_Update(gPARENT_ID, gUPDATE_ID, trn);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gCASE_ID) )
			{
				try
				{
					...
					{
						SqlProcs.spKBDOCUMENTS_CASES_Update(gID, gCASE_ID);
					}
			...
			if ( !IsPostBack )
			{
				...
				// 10/05/2017 Paul.  Add Archive relationship view. 
				ctlDynamicButtons.AppendButtons("KBDocuments." + this.LayoutListView, gASSIGNED_USER_ID, gID);
		...
		private void InitializeComponent()
		{
			...
			arrSelectFields.Add("KBDOCUMENT_ASSIGNED_USER_ID");
			// 11/26/2005 Paul.  Add fields early so that sort events will get called. 
			// 06/07/2015 Paul.  Must include Page_Command in order for Preview to fire. 
			// 10/05/2017 Paul.  Add Archive relationship view. 
			m_sVIEW_NAME = "vwKBDOCUMENTS_CASES";
			if ( ArchiveViewExists() )
				m_sVIEW_NAME = m_sVIEW_NAME + "_ARCHIVE";
			this.LayoutListView = m_sMODULE + (ArchiveView() ? ".ArchiveView" : String.Empty);
			this.AppendGridColumns(grdMain, "KBDocuments." + this.LayoutListView, arrSelectFields, Page_Command);
			// 04/28/2008 Paul.  Make use of dynamic buttons. 
			if ( IsPostBack )
				ctlDynamicButtons.AppendButtons("KBDocuments." + this.LayoutListView, Guid.Empty, Guid.Empty);
		}
		#endregion
	}
}
