...
namespace SplendidCRM.KBDocuments.KBTags
{
	...
	public class AutoComplete : System.Web.Services.WebService
	{
		...
		public KBTag KBTAGS_KBTAG_NAME_Get(string sNAME)
		{
			KBTag item = new KBTag();
			//try
			{
				...
				if ( Sql.IsEmptyGuid(item.ID) )
				{
					...
					throw(new Exception(L10n.Term("KBTags.ERR_KBTAG_NOT_FOUND")));
				}
			}
			//catch
			{
		...
		[WebMethod(EnableSession=true)]
		public string[] KBTAGS_KBTAG_NAME_List(string prefixText, int count)
		{
		...
		// so the field name can be NAME or KBTAG_NAME. 
		[WebMethod(EnableSession=true)]
		public string[] KBTAGS_NAME_List(string prefixText, int count)
		{
			return KBTAGS_KBTAG_NAME_List(prefixText, count);
		}
	}
}
