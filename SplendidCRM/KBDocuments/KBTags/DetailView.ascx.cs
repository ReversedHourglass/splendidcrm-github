...
namespace SplendidCRM.KBDocuments.KBTags
{
	...
	public class DetailView : SplendidControl
	{
		...
		{
			SetPageTitle(L10n.Term(m_sMODULE + ".LBL_KBTAGS"));
			this.Visible = (SplendidCRM.Security.GetUserAccess("KBDocuments", "view") >= 0);
			if ( !this.Visible )
				return;
					...
					if ( !Sql.IsEmptyGuid(gID) )
					{
						...
						using ( IDbConnection con = dbf.CreateConnection() )
						{
							...
							sSQL = "select *           " + ControlChars.CrLf
							     + "  from vwKBTAGS_Edit" + ControlChars.CrLf
								...
								using ( DbDataAdapter da = dbf.CreateDataAdapter() )
								{
									...
									using ( DataTable dtCurrent = new DataTable() )
									{
										...
										if ( dtCurrent.Rows.Count > 0 )
										{
											...
											// 05/31/2015 Paul.  Combine ModuleHeader and DynamicButtons. 
											ctlDynamicButtons.Title = Sql.ToString(rdr["TAG_NAME"]);
