								...
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									try
									{
										while ( stk.Count > 0 )
										{
											...
											SqlProcs.spKBDOCUMENTS_MassUpdate(sIDs, T10n.ToServerTime(ctlMassUpdate.EXP_DATE), ctlMassUpdate.STATUS, ctlMassUpdate.PRIMARY_TEAM_ID, ctlMassUpdate.TEAM_SET_LIST, ctlMassUpdate.ADD_TEAM_SET, ctlMassUpdate.TAG_SET_NAME, ctlMassUpdate.ADD_TAG_SET, trn);
										}
										trn.Commit();
									}
									catch(Exception ex)
							...
							using ( IDbConnection con = dbf.CreateConnection() )
							{
								...
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									try
									{
										...
										{
											string sIDs = Utils.BuildMassIDs(stk);
											// 09/20/2013 Paul.  Procedure was commented out. 
											SqlProcs.spKBDOCUMENTS_MassDelete(sIDs, trn);
										}
										trn.Commit();
									}
									catch(Exception ex)
