<%@ Control Language="c#" AutoEventWireup="false" Codebehind="MassUpdate.ascx.cs" Inherits="SplendidCRM.KBDocuments.MassUpdate" TargetSchema="http://schemas.microsoft.com/intellisense/ie5" %>
...
<SplendidCRM:MassUpdateButtons ID="ctlDynamicButtons" SubPanel="divKBDocumentsMassUpdate" Title=".LBL_MASS_UPDATE_TITLE" Runat="Server" />

<%-- 03/26/2018 Paul.  Hide mass update fields in ArchiveView. --%>
<div id="divKBDocumentsMassUpdate" style='<%= "display:" + (CookieValue("divKBDocumentsMassUpdate") != "1" && !ArchiveView() ? "inline" : "none") %>'>
		...
		<asp:TableRow>
			<asp:TableCell>
				...
				<asp:Table Width="100%" CellPadding="0" CellSpacing="0" runat="server">
					<asp:TableRow>
						<asp:TableCell Width="15%" CssClass="dataLabel"><asp:Label Text='<%# L10n.Term("KBDocuments.LBL_EXP_DATE") %>' runat="server" /></asp:TableCell>
						<asp:TableCell Width="35%" CssClass="dataField"><SplendidCRM:DatePicker ID="ctlEXP_DATE" Runat="Server" /></asp:TableCell>
						<asp:TableCell Width="15%" CssClass="dataLabel"><asp:Label Text='<%# L10n.Term("KBDocuments.LBL_STATUS") %>' runat="server" /></asp:TableCell>
