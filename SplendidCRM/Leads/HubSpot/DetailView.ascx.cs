...
namespace SplendidCRM.Leads.HubSpot
{
	/// <summary>
			...
			try
			{
				...
				if ( !IsPostBack )
				{
					if ( !Sql.IsEmptyGuid(gID) )
					{
						...
						{
							string sSQL ;
							sSQL = "select (select SYNC_REMOTE_KEY from vwLEADS_SYNC where SYNC_SERVICE_NAME = N'HubSpot' and SYNC_LOCAL_ID = vwLEADS.ID) as SYNC_REMOTE_KEY" + ControlChars.CrLf
					...
					if ( nHID > 0 )
					{
						...
						if ( sbErrors.Length == 0 )
						{
							...
							Spring.Social.HubSpot.Api.Lead obj = hubSpot.LeadOperations.GetById(nHID);
							DataRow rdr = Spring.Social.HubSpot.Api.Lead.ConvertToRow(obj);
