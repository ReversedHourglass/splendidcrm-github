...
namespace SplendidCRM.Orders
{
	...
	public class AutoComplete : System.Web.Services.WebService
	{
		...
		public Order ORDERS_ORDER_NAME_Get(string sNAME)
		{
			...
			//try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					sSQL = "select ID        " + ControlChars.CrLf
					     ...
					     + "  from vwORDERS" + ControlChars.CrLf;
				...
				if ( Sql.IsEmptyGuid(item.ID) )
				{
					...
					throw(new Exception(L10n.Term("Orders.ERR_ORDER_NOT_FOUND")));
				}
			}
			//catch
			{
		...
		[WebMethod(EnableSession=true)]
		public string[] ORDERS_ORDER_NAME_List(string prefixText, int count)
		{
		...
		// so the field name can be NAME or ORDER_NAME. 
		[WebMethod(EnableSession=true)]
		public string[] ORDERS_NAME_List(string prefixText, int count)
		{
			return ORDERS_ORDER_NAME_List(prefixText, count);
		}

		[WebMethod(EnableSession=true)]
		public Order ORDERS_ORDER_NUM_Get(string sNAME)
		{
			Order item = new Order();
			//try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					con.Open();
					string sSQL;
					// 07/12/2010 Paul.  We should have been searching for an ORDER_NUM. 
					sSQL = "select ID       " + ControlChars.CrLf
					     + "     , ORDER_NUM" + ControlChars.CrLf
					     + "  from vwORDERS " + ControlChars.CrLf;
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						cmd.CommandText = sSQL;
						...
						Sql.AppendParameter(cmd, sNAME, Sql.SqlFilterMode.StartsWith, "ORDER_NUM");
						// 07/02/2007 Paul.  Sort is important so that the first match is selected. 
						cmd.CommandText += " order by ORDER_NUM" + ControlChars.CrLf;
						using ( IDataReader rdr = cmd.ExecuteReader(CommandBehavior.SingleRow) )
						{
							if ( rdr.Read() )
							{
								item.ID   = Sql.ToGuid   (rdr["ID"       ]);
								item.NAME = Sql.ToString (rdr["ORDER_NUM"]);
							}
						}
					}
				}
		...
		// 03/29/2007 Paul.  In order for AutoComplete to work, the parameter names must be "prefixText" and "count". 
		[WebMethod(EnableSession=true)]
		public string[] ORDERS_ORDER_NUM_List(string prefixText, int count)
		{
			string[] arrItems = new string[0];
			try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						Security.Filter(cmd, "Orders", "list");
						// 07/12/2010 Paul.  It does not make sense to allow a fuzzy search for a number. 
						Sql.AppendParameter(cmd, prefixText, Sql.SqlFilterMode.StartsWith, "ORDER_NUM");
						cmd.CommandText += " order by ORDER_NUM" + ControlChars.CrLf;
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							((IDbDataAdapter)da).SelectCommand = cmd;
							using ( DataTable dt = new DataTable() )
							{
								...
								arrItems = new string[dt.Rows.Count];
								for ( int i=0; i < dt.Rows.Count; i++ )
									arrItems[i] = Sql.ToString(dt.Rows[i]["ORDER_NUM"]);
							}
						}
					}
				}
