...
namespace SplendidCRM.Orders
{
	...
	public class Cases : SubPanelControl
	{
			...
			try
			{
				switch ( e.CommandName )
				{
					...
					case "Cases.Remove":
					{
						...
						{
							SqlProcs.spORDERS_CASES_Delete(gID, gCASE_ID);
						}
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bEditView && IsPostBack && arrUPDATED.Count > 0 )
					{
						...
						arrSelectFields.Remove("ORDER_ID"                );
						arrSelectFields.Remove("ORDER_NAME"              );
						arrSelectFields.Remove("ORDER_ASSIGNED_USER_ID"  );
						sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
						     + "     , ID                        as CASE_ID                 " + ControlChars.CrLf
						     + "     , NAME                      as CASE_NAME               " + ControlChars.CrLf
						     + "     , @ORDER_ID                 as ORDER_ID                " + ControlChars.CrLf
						     + "     , @ORDER_NAME               as ORDER_NAME              " + ControlChars.CrLf
						     + "     , @ORDER_ASSIGNED_USER_ID   as ORDER_ASSIGNED_USER_ID  " + ControlChars.CrLf
						     + "  from vwCASES" + ControlChars.CrLf;
						cmd.CommandText = sSQL;
						Sql.AddParameter(cmd, "@ORDER_ID"              , gID);
						Sql.AddParameter(cmd, "@ORDER_NAME"            , Sql.ToString(Page.Items["NAME"            ]));
						Sql.AddParameter(cmd, "@ORDER_ASSIGNED_USER_ID", Sql.ToGuid  (Page.Items["ASSIGNED_USER_ID"]));
					...
					else
					{
						...
						Sql.AppendParameter(cmd, gID, "ORDER_ID");
					}
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwORDERS_CASES", Sql.ClientScriptBlock(cmd));
					...
					try
					{
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								...
								// 10/05/2017 Paul.  Add Archive relationship view. 
								this.ApplyGridViewRules("Orders." + this.LayoutListView, dt);
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach ( Guid gDELETE_ID in arrDELETED )
			{
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spORDERS_CASES_Delete(gPARENT_ID, gDELETE_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spORDERS_CASES_Update(gPARENT_ID, gUPDATE_ID, trn);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gCASE_ID) )
			{
				try
				{
					...
					{
						SqlProcs.spORDERS_CASES_Update(gID, gCASE_ID);
					}
					// 05/16/2008 Paul.  Instead of redirecting, just rebind the grid and AJAX will repaint. 
					//Response.Redirect("view.aspx?ID=" + gID.ToString());
			...
			if ( !IsPostBack )
			{
				...
				// 10/05/2017 Paul.  Add Archive relationship view. 
				ctlDynamicButtons.AppendButtons("Orders." + this.LayoutListView, gASSIGNED_USER_ID, gID);
				// 02/21/2010 Paul.  The parent needs to be initialized when the page first loads. 
				ctlNewRecord.PARENT_ID = gID;
				// 08/05/2010 Paul.  Use the account of the parent. 
				ctlNewRecord.ACCOUNT_ID = Sql.ToGuid(Page.Items["BILLING_ACCOUNT_ID"]);
			}
		}
		...
		private void InitializeComponent()
		{
			...
			arrSelectFields.Add("ORDER_ID"              );
			arrSelectFields.Add("CASE_ID"               );
			arrSelectFields.Add("ASSIGNED_USER_ID"      );
			arrSelectFields.Add("ORDER_ASSIGNED_USER_ID");
			// 11/26/2005 Paul.  Add fields early so that sort events will get called. 
			// 06/07/2015 Paul.  Must include Page_Command in order for Preview to fire. 
			// 10/05/2017 Paul.  Add Archive relationship view. 
			m_sVIEW_NAME = "vwORDERS_CASES";
			if ( ArchiveViewExists() )
				m_sVIEW_NAME = m_sVIEW_NAME + "_ARCHIVE";
			this.LayoutListView = m_sMODULE + (ArchiveView() ? ".ArchiveView" : String.Empty);
			this.AppendGridColumns(grdMain, "Orders." + this.LayoutListView, arrSelectFields, Page_Command);
			// 04/28/2008 Paul.  Make use of dynamic buttons. 
			if ( IsPostBack )
				ctlDynamicButtons.AppendButtons("Orders." + this.LayoutListView, Guid.Empty, Guid.Empty);
		}
		#endregion
	}
}
