...
namespace SplendidCRM.Orders
{
	...
	public class EditView : SplendidControl
	{
		...
		protected void Page_Command(Object sender, CommandEventArgs e)
		{
						...
						using ( IDbConnection con = dbf.CreateConnection() )
						{
							...
							using ( IDbTransaction trn = Sql.BeginTransaction(con) )
							{
								try
								{
									...
									// 06/11/2009 Paul.  Use the same approach as Invoices to recall the QUOTE_ID. 
									...
									if ( Sql.IsEmptyString(sNAME) )
									{
										...
										sNAME = Sql.ToString(Application["CONFIG.Orders.NameTemplate"]);
										if ( Sql.IsEmptyString(sNAME) )
										{
									...
									SqlProcs.spORDERS_Update
										...
										, new DynamicControl(this, rowCurrent, "ORDER_NUM"                       ).Text
			...
			{
				// 12/09/2015 Paul.  Allow conversion from Opporunity. 
				Guid gOPPORTUNITY_ID = Sql.ToGuid(Request["OPPORTUNITY_ID"]);
				...
				else if ( !Sql.IsEmptyGuid(gOPPORTUNITY_ID) )
					Response.Redirect("~/Opportunities/view.aspx?ID=" + gOPPORTUNITY_ID.ToString());
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				if ( !IsPostBack )
				{
					...
					if ( !Sql.IsEmptyGuid(gID) || !Sql.IsEmptyGuid(gDuplicateID) || !Sql.IsEmptyGuid(gQUOTE_ID) || !Sql.IsEmptyGuid(gOPPORTUNITY_ID) )
					{
						DbProviderFactory dbf = DbProviderFactories.GetFactory();
						using ( IDbConnection con = dbf.CreateConnection() )
						{
							...
							{
								cmd.CommandText = sSQL;
								// 12/09/2015 Paul.  Allow conversion from Opportunity. 
								if ( !Sql.IsEmptyGuid(gOPPORTUNITY_ID) )
								{
									sLOAD_MODULE     = "Opportunities"  ;
									sLOAD_MODULE_KEY = "OPPORTUNITY_ID";
									m_sVIEW_NAME = "vwOPPORTUNITIES_ConvertToOrder";
									sSQL = "select *                             " + ControlChars.CrLf
									     + Sql.AppendRecordLevelSecurityField(m_sMODULE, "edit", m_sVIEW_NAME)
									     + "  from " + m_sVIEW_NAME + ControlChars.CrLf;
									cmd.CommandText = sSQL;
									...
									ViewState["DuplicateID"] = gOPPORTUNITY_ID;
								}
								else if ( !Sql.IsEmptyGuid(gQUOTE_ID) )
								{
									...
									m_sVIEW_NAME = "vwQUOTES_ConvertToOrder";
									sSQL = "select *                      " + ControlChars.CrLf
									     + Sql.AppendRecordLevelSecurityField(m_sMODULE, "edit", m_sVIEW_NAME)
									     + "  from " + m_sVIEW_NAME + ControlChars.CrLf;
									cmd.CommandText = sSQL;
								...
								using ( DbDataAdapter da = dbf.CreateDataAdapter() )
								{
									...
									using ( DataTable dtCurrent = new DataTable() )
									{
										...
										if ( dtCurrent.Rows.Count > 0 && (SplendidCRM.Security.GetRecordAccess(dtCurrent.Rows[0], m_sMODULE, "edit", "ASSIGNED_USER_ID") >= 0) )
										{
											...
											// 08/04/2010 Paul.  Now that we have fixed the stored procedure to allow the ORDER_NUM to be defined, 
											// we needed to make sure that the number gets reset when copying a record. 
											if ( !Sql.IsEmptyGuid(ViewState["DuplicateID"]) )
											{
												new DynamicControl(this, "ORDER_NUM").Text = String.Empty;
											}
											// 12/09/2008 Paul.  Throw an exception if the record has been edited since the last load. 
											...
											if ( !Sql.IsEmptyGuid(gOPPORTUNITY_ID) )
											{
												...
												new DynamicControl(this, "OPPORTUNITY_NAME").Text = Sql.ToString(rdr["OPPORTUNITY_NAME"]);
												ctlEditLineItemsView.LoadLineItems(gOPPORTUNITY_ID, Guid.Empty, con, rdr, sLOAD_MODULE, sLOAD_MODULE_KEY);
											}
											else if ( !Sql.IsEmptyGuid(gQUOTE_ID) )
											{
