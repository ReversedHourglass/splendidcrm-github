	...
	public class DetailView : SplendidControl
	{
								...
								using ( DbDataAdapter da = dbf.CreateDataAdapter() )
								{
									...
									using ( DataTable dtCurrent = new DataTable() )
									{
										...
										if ( dtCurrent.Rows.Count > 0 )
										{
											...
											this.AppendDetailViewRelationships("OrdersLineItems." + LayoutDetailView, plcSubPanel);
											this.AppendDetailViewFields("OrdersLineItems." + LayoutDetailView, tblMain, rdr, new CommandEventHandler(Page_Command));
											ctlDynamicButtons.AppendButtons("OrdersLineItems." + LayoutDetailView, Guid.Empty, rdr);
										}
										else
										{
											// 11/25/2006 Paul.  If item is not visible, then don't show its sub panel either. 
											// 03/20/2008 Paul.  Dynamic buttons need to be recreated in order for events to fire. 
											ctlDynamicButtons.AppendButtons("OrdersLineItems." + LayoutDetailView, Guid.Empty, null);
		...
		{    
			...
			if ( IsPostBack )
			{
				// 02/13/2013 Paul.  Move relationship append so that it can be controlled by business rules. 
				this.AppendDetailViewRelationships("OrdersLineItems." + LayoutDetailView, plcSubPanel);
				this.AppendDetailViewFields("OrdersLineItems." + LayoutDetailView, tblMain, null, new CommandEventHandler(Page_Command));
				ctlDynamicButtons.AppendButtons("OrdersLineItems." + LayoutDetailView, Guid.Empty, null);
			}
		}
		#endregion
