			...
			if ( !Sql.IsEmptyString(sUnifiedSearch.Trim()) )
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					sSQL = "select " + Sql.FormatSelectFields(arrSelectFields) + ControlChars.CrLf
					     + "  from vwACCOUNTS_ORDERS_LINE_ITEMS" + ControlChars.CrLf;
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						if ( bDebug )
							RegisterClientScriptBlock("vwACCOUNTS_ORDERS_LINE_ITEMS", Sql.ClientScriptBlock(cmd));
