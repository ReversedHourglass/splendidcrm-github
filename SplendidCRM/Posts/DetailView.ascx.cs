								...
								using ( DbDataAdapter da = dbf.CreateDataAdapter() )
								{
									...
									using ( DataTable dtCurrent = new DataTable() )
									{
										...
										if ( dtCurrent.Rows.Count > 0 && (SplendidCRM.Security.GetRecordAccess(dtCurrent.Rows[0], m_sMODULE, "view", "ASSIGNED_USER_ID") >= 0) )
										{
											...
											ctlDynamicButtons.ShowButton("Edit"  , (SplendidCRM.Security.AdminUserAccess(m_sMODULE, "edit") >= 0) || gCREATED_BY_ID == Security.USER_ID);
											ctlDynamicButtons.ShowButton("Delete", (SplendidCRM.Security.AdminUserAccess(m_sMODULE, "edit") >= 0) || gCREATED_BY_ID == Security.USER_ID);
