...
namespace SplendidCRM.Products
{
	...
	public class Notes : SplendidControl
	{
		...
		protected void BindGrid()
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					Sql.AppendParameter(cmd, gID, "PRODUCT_ID");
					...
					try
					{
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								...
								this.ApplyGridViewRules("Products." + m_sMODULE, dt);
		...
		private void InitializeComponent()
		{
			...
			this.AppendGridColumns(grdMain, "Products." + m_sMODULE, arrSelectFields, Page_Command);
			// 04/28/2008 Paul.  Make use of dynamic buttons. 
			if ( IsPostBack )
