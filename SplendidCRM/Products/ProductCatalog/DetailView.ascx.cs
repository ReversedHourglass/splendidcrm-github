	...
	public class DetailView : SplendidControl
	{
			...
			try
			{
				...
				if ( !IsPostBack )
				{
					if ( !Sql.IsEmptyGuid(gID) )
					{
						...
						{
							string sSQL ;
							// 05/29/2008 Paul.  Use the product template table. 
							...
							using ( IDbCommand cmd = con.CreateCommand() )
							{
								...
								// 06/05/2008 Paul.  Access rights should be based on Product Templates as that is the view we are using. 
								// This will prevent the filtering by TEAM_ID. 
								Security.Filter(cmd, "ProductTemplates", "view");
								...
								using ( DbDataAdapter da = dbf.CreateDataAdapter() )
								{
									...
									using ( DataTable dtCurrent = new DataTable() )
									{
										...
										if ( dtCurrent.Rows.Count > 0 )
										{
											...
											// 06/05/2008 Paul.  No easy way to point to the ProductCatalog. 
											//Utils.UpdateTracker(Page, m_sMODULE, gID, ctlDynamicButtons.Title);
											ViewState["ctlDynamicButtons.Title"] = ctlDynamicButtons.Title;
											
											this.AppendDetailViewFields(m_sMODULE + ".ProductCatalog", tblMain, rdr, new CommandEventHandler(Page_Command));
		...
		{    
			...
			if ( IsPostBack )
			{
				...
				this.AppendDetailViewFields(m_sMODULE + ".ProductCatalog", tblMain, null, new CommandEventHandler(Page_Command));
				// 03/20/2008 Paul.  Dynamic buttons need to be recreated in order for events to fire. 
				ctlDynamicButtons.AppendButtons(m_sMODULE + "." + LayoutDetailView, Guid.Empty, null);
			}
		}
