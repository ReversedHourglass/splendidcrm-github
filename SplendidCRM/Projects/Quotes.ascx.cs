...
namespace SplendidCRM.Projects
{
	...
	public class Quotes : SubPanelControl
	{
				...
				switch ( e.CommandName )
				{
					...
					case "Quotes.Remove":
					{
						...
						else
						{
							...
							SqlProcs.spPROJECTS_QUOTES_Delete(gID, gQUOTE_ID);
						}
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bEditView && IsPostBack && arrUPDATED.Count > 0 )
					{
						...
						sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
						     ...
						     + "  from vwQUOTE" + ControlChars.CrLf;
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach ( Guid gDELETE_ID in arrDELETED )
			{
				...
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spPROJECTS_QUOTES_Delete(gPARENT_ID, gDELETE_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				...
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spPROJECTS_QUOTES_Update(gPARENT_ID, gUPDATE_ID, trn);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gQUOTE_ID) )
			{
				try
				{
					...
					{
						// 09/08/2012 Paul.  Project Relations data for Accounts, Bugs, Cases, Contacts, Opportunities and Quotes moved to separate tables. 
						SqlProcs.spPROJECTS_QUOTES_Update(gID, gQUOTE_ID);
					}
					// 05/16/2008 Paul.  Instead of redirecting, just rebind the grid and AJAX will repaint. 
					//Response.Redirect("view.aspx?ID=" + gID.ToString());
		...
		private void InitializeComponent()
		{
			...
			// 10/05/2017 Paul.  Add Archive relationship view. 
			m_sVIEW_NAME = "vwPROJECTS_QUOTES";
