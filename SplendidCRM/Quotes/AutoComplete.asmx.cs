...
namespace SplendidCRM.Quotes
{
	...
	{
		public Quote QUOTES_QUOTE_NAME_Get(string sNAME)
		{
			...
			//try
			{
				...
				if ( Sql.IsEmptyGuid(item.ID) )
				{
					...
					throw(new Exception(L10n.Term("Quotes.ERR_QUOTE_NOT_FOUND")));
				}
			}
			//catch
			{
		...
		public string[] QUOTES_QUOTE_NAME_List(string prefixText, int count)
		{
		...
		// so the field name can be NAME or QUOTE_NAME. 
		[WebMethod(EnableSession=true)]
		public string[] QUOTES_NAME_List(string prefixText, int count)
		{
			return QUOTES_QUOTE_NAME_List(prefixText, count);
		}

		[WebMethod(EnableSession=true)]
		public Quote QUOTES_QUOTE_NUM_Get(string sNAME)
		{
			Quote item = new Quote();
			//try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						Sql.AppendParameter(cmd, sNAME, Sql.SqlFilterMode.StartsWith, "QUOTE_NUM");
						// 07/02/2007 Paul.  Sort is important so that the first match is selected. 
						cmd.CommandText += " order by QUOTE_NUM" + ControlChars.CrLf;
						using ( IDataReader rdr = cmd.ExecuteReader(CommandBehavior.SingleRow) )
						{
							if ( rdr.Read() )
							{
								item.ID   = Sql.ToGuid   (rdr["ID"  ]);
								item.NAME = Sql.ToString (rdr["QUOTE_NUM"]);
							}
						}
					}
				}
		...
		// 03/29/2007 Paul.  In order for AutoComplete to work, the parameter names must be "prefixText" and "count". 
		[WebMethod(EnableSession=true)]
		public string[] QUOTES_QUOTE_NUM_List(string prefixText, int count)
		{
			string[] arrItems = new string[0];
			try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						Security.Filter(cmd, "Quotes", "list");
						// 07/12/2010 Paul.  It does not make sense to allow a fuzzy search for a number. 
						Sql.AppendParameter(cmd, prefixText, Sql.SqlFilterMode.StartsWith, "QUOTE_NUM");
						cmd.CommandText += " order by QUOTE_NUM" + ControlChars.CrLf;
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							((IDbDataAdapter)da).SelectCommand = cmd;
							using ( DataTable dt = new DataTable() )
							{
								...
								arrItems = new string[dt.Rows.Count];
								for ( int i=0; i < dt.Rows.Count; i++ )
									arrItems[i] = Sql.ToString(dt.Rows[i]["QUOTE_NUM"]);
							}
						}
					}
				}
