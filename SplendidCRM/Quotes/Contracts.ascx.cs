...
namespace SplendidCRM.Quotes
{
	...
	/// </summary>
	public class Contracts : SubPanelControl
	{
		...
		protected HtmlInputHidden txtCONTRACT_ID ;
		...
		protected SplendidCRM.Contracts.NewRecord ctlNewRecord   ;
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				switch ( e.CommandName )
				{
					...
					// 01/30/2010 Paul.  Add code to allow Contract to be removed. 
					case "Contracts.Remove":
					{
						Guid gCONTRACT_ID = Sql.ToGuid(e.CommandArgument);
						if ( bEditView )
						{
							this.DeleteEditViewRelationship(gCONTRACT_ID);
						}
						else
						{
							SqlProcs.spCONTRACTS_QUOTES_Delete(gCONTRACT_ID, gID);
						}
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bEditView && IsPostBack && arrUPDATED.Count > 0 )
					{
						...
						arrSelectFields.Remove("QUOTE_ASSIGNED_USER_ID");
						// 12/02/2010 Paul.  Should be vwCONTRACTS view. 
						sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
						     + "     , ID                        as CONTRACT_ID           " + ControlChars.CrLf
						     + "     , NAME                      as CONTRACT_NAME         " + ControlChars.CrLf
						     + "     , @QUOTE_ID                 as QUOTE_ID              " + ControlChars.CrLf
						     + "     , @QUOTE_NAME               as QUOTE_NAME            " + ControlChars.CrLf
						     + "     , @QUOTE_ASSIGNED_USER_ID   as QUOTE_ASSIGNED_USER_ID" + ControlChars.CrLf
						     + "  from vwCONTRACTS" + ControlChars.CrLf;
						cmd.CommandText = sSQL;
						Sql.AddParameter(cmd, "@QUOTE_ID"              , gID);
						Sql.AddParameter(cmd, "@QUOTE_NAME"            , Sql.ToString(Page.Items["NAME"            ]));
						Sql.AddParameter(cmd, "@QUOTE_ASSIGNED_USER_ID", Sql.ToGuid  (Page.Items["ASSIGNED_USER_ID"]));
					...
					try
					{
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								...
								// 10/05/2017 Paul.  Add Archive relationship view. 
								this.ApplyGridViewRules("Quotes." + this.LayoutListView, dt);
								...
								{
									this.CreateEditViewRelationships(dt, "CONTRACT_ID");
								}
							}
						}
					}
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach ( Guid gDELETE_ID in arrDELETED )
			{
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spCONTRACTS_QUOTES_Delete(gDELETE_ID, gPARENT_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spCONTRACTS_QUOTES_Update(gUPDATE_ID, gPARENT_ID, trn);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			Guid gCONTRACT_ID = Sql.ToGuid(txtCONTRACT_ID.Value);
			if ( !Sql.IsEmptyGuid(gCONTRACT_ID) )
			{
				try
				{
					if ( bEditView )
					{
						this.UpdateEditViewRelationship(gCONTRACT_ID);
					}
					else
					{
						SqlProcs.spCONTRACTS_QUOTES_Update(gCONTRACT_ID, gID);
					}
					// 05/16/2008 Paul.  Instead of redirecting, just rebind the grid and AJAX will repaint. 
					//Response.Redirect("view.aspx?ID=" + gID.ToString());
					// 05/16/2008 Paul.  If we are not going to redirect,then we must clear the value. 
					txtCONTRACT_ID.Value = String.Empty;
				}
				catch(Exception ex)
				{
					SplendidError.SystemError(new StackTrace(true).GetFrame(0), ex);
			...
			if ( !IsPostBack )
			{
				...
				// 10/05/2017 Paul.  Add Archive relationship view. 
				ctlDynamicButtons.AppendButtons("Quotes." + this.LayoutListView, gASSIGNED_USER_ID, gID);
			}
		}
		...
		private void InitializeComponent()
		{
			...
			arrSelectFields.Add("CONTRACT_ID"     );
			arrSelectFields.Add("CONTRACT_NAME"   );
			arrSelectFields.Add("ASSIGNED_USER_ID");
			// 11/26/2005 Paul.  Add fields early so that sort events will get called. 
			// 06/07/2015 Paul.  Must include Page_Command in order for Preview to fire. 
			// 10/05/2017 Paul.  Add Archive relationship view. 
			m_sVIEW_NAME = "vwQUOTES_CONTRACTS";
			if ( ArchiveViewExists() )
				m_sVIEW_NAME = m_sVIEW_NAME + "_ARCHIVE";
			this.LayoutListView = m_sMODULE + (ArchiveView() ? ".ArchiveView" : String.Empty);
			this.AppendGridColumns(grdMain, "Quotes." + this.LayoutListView, arrSelectFields, Page_Command);
			// 04/28/2008 Paul.  Make use of dynamic buttons. 
			if ( IsPostBack )
				ctlDynamicButtons.AppendButtons("Quotes." + this.LayoutListView, Guid.Empty, Guid.Empty);
		}
		#endregion
	}
}
