...
namespace SplendidCRM.Quotes
{
	...
	public class DetailView : SplendidControl
	{
		...
		protected Button      btnSubmitSignature;
		protected HiddenField hidSIGNATURE      ;
		...
		protected void Page_Command(Object sender, CommandEventArgs e)
		{
			try
			{
				...
				else if ( e.CommandName == "Convert.ToOrder" )
				{
					Response.Redirect("~/Orders/edit.aspx?QUOTE_ID=" + gID.ToString());
				}
				else if ( e.CommandName == "Convert.ToInvoice" )
				{
					Response.Redirect("~/Invoices/edit.aspx?QUOTE_ID=" + gID.ToString());
				}
				else if ( e.CommandName == "Convert.ToOpportunity" )
				{
					Response.Redirect("~/Opportunities/edit.aspx?QUOTE_ID=" + gID.ToString());
				}
				...
				// 03/24/2016 Paul.  Receive signature from jQuery Signature popup. 
				else if ( e.CommandName == "SubmitSignature" )
				{
					Guid gIMAGE_ID           = Guid.Empty;
					Guid gNOTE_ID            = Guid.Empty;
					Guid gNOTE_ATTACHMENT_ID = Guid.Empty;
					Guid gREPORT_ID          = Guid.Empty;
					OrderUtils.CreateSignedPDF(Context, L10n, T10n, m_sMODULE, gID, hidSIGNATURE.Value, out gIMAGE_ID, out gNOTE_ID, out gNOTE_ATTACHMENT_ID, out gREPORT_ID);
					...
					SqlProcs.spQUOTES_UpdateStage(gID, "Signed");
					//Response.Redirect("~/Images/Image.aspx?ID=" + gIMAGE_ID.ToString());
					//Response.Redirect("~/Notes/Attachment.aspx?ID=" + gNOTE_ATTACHMENT_ID.ToString());
					
					// 04/06/2016 Paul.  We want to be able to run a workflow when a signature has been submitted. 
					Page.Items["SIGNATURE_IMAGE_ID"          ] = gIMAGE_ID          ;
					Page.Items["SIGNATURE_NOTE_ID"           ] = gNOTE_ID           ;
					Page.Items["SIGNATURE_NOTE_ATTACHMENT_ID"] = gNOTE_ATTACHMENT_ID;
					Page.Items["SIGNATURE_REPORT_ID"         ] = gREPORT_ID         ;
					DataRow rowCurrent = Crm.Modules.ItemEdit(m_sMODULE, gID);
					this.ApplyEditViewPostSaveEventRules(m_sMODULE + ".SubmitSignature", rowCurrent);
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				// 04/01/2008 Paul.  Place AppendDetailViewFields inside OnInit to avoid having to re-populate the data. 
								...
								using ( DbDataAdapter da = dbf.CreateDataAdapter() )
								{
									...
									using ( DataTable dtCurrent = new DataTable() )
									{
										...
										if ( dtCurrent.Rows.Count > 0 && (SplendidCRM.Security.GetRecordAccess(dtCurrent.Rows[0], m_sMODULE, "view", "ASSIGNED_USER_ID") >= 0) )
										{
											...
											// 04/01/2008 Paul.  Dynamic buttons need to be recreated in order for events to fire. 
											...
											// 08/05/2010 Paul.  The Cases subpanel will use the account. 
											Page.Items["BILLING_ACCOUNT_ID"] = Sql.ToGuid(rdr["BILLING_ACCOUNT_ID"]);
		...
		{    
			...
			if ( IsPostBack )
			{
				...
				this.AppendDetailViewFields(m_sMODULE + ".SummaryView", tblSummary, null, new CommandEventHandler(Page_Command));
				// 04/01/2008 Paul.  Dynamic buttons need to be recreated in order for events to fire. 
				ctlDynamicButtons.AppendButtons(m_sMODULE + "." + LayoutDetailView, Guid.Empty, null);
				// 08/06/2015 paul.  Separate method to include process buttons so that we don't do a process query for all the non-detailview buttons. 
				ctlDynamicButtons.AppendProcessButtons(null);
