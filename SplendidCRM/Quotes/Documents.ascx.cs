...
namespace SplendidCRM.Quotes
{
	...
	public class Documents : SubPanelControl
	{
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				switch ( e.CommandName )
				{
					...
					{
						Guid gDOCUMENT_ID = Sql.ToGuid(e.CommandArgument);
						SqlProcs.spDOCUMENTS_QUOTES_Delete(gID, gDOCUMENT_ID);
						BindGrid();
						break;
					}
					case "Documents.GetLatest":
					{
						...
						{
							SqlProcs.spDOCUMENTS_QUOTES_GetLatest(gID, gDOCUMENT_ID);
						}
						BindGrid();
						break;
					}
		...
		protected void BindGrid()
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwQUOTES_DOCUMENTS", Sql.ClientScriptBlock(cmd));
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach ( Guid gDELETE_ID in arrDELETED )
			{
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spDOCUMENTS_QUOTES_Delete(gPARENT_ID, gDELETE_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spDOCUMENTS_QUOTES_Update(gPARENT_ID, gUPDATE_ID, trn);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gDOCUMENT_ID) )
			{
				try
				{
					...
					{
						SqlProcs.spDOCUMENTS_QUOTES_Update(gID, gDOCUMENT_ID);
					}
		...
		private void InitializeComponent()
		{
			...
			// 10/05/2017 Paul.  Add Archive relationship view. 
			m_sVIEW_NAME = "vwQUOTES_DOCUMENTS";
