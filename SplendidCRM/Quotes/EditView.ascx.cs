	...
	public class EditView : SplendidControl
	{
						...
						using ( IDbConnection con = dbf.CreateConnection() )
						{
							...
							using ( IDbTransaction trn = Sql.BeginTransaction(con) )
							{
								try
								{
									...
									// 08/06/2009 Paul.  QUOTE_NUM now uses our number sequence table. 
									...
									if ( Sql.IsEmptyString(sNAME) )
									{
										...
										sNAME = Sql.ToString(Application["CONFIG.Quotes.NameTemplate"]);
										if ( Sql.IsEmptyString(sNAME) )
										{
									...
									SqlProcs.spQUOTES_Update
										...
										, new DynamicControl(this, rowCurrent, "QUOTE_NUM"                       ).Text
		...
		private void UpdateAccount(Guid gACCOUNT_ID, bool bUpdateBilling, bool bUpdateShipping)
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				{
					cmd.CommandText = sSQL;
					// 09/19/2008 Paul.  Use new Security.Filter() function to apply Team and ACL security rules.
								...
								using ( DbDataAdapter da = dbf.CreateDataAdapter() )
								{
									...
									using ( DataTable dtCurrent = new DataTable() )
									{
										...
										if ( dtCurrent.Rows.Count > 0 && (SplendidCRM.Security.GetRecordAccess(dtCurrent.Rows[0], m_sMODULE, "edit", "ASSIGNED_USER_ID") >= 0) )
										{
											...
											// 08/04/2010 Paul.  Now that we have fixed the stored procedure to allow the QUOTE_NUM to be defined, 
											// we needed to make sure that the number gets reset when copying a record. 
											if ( !Sql.IsEmptyGuid(ViewState["DuplicateID"]) )
											{
												new DynamicControl(this, "QUOTE_NUM").Text = String.Empty;
											}
											// 12/09/2008 Paul.  Throw an exception if the record has been edited since the last load. 
											...
											// 02/11/2010 Paul.  LoadLineItems will close the reader, so make sure to get the data in advance. 
					...
					else
					{
						...
						if ( !Sql.IsEmptyGuid(gPARENT_ID) )
						{
							...
							else if ( !Sql.IsEmptyGuid(gPARENT_ID) && sMODULE == "Opportunities" )
							{
								...
								// 09/19/2008 Paul.  Pull the account from the opportunity. 
								DbProviderFactory dbf = DbProviderFactories.GetFactory();
								using ( IDbConnection con = dbf.CreateConnection() )
								{
									con.Open();
									...
									using ( IDbCommand cmd = con.CreateCommand() )
									{
										...
										using ( IDataReader rdr = cmd.ExecuteReader(CommandBehavior.SingleRow) )
										{
											...
											{
												Guid gACCOUNT_ID = Sql.ToGuid  (rdr["ACCOUNT_ID"]);
												UpdateAccount(gACCOUNT_ID, true, true);
											}
										}
									}
								}
