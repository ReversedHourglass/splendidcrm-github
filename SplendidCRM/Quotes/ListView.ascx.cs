								...
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									try
									{
										while ( stk.Count > 0 )
										{
											...
											SqlProcs.spQUOTES_MassUpdate(sIDs, ctlMassUpdate.ASSIGNED_USER_ID, ctlMassUpdate.PAYMENT_TERMS, ctlMassUpdate.QUOTE_STAGE, T10n.ToServerTime(ctlMassUpdate.ORIGINAL_PO_DATE), T10n.ToServerTime(ctlMassUpdate.DATE_QUOTE_EXPECTED_CLOSED), ctlMassUpdate.BILLING_ACCOUNT_ID, ctlMassUpdate.BILLING_CONTACT_ID, ctlMassUpdate.SHIPPING_ACCOUNT_ID, ctlMassUpdate.SHIPPING_CONTACT_ID, ctlMassUpdate.PRIMARY_TEAM_ID, ctlMassUpdate.TEAM_SET_LIST, ctlMassUpdate.ADD_TEAM_SET, ctlMassUpdate.TAG_SET_NAME, ctlMassUpdate.ADD_TAG_SET, String.Empty, false, trn);
										}
										trn.Commit();
									}
									catch(Exception ex)
			...
			try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						grdMain.OrderByClause("QUOTE_NUM", "desc");
						...
						// 06/20/2012 Paul.  Security.Filter was accidentally deleted. 
						// 09/26/2017 Paul.  Add Archive access right. 
						Security.Filter(cmd, m_sMODULE, (ArchiveViewEnabled() ? "archive" : "list"));
						ctlSearchView.SqlSearchClause(cmd);
