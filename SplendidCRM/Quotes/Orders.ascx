...
<SplendidCRM:SubPanelButtons ID="ctlDynamicButtons" Module="Orders" SubPanel="divQuotesOrders" Title="Orders.LBL_MODULE_NAME" Runat="Server" />

<div id="divQuotesOrders" style='<%= "display:" + (CookieValue("divQuotesOrders") != "1" ? "inline" : "none") %>'>
			...
			<asp:TemplateColumn HeaderText="" ItemStyle-Width="1%" ItemStyle-HorizontalAlign="Left" ItemStyle-Wrap="false">
				<ItemTemplate>
					...
					<span onclick="return confirm('<%= L10n.TermJavaScript(".NTC_DELETE_CONFIRMATION") %>')">
						<asp:ImageButton Visible='<%# SplendidCRM.Security.GetRecordAccess(Container, m_sMODULE, "delete", "ASSIGNED_USER_ID") >= 0 && !ArchiveViewExists() %>' CommandName="Orders.Delete" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "ORDER_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" AlternateText='<%# L10n.Term(".LNK_DELETE") %>' SkinID="delete_inline" Runat="server" />
						<asp:LinkButton  Visible='<%# SplendidCRM.Security.GetRecordAccess(Container, m_sMODULE, "delete", "ASSIGNED_USER_ID") >= 0 && !ArchiveViewExists() %>' CommandName="Orders.Delete" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "ORDER_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" Text='<%# L10n.Term(".LNK_DELETE") %>' Runat="server" />
					</span>
				</ItemTemplate>
			</asp:TemplateColumn>
		</Columns>
