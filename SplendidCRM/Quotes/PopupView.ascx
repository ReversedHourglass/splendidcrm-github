<%@ Control CodeBehind="PopupView.ascx.cs" Language="c#" AutoEventWireup="false" Inherits="SplendidCRM.Quotes.PopupView" %>
<script runat="server">
/**********************************************************************************************************************
...
<div id="divPopupView">
	...
	function SelectChecked()
	{
		if ( window.opener != null && window.opener.ChangeQuote != null )
		{
			...
			window.opener.ChangeQuote(sSelectedItems, '');
			window.close();
		}
		else
		{
	...
	<asp:UpdatePanel UpdateMode="Conditional" Visible='<%# false && !Sql.ToBoolean(Application["CONFIG.disable_popupview_inline"]) %>' runat="server">
		<ContentTemplate>
			<asp:Button ID="btnCreateInline" CommandName="NewRecord.Show" OnCommand="Page_Command" Text='<%# L10n.Term(m_sMODULE + ".LNK_NEW_QUOTE") %>' CssClass="button" style="margin-bottom: 4px;" Visible="<%# !this.IsMobile %>" runat="server" />
