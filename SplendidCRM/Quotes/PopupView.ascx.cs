			...
			try
			{
				...
				else if ( e.CommandName == "NewRecord" )
				{
					...
					if ( mgrAjax != null )
					{
						...
						ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "NewRecord", "SelectQuote('" + gID.ToString() + "', '" + Sql.EscapeJavaScript(sNAME) + "');", true);
					}
					else
					{
		...
		protected void grdMain_OnSelectMethod(int nCurrentPageIndex, int nPageSize)
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					// 11/12/2011 Paul.  The Quotes popup may need to filter on the account. 
					FilterAccount(cmd);
