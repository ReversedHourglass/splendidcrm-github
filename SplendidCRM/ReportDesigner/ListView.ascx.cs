...
namespace SplendidCRM.ReportDesigner
{
	...
	public class ListView : SplendidControl
	{
		...
		protected Reports.MassUpdate     ctlMassUpdate    ;
		protected bool                   bSchedulerEnabled;
		// 05/07/2017 Paul.  Combine ModuleHeader and DynamicButtons. 
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				switch ( e.CommandName )
				{
					...
					// 05/07/2017 Paul.  Change standard MassUpdate command to a command to toggle visibility. 
			...
			try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						// 04/26/2018 Paul.  Although not ideal, there could be multiple workflows for a report, so use a join instead of a subquery. 
		...
		private void InitializeComponent()
		{
			...
			if ( SplendidDynamic.StackedLayout(Page.Theme) )
			{
				...
				// 05/07/2017 Paul.  Move MassUpdate buttons to the SplendidGrid. 
				grdMain.IsMobile       = this.IsMobile;
				grdMain.MassUpdateView = "Reports.MassUpdate";
