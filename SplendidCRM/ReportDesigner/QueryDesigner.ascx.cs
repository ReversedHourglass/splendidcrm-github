...
namespace SplendidCRM.ReportDesigner
{
	...
	/// </summary>
	public class QueryDesigner : SplendidControl
	{
		#region Properties
		protected Label           lblError                  ;
		protected HiddenField     hidDESIGNER_JSON          ;
		...
		private static void ACLFilter(HttpApplicationState Application, StringBuilder sbJoin, StringBuilder sbWhere, string sMODULE_NAME, string sACCESS_TYPE, string sASSIGNED_USER_ID_Field, bool bIsCaseSignificantDB, bool bUseSQLParameters)
		{
			...
			bool bEnableTeamManagement  = Crm.Config.enable_team_management (Application);
			bool bRequireTeamManagement = Crm.Config.require_team_management(Application);
			bool bRequireUserAssignment = Crm.Config.require_user_assignment(Application);
			// 11/27/2009 Paul.  Allow dynamic teams to be turned off. 
			bool bEnableDynamicTeams    = Crm.Config.enable_dynamic_teams(Application);
			// 02/13/2018 Paul.  Allow team hierarchy. 
			bool bEnableTeamHierarchy   = Crm.Config.enable_team_hierarchy(Application);
		...
		public static bool IsNumericField(DataView vwColumns, string sColumnName)
		{
			bool b = false;
		...
		public static bool IsDateField(DataView vwColumns, string sColumnName)
		{
			bool b = false;
			vwColumns.RowFilter = "ColumnName = '" + sColumnName + "'";
			if ( vwColumns.Count > 0 )
		...
		public static bool IsBooleanField(DataView vwColumns, string sColumnName)
		{
			bool b = false;
			vwColumns.RowFilter = "ColumnName = '" + sColumnName + "'";
			if ( vwColumns.Count > 0 )
		...
		private static List<string> AvailableTables(HttpApplicationState Application)
		{
		...
		// 03/31/2020 Paul.  Separate out GetReportDesign so that it can be called from the React Client. 
		public static string BuildReportSQL(HttpApplicationState Application, L10N L10n, RdlDocument rdl, ReportDesign rd, bool bPrimaryKeyOnly, bool bUseSQLParameters, bool bUserSpecific, ref int nSelectedColumns)
		{
			...
			if ( rd != null && rd.Tables != null )
			{
				...
				List<string> lstAvailableTables = AvailableTables(Application);
							...
							if ( !Sql.IsEmptyString(oReportRelationship.LeftTableName) && !Sql.IsEmptyString(oReportRelationship.RightTableName) )
							{
								...
								if ( bUserSpecific || bUseSQLParameters )
								{
									if ( oReportRelationship.RightTableName != "vwUSERS" )
										ACLFilter(Application, sb, sbACLWhere, oReportRelationship.RightTableName, "list", "ASSIGNED_USER_ID", bIsOracle || bIsDB2, bUseSQLParameters);
								}
							}
						}
						...
						else if ( oUsedTables[oReportRelationship.RightTableName] > 0 )
						{
							...
							if ( bUserSpecific || bUseSQLParameters )
							{
								if ( oReportRelationship.LeftTableName != "vwUSERS" )
									ACLFilter(Application, sb, sbACLWhere, oReportRelationship.LeftTableName, "list", "ASSIGNED_USER_ID", bIsOracle || bIsDB2, bUseSQLParameters);
							}
						}
					}
				}
				...
				if ( bUserSpecific || bUseSQLParameters )
				{
					...
					if ( sMODULE_TABLE != "USERS" )
						ACLFilter(Application, sb, sbACLWhere, sMODULE_TABLE, "list", "ASSIGNED_USER_ID", bIsOracle || bIsDB2, bUseSQLParameters);
				}
		...
		private static DataTable ReportFilters(RdlDocument rdl)
		{
		...
		private static DataTable ReportRelationships(RdlDocument rdl)
		{
		...
		private static DataTable ReportDisplayColumns(RdlDocument rdl)
		{
		...
		private static XmlDocument ReplatedModulesXML(RdlDocument rdl, string sMODULE, L10N L10n, bool bDebug)
		{
		...
		public static void DisplayColumnsUpdate(RdlDocument rdl, ReportDesign rd)
		{
		...
		public static void UpdateDataTable(RdlDocument rdl)
		{
			XmlDocument xmlDisplayColumns = rdl.GetCustomProperty("DisplayColumns");
		...
		// 03/31/2020 Paul.  Separate out GetReportDesign so that it can be called from the React Client. 
		public static string GetReportDesign(RdlDocument rdl, string[] arrModules, string sRDL, L10N L10n, bool bDebug)
		{
			...
			if ( !Sql.IsEmptyString(sRDL) )
			{
				...
				if ( Sql.IsEmptyString(sReportDesign) )
				{
					...
					string sMODULE_TABLE = Crm.Modules.TableName(sMODULE_NAME);
					...
					if ( !Sql.IsEmptyString(sRELATED_NAME) )
					{
						...
						rdTable.TableName  = Crm.Modules.TableName(sRELATED);
						lstTables.Add(rdTable);
					}
					...
					foreach ( DataRow row in dtDisplayColumns.Rows )
					{
						...
						{
							rdField.TableName  = Crm.Modules.TableName(sMODULE_NAME);
		...
		public void LoadRdl(string sRDL)
		{
			try
			{
				...
				try
				{
					hidDESIGNER_JSON.Value = GetReportDesign(rdl, arrModules, sRDL, L10n, bDebug);
				}
				catch(Exception ex)
				{
				...
				// 03/31/2020 Paul.  Separate out GetReportDesign so that it can be called from the React Client. 
				JavaScriptSerializer json = new JavaScriptSerializer();
				ReportDesign rd = json.Deserialize<ReportDesign>(hidDESIGNER_JSON.Value);
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				Sql.AddScriptReference(mgrAjax, "~/Include/javascript/jquery-1.9.1.min.js"          );
				...
				string sBundleName = "~/ReportDesigner/ScriptsCombined" + "_" + Sql.ToString(Application["SplendidVersion"]);
				...
				// 07/01/2017 Paul.  Use Microsoft ASP.NET Web Optimization 1.1.3 to combine stylesheets and javascript. 
				// 01/24/2018 Paul.  Include version in url to ensure updates of combined files. 
				sBundleName = "~/ReportDesigner/SplendidScriptsCombined" + "_" + Sql.ToString(Application["SplendidVersion"]);
				...
				// 07/01/2017 Paul.  Use Microsoft ASP.NET Web Optimization 1.1.3 to combine stylesheets and javascript. 
				// 01/24/2018 Paul.  Include version in url to ensure updates of combined files. 
				sBundleName = "~/ReportDesigner/SplendidUICombined" + "_" + Sql.ToString(Application["SplendidVersion"]);
...
#endif
				Sql.AddScriptReference(mgrAjax, "~/ReportDesigner/QueryDesigner.js"                 );
				...
				ScriptManager.RegisterOnSubmitStatement(Page, Page.GetType(), "QueryDesigner_OnSubmit", "QueryDesigner_OnSubmit();");
				JavaScriptSerializer json = new JavaScriptSerializer();
