...
<div id="divListView">
		...
		<Columns>
			...
			<asp:TemplateColumn  HeaderText="Reports.LBL_LIST_REPORT_NAME"                          SortExpression="NAME" ItemStyle-Width="35%" ItemStyle-CssClass="listViewTdLinkS1">
				<ItemTemplate>
					...
					<asp:HyperLink Visible='<%# Sql.ToString(Eval("REPORT_TYPE")) == L10n.Term(".dom_report_types.", "tabular") && SplendidCRM.Security.GetRecordAccess(Container, m_sMODULE, "edit", "ASSIGNED_USER_ID") >= 0 %>' NavigateUrl='<%# "~/Reports/edit.aspx?id=" + Eval("ID") %>' Text='<%# Eval("NAME") %>' Runat="server" />
					<asp:HyperLink Visible='<%# Sql.ToString(Eval("REPORT_TYPE")) != L10n.Term(".dom_report_types.", "tabular") && SplendidCRM.Security.GetRecordAccess(Container, m_sMODULE, "view", "ASSIGNED_USER_ID") >= 0 %>' NavigateUrl='<%# "~/Reports/view.aspx?id=" + Eval("ID") %>' Text='<%# Eval("NAME") %>' Runat="server" />
				</ItemTemplate>
			</asp:TemplateColumn>
			...
			<asp:TemplateColumn  HeaderText="Reports.LBL_LIST_SCHEDULE_REPORT">
				<ItemTemplate>
					<%-- 10/31/2017 Paul.  Provide a way to inject Record level ACL. --%>
					<asp:HyperLink Visible='<%# SplendidCRM.Security.GetRecordAccess(Container, m_sMODULE, "edit") >= 0 %>' NavigateUrl=<%# "Schedule.aspx?REPORT_ID=" + Eval("ID") %> Text='<%# Sql.IsEmptyString(Eval("JOB_INTERVAL")) ? L10n.Term("Reports.LNK_SCHEDULE") : Sql.ToString(Eval("JOB_INTERVAL")) %>' Runat="server" />
				</ItemTemplate>
			</asp:TemplateColumn>
			...
			<asp:TemplateColumn  HeaderText="" ItemStyle-HorizontalAlign="Right" ItemStyle-Wrap="false">
				<ItemTemplate>
					<%-- 10/31/2017 Paul.  Provide a way to inject Record level ACL. --%>
					<asp:HyperLink Visible='<%# Sql.ToString(Eval("REPORT_TYPE")) != L10n.Term(".dom_report_types.", "Freeform") && SplendidCRM.Security.GetRecordAccess(Container, m_sMODULE, "edit", "ASSIGNED_USER_ID") >= 0 %>' NavigateUrl='<%# "edit.aspx?DuplicateID=" + Eval("ID") %>' Text='<%# L10n.Term(".LBL_DUPLICATE_BUTTON_LABEL") %>' CssClass="listViewTdToolsS1" Runat="server" />
					&nbsp;
	...
	<br />
	<SplendidCRM:ListHeader ID="ctlListHeaderPublished" Visible="false" Title=".published_reports_dom." Runat="Server" />
	<SplendidCRM:SplendidGrid id="grdPublished" SkinID="grdListView" AllowPaging="<%# !PrintView %>" EnableViewState="true" Visible="false" runat="server">
		<Columns>
			<asp:HyperLinkColumn HeaderText="Reports.LBL_LIST_REPORT_NAME"    DataTextField="NAME"  SortExpression="NAME" ItemStyle-Width="35%" ItemStyle-CssClass="listViewTdLinkS1" DataNavigateUrlField="ID" DataNavigateUrlFormatString="edit.aspx?id={0}" Visible="false" />
			<asp:TemplateColumn  HeaderText="Reports.LBL_LIST_REPORT_NAME"                          SortExpression="NAME" ItemStyle-Width="35%" ItemStyle-CssClass="listViewTdLinkS1">
				<ItemTemplate>
					<%-- 10/31/2017 Paul.  Provide a way to inject Record level ACL. --%>
					<asp:HyperLink Visible='<%#  Sql.ToString(Eval("REPORT_TYPE")) != L10n.Term(".dom_report_types.", "Freeform") && SplendidCRM.Security.GetRecordAccess(Container, m_sMODULE, "edit", "ASSIGNED_USER_ID") >= 0 %>' NavigateUrl='<%# "~/Reports/edit.aspx?id=" + Eval("ID") %>' Text='<%# Eval("NAME") %>' Runat="server" />
					<asp:HyperLink Visible='<%# (Sql.ToString(Eval("REPORT_TYPE")) == L10n.Term(".dom_report_types.", "Freeform") || SplendidCRM.Security.GetRecordAccess(Container, m_sMODULE, "edit", "ASSIGNED_USER_ID") < 0) && SplendidCRM.Security.GetRecordAccess(Container, m_sMODULE, "view", "ASSIGNED_USER_ID") >= 0 %>' NavigateUrl='<%# "~/Reports/view.aspx?id=" + Eval("ID") %>' Text='<%# Eval("NAME") %>' Runat="server" />
				</ItemTemplate>
			</asp:TemplateColumn>
			<asp:BoundColumn     HeaderText="Reports.LBL_LIST_MODULE_NAME"    DataField="MODULE_NAME"                     ItemStyle-Width="15%" />
			<asp:BoundColumn     HeaderText="Reports.LBL_LIST_REPORT_TYPE"    DataField="REPORT_TYPE"                     ItemStyle-Width="25%" />
			<asp:TemplateColumn  HeaderText="" ItemStyle-HorizontalAlign="Right" ItemStyle-Wrap="false">
				<ItemTemplate>
					...
					&nbsp;
					<%-- 10/31/2017 Paul.  Provide a way to inject Record level ACL. --%>
					<asp:HyperLink Visible='<%# SplendidCRM.Security.GetRecordAccess(Container, m_sMODULE, "export") >= 0 %>' NavigateUrl='<%# "~/Reports/ExportRDL.aspx?id=" + Eval("ID") %>' Target="_blank" AlternateText='<%# L10n.Term(".LBL_EXPORT") %>' Runat="server">
						<asp:Image SkinID="export" Runat="server" />&nbsp;<%# L10n.Term(".LBL_EXPORT") %>
					</asp:HyperLink>
					&nbsp;
					<asp:ImageButton Visible='<%# SplendidCRM.Security.GetRecordAccess(Container, m_sMODULE, "view", "ASSIGNED_USER_ID") >= 0 %>' CommandName="Reports.View" CommandArgument='<%# Eval("ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" AlternateText='<%# L10n.Term("Reports.LNK_VIEW") %>' SkinID="view_inline" Runat="server" />
					<asp:LinkButton  Visible='<%# SplendidCRM.Security.GetRecordAccess(Container, m_sMODULE, "view", "ASSIGNED_USER_ID") >= 0 %>' CommandName="Reports.View" CommandArgument='<%# Eval("ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" Text='<%# L10n.Term("Reports.LNK_VIEW") %>' Runat="server" />
					&nbsp;
					<asp:ImageButton Visible='<%# SplendidCRM.Security.GetRecordAccess(Container, m_sMODULE, "edit", "ASSIGNED_USER_ID") >= 0 %>' CommandName="Reports.Unpublish" CommandArgument='<%# Eval("ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" AlternateText='<%# L10n.Term("Reports.LNK_UNPUBLISH") %>' SkinID="unpublish_inline" Runat="server" />
					<asp:LinkButton  Visible='<%# SplendidCRM.Security.GetRecordAccess(Container, m_sMODULE, "edit", "ASSIGNED_USER_ID") >= 0 %>' CommandName="Reports.Unpublish" CommandArgument='<%# Eval("ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" Text='<%# L10n.Term("Reports.LNK_UNPUBLISH") %>' Runat="server" />
				</ItemTemplate>
			</asp:TemplateColumn>
		</Columns>
	</SplendidCRM:SplendidGrid>
