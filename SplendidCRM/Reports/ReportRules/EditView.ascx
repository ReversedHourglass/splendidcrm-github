<%@ Control CodeBehind="EditView.ascx.cs" Language="c#" AutoEventWireup="false" Inherits="SplendidCRM.ReportRules.EditView" %>
<script runat="server">
/**********************************************************************************************************************
...
{
	var sModule = '<%= lstMODULE.SelectedValue.ToUpper() %>';
	var fld = document.getElementById(sID);
	if ( fld != undefined )
	{
		switch ( sCsType )
		{
			case 'Guid'      :  fld.value += 'this.ToGuid(this["'     + sModule + '.' + sValue + '"]) ';  break;
			case 'short'     :  fld.value += 'this.ToShort(this["'    + sModule + '.' + sValue + '"]) ';  break;
			case 'Int32'     :  fld.value += 'this.ToInteger(this["'  + sModule + '.' + sValue + '"]) ';  break;
			case 'Int16'     :  fld.value += 'this.ToInteger(this["'  + sModule + '.' + sValue + '"]) ';  break;
			case 'Int64'     :  fld.value += 'this.ToLong(this["'     + sModule + '.' + sValue + '"]) ';  break;
			case 'float'     :  fld.value += 'this.ToFloat(this["'    + sModule + '.' + sValue + '"]) ';  break;
			case 'decimal'   :  fld.value += 'this.ToDecimal(this["'  + sModule + '.' + sValue + '"]) ';  break;
			case 'bool'      :  fld.value += 'this.ToBoolean(this["'  + sModule + '.' + sValue + '"]) ';  break;
			case 'ansistring':  fld.value += 'this.ToString(this["'   + sModule + '.' + sValue + '"]) ';  break;
			case 'string'    :  fld.value += 'this.ToString(this["'   + sModule + '.' + sValue + '"]) ';  break;
			case 'DateTime'  :  fld.value += 'this.ToDateTime(this["' + sModule + '.' + sValue + '"]) ';  break;
			case 'byte[]'    :  fld.value += 'this.ToBinary(this["'   + sModule + '.' + sValue + '"]) ';  break;
			default          :  fld.value += 'this.ToString(this["'   + sModule + '.' + sValue + '"]) ';  break;
		}
	}
}
...
<div id="divEditView" runat="server">
	<asp:UpdatePanel UpdateMode="Conditional" runat="server">
		<ContentTemplate>
			...
			<SplendidCRM:HeaderButtons ID="ctlDynamicButtons" ShowRequired="true" EditView="true" Module="ReportRules" Title=".moduleList.Home" EnablePrint="false" HelpName="EditView" EnableHelp="true" Visible="<%# ShowHeader %>" Runat="Server" />
			...
			<%-- 06/01/2015 Paul.  InlineScript with DumpSQL is causing a JavaScript error in Chrome, Firefox and Safari. --%>
			<%-- 05/31/2015 Paul.  Combine ModuleHeader and DynamicButtons. --%>
			<%-- 03/16/2016 Paul.  HeaderButtons must be inside UpdatePanel in order to display errors. --%>
