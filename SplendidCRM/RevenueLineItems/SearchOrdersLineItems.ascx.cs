...
namespace SplendidCRM.RevenueLineItems
{
	...
	/// </summary>
	public class SearchLineItems : SplendidControl
	{
			...
			if ( !Sql.IsEmptyString(sUnifiedSearch.Trim()) )
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					sSQL = "select " + Sql.FormatSelectFields(arrSelectFields) + ControlChars.CrLf
					     + "  from vwOPPORTUNITIES_REVENUE_LINE_ITEMS" + ControlChars.CrLf;
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						if ( bDebug )
							RegisterClientScriptBlock("vwOPPORTUNITIES_REVENUE_LINE_ITEMS", Sql.ClientScriptBlock(cmd));
