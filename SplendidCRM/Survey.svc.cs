/*
 * Copyright (C) 2013-2018 SplendidCRM Software, Inc. All Rights Reserved. 
 *
...
namespace SplendidCRM
{
	...
	public class Survey
	{
		...
		public Stream GetSurvey(Guid ID, bool IMAGE_CONTENT)
		{
			HttpApplicationState Application = HttpContext.Current.Application;
			HttpRequest          Request     = HttpContext.Current.Request    ;
		...
		public Guid UpdateSurvey(Stream input)
		{
			...
			try
			{
				...
				string   sIP_ADDRESS      = Request.UserHostName;
				...
				{
					SurveyCreateLead build = new SurveyCreateLead(HttpContext.Current, gID, gCURRENT_USER_ID, gCURRENT_TEAM_ID);
		...
		public class SurveyCreateLead
		{
			...
			public SurveyCreateLead(HttpContext Context, Guid gSURVEY_RESULT_ID, Guid gCURRENT_USER_ID, Guid gCURRENT_TEAM_ID)
			{
					...
					using ( IDbConnection con = dbf.CreateConnection() )
					{
						...
						using ( DataTable dt = new DataTable() )
						{
							...
							if ( dt.Rows.Count > 0 )
							{
								...
								bool   bEnableTeamManagement     = Sql.ToBoolean(Context.Application["CONFIG.enable_team_management"]) || Sql.ToBoolean(Context.Application["CONFIG.enable_multi_tenant_teams"]);
								string sTABLE_NAME               = Crm.Modules.TableName(Context.Application, sSURVEY_TARGET_MODULE);
								// 01/04/2019 Paul.  Add SURVEY_TARGET_ASSIGNMENT. 
