...
namespace SplendidCRM.SurveyPages
{
	...
	public class NewRecord : NewRecordControl
	{
		...
		{
			get { return Sql.ToGuid(ViewState["SURVEY_ID"]); }
			set { ViewState["SURVEY_ID"] = value; }
		}
		...
		public override bool IsEmpty()
		{
			...
			Guid gSURVEY_ID = new DynamicControl(this, "SURVEY_ID").ID;
			if ( Sql.IsEmptyGuid(gSURVEY_ID) )
				gSURVEY_ID = this.SURVEY_ID;
			return Sql.IsEmptyGuid(gSURVEY_ID);
		}
				...
				if ( e.CommandName == "NewRecord" )
				{
					...
					if ( Page.IsValid )
					{
						...
						if ( !Sql.IsEmptyString(RulesRedirectURL) )
							...
							Response.Redirect("~/SurveyPages/view.aspx?ID=" + gID.ToString());
					}
				}
				else if ( Command != null )
				{
			...
			try
			{
				...
				if ( !bIsPostBack )
				{
					...
					if ( bShowFullForm || bShowCancel || sEditView != "NewRecord" )
					{
						...
						Guid gSURVEY_ID = this.SURVEY_ID;
						if ( !Sql.IsEmptyGuid(gSURVEY_ID) )
						{
