...
{
	/// <summary>
	///		Summary description for RespondantsPopupView.
	/// </summary>
	public class RespondantsPopupView : SplendidControl
	{
		...
		protected void grdMain_OnSelectMethod(int nCurrentPageIndex, int nPageSize)
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					cmd.CommandText = "  from vw" + sTABLE_NAME + "_Edit" + ControlChars.CrLf;
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				gSURVEY_ID = Sql.ToGuid(Request["SURVEY_ID"]);
				DbProviderFactory dbf = DbProviderFactories.GetFactory();
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					con.Open();
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						grdMain.OrderByClause("PARENT_NAME", "asc");
		...
		{    
			...
			this.AppendGridColumns(grdMain, m_sMODULE + ".RespondantsPopupView", arrSelectFields);
			ctlDynamicButtons.AppendButtons(m_sMODULE + ".RespondantsPopup" + (bMultiSelect ? "MultiSelect" : "View"), Guid.Empty, Guid.Empty);
