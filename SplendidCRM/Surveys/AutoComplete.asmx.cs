...
namespace SplendidCRM.Surveys
{
	...
	public class AutoComplete : System.Web.Services.WebService
	{
		...
		public Survey SURVEYS_SURVEY_NAME_Get(string sNAME)
		{
			...
			if ( Sql.IsEmptyGuid(item.ID) )
			{
				...
				throw(new Exception(L10n.Term("Surveys.ERR_SURVEY_NOT_FOUND")));
			}
			return item;
		}

		[WebMethod(EnableSession=true)]
		public string[] SURVEYS_SURVEY_NAME_List(string prefixText, int count)
		{
		...
		[WebMethod(EnableSession=true)]
		public string[] SURVEYS_NAME_List(string prefixText, int count)
		{
			return SURVEYS_SURVEY_NAME_List(prefixText, count);
		}
	}
}
