<%@ Control Language="c#" AutoEventWireup="false" Codebehind="DetailView.ascx.cs" Inherits="SplendidCRM.Surveys.DetailView" TargetSchema="http://schemas.microsoft.com/intellisense/ie5" %>
<script runat="server">
/**********************************************************************************************************************
...
function Test()
{
	window.open('<%= GetSurveySiteURL() %>run.aspx?ID=<%= gID %>', 'SurveyRun_<%= gID.ToString().Replace("-", "_") %>');
	return false;
}
</script>
<div id="divDetailView" runat="server">
	...
	<SplendidCRM:HeaderButtons ID="ctlDynamicButtons" Module="Surveys" EnablePrint="true" HelpName="DetailView" EnableHelp="true" EnableFavorites="true" Runat="Server" />
