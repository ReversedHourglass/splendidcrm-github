...
namespace SplendidCRM.TwitterTracks
{
	...
	public class DetailView : SplendidControl
	{
		...
		protected string      sUSER_TWITTER_TRACKS;
		...
		protected void Page_Command(Object sender, CommandEventArgs e)
		{
			...
			else if ( e.CommandName == "Delete" )
			{
				try
				{
					SqlProcs.spTWITTER_TRACKS_Delete(gID);
								...
								using ( DbDataAdapter da = dbf.CreateDataAdapter() )
								{
									...
									using ( DataTable dtCurrent = new DataTable() )
									{
										...
										if ( dtCurrent.Rows.Count > 0 && (SplendidCRM.Security.GetRecordAccess(dtCurrent.Rows[0], m_sMODULE, "view", "ASSIGNED_USER_ID") >= 0) )
										{
											...
											// 11/04/2013 Paul.  Show immediate tracking. 
											// 11/05/2013 paul.  The TwitterManagerHub expects the track to be in lowercase. 
											sUSER_TWITTER_TRACKS = Sql.ToString(rdr["NAME"]).ToLower();
											ScriptManager mgrAjax = ScriptManager.GetCurrent(this.Page);
											TwitterManager.RegisterScripts(Context, mgrAjax);
										}
										else
										{
