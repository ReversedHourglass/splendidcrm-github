...
{
	/// <summary>
	///		Summary description for RedistributeView.
	/// </summary>
	public class RedistributeView : SplendidControl
	{
		...
		protected void Page_Command(Object sender, CommandEventArgs e)
		{
			try
			{
				...
				{
					int nSelectedUsers = 0;
					StringBuilder sbUsers = new StringBuilder();
					foreach ( ListItem item in lstTO.Items )
					{
						if ( item.Selected )
						{
							nSelectedUsers++;
							if ( lstFROM.SelectedValue == item.Value )
							{
								// 04/17/2013 Paul.  We may not need this restriction.  If the source and destination user is the same, then don't call the stored procedure. 
								//throw(new Exception(L10n.Term("Users.ERR_REDISTRIBUTE_DIFF_USERS")));
							}
							if ( sbUsers.Length > 0 )
								sbUsers.Append(", ");
							sbUsers.Append(item.Text);
						}
					}
					if ( nSelectedUsers == 0 )
					{
						throw(new Exception(L10n.Term("Users.ERR_REDISTRIBUTE_SELECT_USERS")));
					}
					...
					if ( Page.IsValid )
					{
						...
						using ( IDbConnection con = dbf.CreateConnection() )
						{
							con.Open();
							string sSQL;
							ListItem item = lstMODULES.SelectedItem;
								...
								if ( item.Selected )
								{
									...
									using ( IDbCommand cmd = con.CreateCommand() )
									{
										...
										try
										{
											...
											SqlProcs.Factory(con, "sp" + sTABLE_NAME + "_MassRedist");
											litUpdated.Text = String.Format(L10n.Term("Users.LBL_REDISTRIBUTE_WILL_BE_UPDATED"), nUpdatedRecords.ToString(), item.Text, sbUsers.ToString());
										}
										catch ( Exception ex )
										{
									...
									//foreach ( ListItem item in lstMODULES.Items )
									{
										if ( item.Selected )
										{
											...
											using ( IDbCommand cmd = con.CreateCommand() )
											{
												...
												IDbCommand cmdMassAssign = SqlProcs.Factory(con, "sp" + sTABLE_NAME + "_MassRedist");
												...
												Sql.SetParameter(cmdMassAssign, "@EXISTING_USER_ID", Sql.ToGuid(lstFROM.SelectedValue));
												Sql.SetParameter(cmdMassAssign, "@PARENT_MODULE"   , String.Empty    );
												
												int nUserDistributionCount = stk.Count / nSelectedUsers;
												if ( stk.Count % nSelectedUsers > 0 )
													nUserDistributionCount++;
												foreach ( ListItem itemTO in lstTO.Items )
												{
													if ( itemTO.Selected )
													{
														...
														Literal litUpdated = new Literal();
														td.Controls.Add(litUpdated);
														litUpdated.Text = String.Empty;

														System.Collections.Stack stkTO = new System.Collections.Stack();
														for ( int i = 0; i < nUserDistributionCount && stk.Count > 0; i++ )
														{
															stkTO.Push(stk.Pop());
														}
														if ( itemTO.Value != lstFROM.SelectedValue )
														{
															nUpdatedRecords = stkTO.Count;
															while ( stkTO.Count > 0 )
															{
																string sIDs = Utils.BuildMassIDs(stkTO);
															...
															litUpdated.Text += String.Format(L10n.Term("Users.LBL_REDISTRIBUTE_SUCCESSFUL"), nUpdatedRecords.ToString(), item.Text);
#if DEBUG
															litUpdated.Text += "<pre>" + Sql.ExpandParameters(cmd) + "</pre>";
#endif
														}
				...
				if ( !IsPostBack )
				{
					...
					using ( IDbConnection con = dbf.CreateConnection() )
					{
						...
						sSQL = "select MODULE_NAME      " + ControlChars.CrLf
						     ...
						     + "   and MODULE_NAME not in (N'Activities', N'BusinessRules', N'CallMarketing', N'RulesWizard', N'ReportRules', N'WorkflowAlertShells')" + ControlChars.CrLf
