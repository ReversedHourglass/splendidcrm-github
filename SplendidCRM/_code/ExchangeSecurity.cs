...
namespace SplendidCRM
{
	...
	public class ExchangeSecurity
	{
		...
		public static void SetUserAccess(ExchangeSession Session, string sMODULE_NAME, string sACCESS_TYPE, int nACLACCESS)
		{
		...
		public static void SetUserFieldSecurity(ExchangeSession Session, string sMODULE_NAME, string sFIELD_NAME, int nACLACCESS)
		{
			if ( Sql.IsEmptyString(sMODULE_NAME) )
				throw(new Exception("sMODULE_NAME should not be empty."));
		...
		public static Security.ACL_FIELD_ACCESS GetUserFieldSecurity(ExchangeSession Session, string sMODULE_NAME, string sFIELD_NAME, Guid gASSIGNED_USER_ID)
		{
			int nACLACCESS = GetUserFieldSecurity(Session, sMODULE_NAME, sFIELD_NAME);
			Security.ACL_FIELD_ACCESS acl = new Security.ACL_FIELD_ACCESS(nACLACCESS, gASSIGNED_USER_ID);
			return acl;
		}
		
		// 06/09/2009 Paul.  We need to access LoadUserACL from the SOAP calls. 
		public static ExchangeSession LoadUserACL(HttpApplicationState Application, Guid gUSER_ID)
		{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						using ( IDataReader rdr = cmd.ExecuteReader() )
						{
							while ( rdr.Read() )
							{
								...
								ExchangeSecurity.SetUserAccess(Session, sMODULE_NAME, "admin" , Sql.ToInteger(rdr["ACLACCESS_ADMIN" ]));
								ExchangeSecurity.SetUserAccess(Session, sMODULE_NAME, "access", Sql.ToInteger(rdr["ACLACCESS_ACCESS"]));
								ExchangeSecurity.SetUserAccess(Session, sMODULE_NAME, "view"  , Sql.ToInteger(rdr["ACLACCESS_VIEW"  ]));
								ExchangeSecurity.SetUserAccess(Session, sMODULE_NAME, "list"  , Sql.ToInteger(rdr["ACLACCESS_LIST"  ]));
								ExchangeSecurity.SetUserAccess(Session, sMODULE_NAME, "edit"  , Sql.ToInteger(rdr["ACLACCESS_EDIT"  ]));
								ExchangeSecurity.SetUserAccess(Session, sMODULE_NAME, "delete", Sql.ToInteger(rdr["ACLACCESS_DELETE"]));
								ExchangeSecurity.SetUserAccess(Session, sMODULE_NAME, "import", Sql.ToInteger(rdr["ACLACCESS_IMPORT"]));
								ExchangeSecurity.SetUserAccess(Session, sMODULE_NAME, "export", Sql.ToInteger(rdr["ACLACCESS_EXPORT"]));
								// 09/26/2017 Paul.  Add Archive access right. 
								ExchangeSecurity.SetUserAccess(Session, sMODULE_NAME, "archive", Sql.ToInteger(rdr["ACLACCESS_ARCHIVE"]));
							}
						}
					}
						...
						using ( IDbCommand cmd = con.CreateCommand() )
						{
							...
							using ( IDataReader rdr = cmd.ExecuteReader() )
							{
								while ( rdr.Read() )
								{
									...
									ExchangeSecurity.SetUserFieldSecurity(Session, sMODULE_NAME, sFIELD_NAME, nACLACCESS);
								}
							}
						}
					}
				}
				Cache.Insert("ExchangeSession." + gUSER_ID.ToString(), Session, null, DefaultCacheExpiration(), Cache.NoSlidingExpiration);
			}
			return Session;
		}
		...
		public static int GetUserAccess(HttpApplicationState Application, ExchangeSession Session, string sMODULE_NAME, string sACCESS_TYPE)
		{
			...
			if ( sMODULE_NAME == "Calendar" )
			{
				...
				int nACLACCESS_Calls    = ExchangeSecurity.GetUserAccess(Application, Session, "Calls"   , sACCESS_TYPE);
				int nACLACCESS_Meetings = ExchangeSecurity.GetUserAccess(Application, Session, "Meetings", sACCESS_TYPE);
			...
			else if ( sMODULE_NAME == "Activities" )
			{
				...
				int nACLACCESS_Calls    = ExchangeSecurity.GetUserAccess(Application, Session, "Calls"   , sACCESS_TYPE);
				int nACLACCESS_Meetings = ExchangeSecurity.GetUserAccess(Application, Session, "Meetings", sACCESS_TYPE);
				int nACLACCESS_Tasks    = ExchangeSecurity.GetUserAccess(Application, Session, "Tasks"   , sACCESS_TYPE);
				int nACLACCESS_Notes    = ExchangeSecurity.GetUserAccess(Application, Session, "Notes"   , sACCESS_TYPE);
				int nACLACCESS_Emails   = ExchangeSecurity.GetUserAccess(Application, Session, "Emails"  , sACCESS_TYPE);
		...
		public static void Filter(HttpApplicationState Application, ExchangeSession Session, IDbCommand cmd, Guid gASSIGNED_USER_ID,string sMODULE_NAME, string sACCESS_TYPE)
		{
			ExchangeSecurity.Filter(Application, Session, cmd, gASSIGNED_USER_ID, sMODULE_NAME, sACCESS_TYPE, "ASSIGNED_USER_ID", false);
		}
		...
		public static void Filter(HttpApplicationState Application, ExchangeSession Session, IDbCommand cmd, Guid gASSIGNED_USER_ID, string sMODULE_NAME, string sACCESS_TYPE, string sASSIGNED_USER_ID_Field, bool bActivitiesFilter)
		{
			...
			//else
				nACLACCESS = ExchangeSecurity.GetUserAccess(Application, Session, sMODULE_NAME, sACCESS_TYPE);
			...
			if ( bModuleIsAssigned )
			{
				...
				if ( nACLACCESS == ACL_ACCESS.OWNER || (bRequireUserAssignment && !bIsAdmin) )
				{
					...
					else
					{
						...
						bool bShowUnassigned = Crm.Config.show_unassigned(Application);
						if ( bShowUnassigned )
						{
