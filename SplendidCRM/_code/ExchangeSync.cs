...
namespace SplendidCRM
{
	public class ExchangeSync
	{
		...
		public static void StopPullSubscription(HttpContext Context, Guid gUSER_ID)
		{
			if ( dictPullSubscriptions.ContainsKey(gUSER_ID) )
			{
				try
				{
					...
					SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "Sync: Stopping Pull subscription for " + gUSER_ID.ToString() + ", " + pull.Id);
					pull.Unsubscribe();
		...
		public static void StopPushSubscription(HttpContext Context, Guid gUSER_ID)
		{
			if ( dictPushSubscriptions.ContainsKey(gUSER_ID) )
			{
				try
				{
					...
					SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "Sync: Stopping Push subscription for " + gUSER_ID.ToString() + ", " + push.Id);
		...
		public static void StopSubscriptions(HttpContext Context)
		{
			try
			{
				...
				if ( Context != null )
				{
					...
					{
						StopPullSubscription(Context, gUSER_ID);
					}
					foreach ( Guid gUSER_ID in arrPushUsers )
					{
						StopPushSubscription(Context, gUSER_ID);
					}
				}
			}
			catch(Exception ex)
		...
		public static void GetPushSubscription(string sSubscriptionID, ref PushSubscription push, ref Guid gUSER_ID)
		{
		...
		public static void SyncAllUsers(Object sender)
		{
			...
			string sUSER_NAME  = Sql.ToString(Context.Application["CONFIG.Exchange.UserName" ]);
			...
			{
				Spring.Social.Office365.Office365Sync.SyncAllUsers(sender);
			}
				...
				try
				{
					using ( DataTable dt = new DataTable() )
					{
						...
						using ( IDbConnection con = dbf.CreateConnection() )
						{
							...
							bool   bPushNotifications   = Sql.ToBoolean(Context.Application["CONFIG.Exchange.PushNotifications"  ]);
							string sPushNotificationURL = Sql.ToString (Context.Application["CONFIG.Exchange.PushNotificationURL"]);
						...
						foreach ( DataRow row in dt.Rows )
						{
							...
							{
								if ( bVERBOSE_STATUS )
									SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "ExchangeSync.SyncAllUsers(" + sEXCHANGE_ALIAS + " " + sEXCHANGE_EMAIL + ")<br />" + "No OAuth record exists.  The user will need to re-authorize.");
								continue;
							}
							StringBuilder sbErrors = new StringBuilder();
							// 11/23/2011 Paul.  Add MAIL_SMTPUSER and MAIL_SMTPPASS so that we can avoid impersonation. 
							ExchangeSync.UserSync User = new ExchangeSync.UserSync(Context, sEXCHANGE_ALIAS, sEXCHANGE_EMAIL, sMAIL_SMTPUSER, sMAIL_SMTPPASS, gASSIGNED_USER_ID, sEXCHANGE_WATERMARK, false, bOFFICE365_OAUTH_ENABLED);
		...
		public static void SyncUser(HttpContext Context, Guid gUSER_ID)
		{
			...
			if ( !Sql.IsEmptyString(sSERVER_URL) && !Sql.IsEmptyString(sOAUTH_CLIENT_ID) && !Sql.IsEmptyString(sOAUTH_CLIENT_SECRET) )
			{
				Spring.Social.Office365.Office365Sync.SyncUser(Context, gUSER_ID);
			}
		...
		public class UserSync
		{
			...
			public void SyncSentItems()
			{
				...
				try
				{
					...
					{
						con.Open();
						ExchangeSync.SyncSentItems(Context, Session, service, con, this.EXCHANGE_ALIAS, this.USER_ID, true, DateTime.MinValue, sbErrors);
					}
				}
				catch(Exception ex)
				{
			...
			public void SyncInbox()
			{
				...
				try
				{
					...
					{
						con.Open();
						ExchangeSync.SyncInbox(Context, Session, service, con, this.EXCHANGE_ALIAS, this.USER_ID, true, DateTime.MinValue, sbErrors);
					}
				}
				catch(Exception ex)
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						using ( IDataReader row = cmd.ExecuteReader(CommandBehavior.SingleRow) )
						{
							if ( row.Read() )
							{
								...
								User = new ExchangeSync.UserSync(Context, sEXCHANGE_ALIAS, sEXCHANGE_EMAIL, sMAIL_SMTPUSER, sMAIL_SMTPPASS, gASSIGNED_USER_ID, sEXCHANGE_WATERMARK, bSyncAll, bOFFICE365_OAUTH_ENABLED);
							}
						}
					}
				}
		...
		#region Sync
		// 11/23/2011 Paul.  Add MAIL_SMTPUSER and MAIL_SMTPPASS so that we can avoid impersonation. 
		public static void Sync(UserSync User, StringBuilder sbErrors)
		{
			...
			if ( !arrProcessing.Contains(User.USER_ID) )
			{
				...
				try
				{
					...
					if ( !bDisableContacts )
						ExchangeSync.SyncContacts    (User.Context, Session, service, User.EXCHANGE_ALIAS, User.USER_ID, User.SyncAll, sbErrors);
					...
					if ( !bDisableAppointments )
						ExchangeSync.SyncAppointments(User.Context, Session, service, User.EXCHANGE_ALIAS, User.USER_ID, User.SyncAll, sbErrors);
					...
					using ( IDbConnection con = dbf.CreateConnection() )
					{
						...
						if ( !dictSubscriptions.ContainsKey(User.USER_ID) )
						{
							...
							// 07/18/2010 Paul.  Move Exchange Sync functions to a separate class. 
							ExchangeSync.SyncModuleFolders(User.Context, Session, service, con, fldExchangeRoot, ref fldSplendidRoot, ref fldModuleFolder, String.Empty, Guid.Empty, String.Empty, User.EXCHANGE_ALIAS, User.USER_ID, User.SyncAll, sbErrors);
							...
							foreach ( DataRow rowModule in dtSync.Rows )
							{
								...
								bool   bEXCHANGE_FOLDERS = Sql.ToBoolean(rowModule["EXCHANGE_FOLDERS"]);
								// 07/18/2010 Paul.  Move Exchange Sync functions to a separate class. 
								ExchangeSync.SyncModuleFolders(User.Context, Session, service, con, fldExchangeRoot, ref fldSplendidRoot, ref fldModuleFolder, sMODULE_NAME, Guid.Empty, String.Empty, User.EXCHANGE_ALIAS, User.USER_ID, User.SyncAll, sbErrors);
								if ( bEXCHANGE_FOLDERS )
								{
									...
									if ( dtModuleFolders != null )
									{
										foreach ( DataRow rowFolder in dtModuleFolders.Rows )
										{
											...
											// 07/18/2010 Paul.  Move Exchange Sync functions to a separate class. 
											ExchangeSync.SyncModuleFolders(User.Context, Session, service, con, fldExchangeRoot, ref fldSplendidRoot, ref fldModuleFolder, sMODULE_NAME, gPARENT_ID, sPARENT_NAME, User.EXCHANGE_ALIAS, User.USER_ID, User.SyncAll || bNEW_FOLDER, sbErrors);
										}
									}
								}
							}
									...
									if ( bEXCHANGE_FOLDERS )
									{
										...
										if ( dtModuleFolders != null )
										{
											foreach ( DataRow rowFolder in dtModuleFolders.Rows )
											{
												...
												string sPARENT_NAME = Sql.ToString(rowFolder["NAME"]);
												// 07/18/2010 Paul.  Move Exchange Sync functions to a separate class. 
												ExchangeSync.SyncModuleFolders(User.Context, Session, service, con, fldExchangeRoot, ref fldSplendidRoot, ref fldModuleFolder, sMODULE_NAME, gPARENT_ID, sPARENT_NAME, User.EXCHANGE_ALIAS, User.USER_ID, User.SyncAll, sbErrors);
											}
										}
									}
								}
							...
							if ( bSentItemsSync )
							{
								...
								ExchangeSync.SyncSentItems(User.Context, Session, service, con, User.EXCHANGE_ALIAS, User.USER_ID, false, DateTime.UtcNow.AddHours(-2), sbErrors);
							}
							// 07/05/2017 Paul.  Import messages from Inbox if the FROM email exists in the CRM. 
							if ( bInboxSync )
							{
								...
								ExchangeSync.SyncInbox(User.Context, Session, service, con, User.EXCHANGE_ALIAS, User.USER_ID, false, DateTime.UtcNow.AddHours(-2), sbErrors);
							}
							...
							SyncError.SystemMessage(User.Context, "Warning", new StackTrace(true).GetFrame(0), "Sync: Adding " + (bPushNotifications ? "Push" : "Pull") + " subscription for " + User.EXCHANGE_ALIAS + ", " + User.USER_ID.ToString() + " on " + sMachineName + "." + ControlChars.CrLf + "Folders: " + sbUserFolders.ToString());
									...
									if ( vwUserFolders.Count > 0 )
									{
										...
										try
										{
											if ( bWELL_KNOWN_FOLDER && sMODULE_NAME == "Contacts" )
											{
												...
												{
													// 07/18/2010 Paul.  Move Exchange Sync functions to a separate class. 
													ExchangeSync.ImportContact(User.Context, Session, service, con, User.EXCHANGE_ALIAS, User.USER_ID, contact, sbErrors);
											...
											else if ( bWELL_KNOWN_FOLDER && sMODULE_NAME == "Calendar" )
											{
												...
												{
													// 07/18/2010 Paul.  Move Exchange Sync functions to a separate class. 
													ExchangeSync.ImportAppointment(User.Context, Session, service, con, User.EXCHANGE_ALIAS, User.USER_ID, appointment, sbErrors);
													bUpdateWatermark = true;
												}
											}
											// 03/11/2012 Paul.  Import messages from Sent Items if the TO email exists in the CRM. 
											else if ( bWELL_KNOWN_FOLDER && sMODULE_NAME == "Sent Items" )
											{
												...
												ExchangeSync.ImportSentItem(User.Context, Session, con, User.EXCHANGE_ALIAS, User.USER_ID, email, sbErrors);
												bUpdateWatermark = true;
											}
											// 07/05/2017 Paul.  Import messages from Inbox if the FROM email exists in the CRM. 
											else if ( bWELL_KNOWN_FOLDER && sMODULE_NAME == "Inbox" )
											{
												...
												{
													ExchangeSync.ImportInbox(User.Context, Session, con, User.EXCHANGE_ALIAS, User.USER_ID, email, sbErrors);
													bUpdateWatermark = true;
												}
											}
											else
											{
												EmailMessage email = EmailMessage.Bind(service, sItemID);
												// 07/18/2010 Paul.  Move Exchange Sync functions to a separate class. 
												ExchangeSync.ImportMessage(User.Context, Session, con, sMODULE_NAME, gPARENT_ID, User.EXCHANGE_ALIAS, User.USER_ID, email, sbErrors);
												bUpdateWatermark = true;
											}
										}
										catch(Exception ex)
		...
		// 03/12/2013 Paul.  Use sCRM_FOLDER_NAME as the category name. 
		private static bool SetExchangeContact(Contact contact, DataRow row, StringBuilder sbChanges, string sCONTACTS_CATEGORY)
		{
			bool bChanged = false;
		...
		public static void SyncContacts(HttpContext Context, ExchangeSession Session, ExchangeService service, string sEXCHANGE_ALIAS, Guid gUSER_ID, bool bSyncAll, StringBuilder sbErrors)
		{
			bool bVERBOSE_STATUS = Sql.ToBoolean(Context.Application["CONFIG.Exchange.VerboseStatus"]);
			if ( bVERBOSE_STATUS )
						...
						do
						{
							...
							foreach (Item itemContact in results.Items )
							{
								if ( itemContact is Contact )
								{
									...
									// 07/18/2010 Paul.  Move Exchange Sync functions to a separate class. 
									ExchangeSync.ImportContact(Context, Session, service, con, sEXCHANGE_ALIAS, gUSER_ID, contact, sbErrors);
								}
							}
								...
								foreach ( DataRow row in dt.Rows )
								{
									...
									try
									{
										...
										if ( sSYNC_ACTION == "local new" || Sql.IsEmptyString(sSYNC_REMOTE_KEY) )
										{
											...
											// 03/12/2013 Paul.  Use sCRM_FOLDER_NAME as the category name. 
											// 09/02/2013 Paul.  Add separate category fields so that we can allow an empty category to mean all. 
											ExchangeSync.SetExchangeContact(contact, row, sbChanges, sCONTACTS_CATEGORY);
											contact.Save();
											contact.Load();
										...
										else
										{
											...
											if ( sSYNC_ACTION == "local changed" )
											{
												...
												// 03/12/2013 Paul.  Use sCRM_FOLDER_NAME as the category name. 
												// 09/02/2013 Paul.  Add separate category fields so that we can allow an empty category to mean all. 
												bool bChanged = ExchangeSync.SetExchangeContact(contact, row, sbChanges, sCONTACTS_CATEGORY);
												if ( bChanged )
												{
													if ( bVERBOSE_STATUS )
		...
		public static void ImportContact(HttpContext Context, ExchangeSession Session, ExchangeService service, IDbConnection con, string sEXCHANGE_ALIAS, Guid gUSER_ID, Contact contact, StringBuilder sbErrors)
		{
			HttpApplicationState Application = Context.Application;
							...
							using ( DbDataAdapter da = dbf.CreateDataAdapter() )
							{
								...
								using ( DataTable dt = new DataTable() )
								{
									...
									foreach ( DataRow row in dt.Rows )
									{
										...
										if ( bVERBOSE_STATUS )
											SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "ExchangeSync.UnsyncContact: Unsyncing contact for " + sEXCHANGE_ALIAS + " to " + gCONTACT_ID.ToString());
										
										if ( !Sql.IsEmptyString(sSERVER_URL) && !Sql.IsEmptyString(sOAUTH_CLIENT_ID) && !Sql.IsEmptyString(sOAUTH_CLIENT_SECRET) )
										{
											Spring.Social.Office365.Office365Sync.UnsyncContact(Context, gUSER_ID, sEXCHANGE_ALIAS, gCONTACT_ID, sREMOTE_KEY);
										}
										else if ( !Sql.IsEmptyString(sSERVER_URL) && !Sql.IsEmptyString(sUSER_NAME) )
										{
											ExchangeSync.UnsyncContact(Context, gUSER_ID, sEXCHANGE_ALIAS, gCONTACT_ID, sREMOTE_KEY);
										}
									}
								}
							}
		...
		public static void UnsyncContact(HttpContext Context, Guid gUSER_ID, string sEXCHANGE_ALIAS, Guid gCONTACT_ID, string sREMOTE_KEY)
		{
			try
			{
				ExchangeSync.UserSync User = ExchangeSync.UserSync.Create(Context, gUSER_ID, false);
				if ( User != null )
				{
					HttpApplicationState Application = Context.Application;
					string sCONTACTS_CATEGORY     = Sql.ToString(Application["CONFIG.Exchange.Contacts.Category"]);
					...
					using ( IDbConnection con = dbf.CreateConnection() )
					{
						...
						bool bVERBOSE_STATUS = Sql.ToBoolean(Application["CONFIG.Exchange.VerboseStatus"]);
						if ( bVERBOSE_STATUS )
							SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "ExchangeSync.UnsyncContact: for " + sEXCHANGE_ALIAS + " to " + sREMOTE_KEY + ".");
		...
		// 03/12/2013 Paul.  Use sCRM_FOLDER_NAME as the category name. 
		private static bool SetExchangeAppointment(Appointment appointment, DataRow row, DataTable dtAppointmentEmails, StringBuilder sbChanges, bool bDISABLE_PARTICIPANTS, string sCALENDAR_CATEGORY)
		{
			bool bChanged = false;
		...
		public static void SyncAppointments(HttpContext Context, ExchangeSession Session, ExchangeService service, string sEXCHANGE_ALIAS, Guid gUSER_ID, bool bSyncAll, StringBuilder sbErrors)
		{
			// 07/26/2012 James.  Add the ability to disable participants. 
				...
				try
				{
					...
					//if ( bSyncAll )
					{
						...
						do
						{
							...
							if ( results.Items.Count > 0 )
								SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncAppointments: " + results.Items.Count.ToString() + " appointments to retrieve from " + sEXCHANGE_ALIAS);
							foreach (Item itemAppointment in results.Items )
							{
								if ( itemAppointment is Appointment )
								{
									...
									// 07/18/2010 Paul.  Move Exchange Sync functions to a separate class. 
									ExchangeSync.ImportAppointment(Context, Session, service, con, sEXCHANGE_ALIAS, gUSER_ID, appointment, sbErrors);
								}
							}
							nPageOffset += nPageSize;
						}
								...
								foreach ( DataRow row in dt.Rows )
								{
									...
									try
									{
										...
										if ( sSYNC_ACTION == "local new" || Sql.IsEmptyString(sSYNC_REMOTE_KEY) )
										{
											...
											// 03/12/2013 Paul.  Use sCRM_FOLDER_NAME as the category name. 
											// 09/02/2013 Paul.  Add separate category fields so that we can allow an empty category to mean all. 
											ExchangeSync.SetExchangeAppointment(appointment, row, dtAppointmentEmails, sbChanges, bDISABLE_PARTICIPANTS, sCALENDAR_CATEGORY);
										...
										else
										{
											...
											if ( sSYNC_ACTION == "local changed" )
											{
												...
												// 03/12/2013 Paul.  Use sCRM_FOLDER_NAME as the category name. 
												// 09/02/2013 Paul.  Add separate category fields so that we can allow an empty category to mean all. 
												bool bChanged = ExchangeSync.SetExchangeAppointment(appointment, row, dtAppointmentEmails, sbChanges, bDISABLE_PARTICIPANTS, sCALENDAR_CATEGORY);
												if ( bChanged )
												{
													if ( bVERBOSE_STATUS )
		...
		// 01/24/2012 Paul.  Only update the record if it has changed.  This is to prevent an endless loop caused by sync operations. 
		public static bool BuildAPPOINTMENTS_Update(HttpApplicationState Application, ExchangeSession Session, IDbCommand spAPPOINTMENTS_Update, DataRow row, Appointment appointment, Guid gUSER_ID, Guid gTEAM_ID, Guid gASSIGNED_USER_ID)
		{
			bool bChanged = false;
			// 03/25/2010 Paul.  We start with the existing record values so that we can apply ACL Field Security rules. 
			if ( row != null && row.Table != null )
		...
		public static void ImportAppointment(HttpContext Context, ExchangeSession Session, ExchangeService service, IDbConnection con, string sEXCHANGE_ALIAS, Guid gUSER_ID, Appointment appointment, StringBuilder sbErrors)
		{
			HttpApplicationState Application = Context.Application;
			// 07/26/2012 James.  Add the ability to disable participants. 
					...
					using ( DataTable dt = new DataTable() )
					{
						...
						if ( sSYNC_ACTION == "remote new" || sSYNC_ACTION == "remote changed" )
						{
							...
							using ( IDbTransaction trn = Sql.BeginTransaction(con) )
							{
								...
								{
									if ( bVERBOSE_STATUS )
										SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncAppointments: Retrieving appointment " + Sql.ToString(appointment.Subject) + " " + Sql.ToString(appointment.Start.ToLocalTime().ToString()) + " from " + sEXCHANGE_ALIAS);
						...
						else if ( (sSYNC_ACTION == "local changed" || sSYNC_ACTION == "local new") && !Sql.IsEmptyGuid(gID) )
						{
							...
							if ( dt.Rows.Count > 0 )
							{
								...
								{
									if ( bVERBOSE_STATUS )
										SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncAppointments: Syncing appointment " + Sql.ToString(row["NAME"]) + " " + Sql.ToString(row["DATE_START"]) + " to " + sEXCHANGE_ALIAS);
									// 03/28/2010 Paul.  The EWS Managed API requires one field to change in order to update the record. 
									// 07/18/2010 Paul.  Move Exchange Sync functions to a separate class. 
									DataTable dtAppointmentEmails = AppointmentEmails(Application, con, Sql.ToGuid(row["ID"]));
									// 07/26/2012 James.  Add the ability to disable participants. 
						...
						else if ( sSYNC_ACTION == "local deleted" )
						{
							...
							{
								if ( bVERBOSE_STATUS )
									SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncAppointments: Deleting " + Sql.ToString(appointment.Subject) + " " + Sql.ToString(appointment.Start.ToLocalTime().ToString()) + " from " + sEXCHANGE_ALIAS);
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									try
									{
		...
		#region Sync Folders
		// 03/11/2012 Paul.  Import messages from Sent Items if the TO email exists in the CRM. 
		public static void SyncSentItems(HttpContext Context, ExchangeSession Session, ExchangeService service, IDbConnection con, string sEXCHANGE_ALIAS, Guid gUSER_ID, bool bSyncAll, DateTime dtMAX_SYNC_REMOTE_DATE_MODIFIED_UTC, StringBuilder sbErrors)
		{
			HttpApplicationState Application = Context.Application;
			bool bVERBOSE_STATUS = Sql.ToBoolean(Application["CONFIG.Exchange.VerboseStatus"]);
			...
			try
			{
				...
				if ( bSyncAll || dtMAX_SYNC_REMOTE_DATE_MODIFIED_UTC > DateTime.MinValue )
				{
					...
					do
					{
						...
						if ( results.Items.Count > 0 )
							SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncSentItems: " + results.Items.Count.ToString() + " Sent Items messages to retrieve for " + sEXCHANGE_ALIAS);
						...
						foreach (Item itemMessage in results.Items )
						{
							if ( itemMessage is EmailMessage )
							{
								...
								ExchangeSync.ImportSentItem(Context, Session, con, sEXCHANGE_ALIAS, gUSER_ID, email, sbErrors);
								nImportCount++;
							}
						}
						nPageOffset += nPageSize;
		...
		// 07/05/2017 Paul.  Import messages from Inbox if the FROM email exists in the CRM. 
		public static void SyncInbox(HttpContext Context, ExchangeSession Session, ExchangeService service, IDbConnection con, string sEXCHANGE_ALIAS, Guid gUSER_ID, bool bSyncAll, DateTime dtMAX_SYNC_REMOTE_DATE_MODIFIED_UTC, StringBuilder sbErrors)
		{
			HttpApplicationState Application = Context.Application;
			bool bVERBOSE_STATUS = Sql.ToBoolean(Application["CONFIG.Exchange.VerboseStatus"]);
			...
			try
			{
				...
				if ( bSyncAll || dtMAX_SYNC_REMOTE_DATE_MODIFIED_UTC > DateTime.MinValue )
				{
					...
					do
					{
						...
						if ( results.Items.Count > 0 )
							SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncInbox: " + results.Items.Count.ToString() + " Inbox messages to retrieve for " + sEXCHANGE_ALIAS);
						...
						foreach (Item itemMessage in results.Items )
						{
							...
							{
								EmailMessage email = itemMessage as EmailMessage;
								ExchangeSync.ImportInbox(Context, Session, con, sEXCHANGE_ALIAS, gUSER_ID, email, sbErrors);
								nImportCount++;
							}
						}
						nPageOffset += nPageSize;
						...
						if ( bSyncAll )
						{
							...
							*/
							...
							do
							{
								...
								if ( results.Items.Count > 0 )
									SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncModuleFolders: " + results.Items.Count.ToString() + " " + sMODULE_NAME + (Sql.IsEmptyString(sPARENT_NAME) ? String.Empty : " - " + sPARENT_NAME) + " messages to retrieve for " + sEXCHANGE_ALIAS);
								...
								foreach (Item itemMessage in results.Items )
								{
									if ( itemMessage is EmailMessage )
									{
										EmailMessage email = itemMessage as EmailMessage;
										// 07/18/2010 Paul.  Move Exchange Sync functions to a separate class. 
										ExchangeSync.ImportMessage(Context, Session, con, sMODULE_NAME, gPARENT_ID, sEXCHANGE_ALIAS, gUSER_ID, email, sbErrors);
										nImportCount++;
									}
								}
								nPageOffset += nPageSize;
		...
		// 03/11/2012 Paul.  Import messages from Sent Items if the TO email exists in the CRM. 
		public static void ImportSentItem(HttpContext Context, ExchangeSession Session, IDbConnection con, string sEXCHANGE_ALIAS, Guid gUSER_ID, EmailMessage email, StringBuilder sbErrors)
		{
		...
		// 07/05/2017 Paul.  Import messages from Inbox if the FROM email exists in the CRM. 
		public static void ImportInbox(HttpContext Context, ExchangeSession Session, IDbConnection con, string sEXCHANGE_ALIAS, Guid gUSER_ID, EmailMessage email, StringBuilder sbErrors)
		{
			bool bLoadSuccessful = false;
			string sDESCRIPTION = String.Empty;
			try
		...
		public static void ImportMessage(HttpContext Context, ExchangeSession Session, IDbConnection con, string sMODULE_NAME, Guid gPARENT_ID, string sEXCHANGE_ALIAS, Guid gUSER_ID, EmailMessage email, StringBuilder sbErrors)
		{
			HttpApplicationState Application = Context.Application;
			bool   bVERBOSE_STATUS = Sql.ToBoolean(Application["CONFIG.Exchange.VerboseStatus"]);
			Guid   gTEAM_ID        = Sql.ToGuid(Session["TEAM_ID"]);
			...
			{
				string sMODULE_TABLE = Crm.Modules.TableName(Application, sMODULE_NAME);
					...
					if ( bLoadSuccessful )
					{
						...
						if ( email.ToRecipients != null && email.ToRecipients.Count > 0 )
						{
							foreach ( EmailAddress addr in email.ToRecipients )
							{
								...
								// 04/26/2018 Paul.  Exchange Sync needs to follow team hierarchy rules. 
								Guid gRECIPIENT_ID = ExchangeSync.RecipientByEmail(Application, Session, con, gUSER_ID, addr.Address);
