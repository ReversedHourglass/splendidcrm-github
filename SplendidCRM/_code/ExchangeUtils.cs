...
namespace SplendidCRM
{
	public class ExchangeUtils
	{
		...
		public static void SendTestMessage(HttpApplicationState Application, string sSERVER_URL, string sUSER_NAME, string sPASSWORD, string sFromAddress, string sFromName, string sToAddress, string sToName)
		{
			...
			SplendidMailExchangePassword client = new SplendidMailExchangePassword(Application, sSERVER_URL, sUSER_NAME, sPASSWORD);
		...
		public static int ValidateImpersonation(HttpApplicationState Application, string sEXCHANGE_ALIAS, string sEXCHANGE_EMAIL)
		{
			...
			ExchangeService service = ExchangeUtils.CreateExchangeService(Application, sEXCHANGE_ALIAS, sEXCHANGE_EMAIL, String.Empty, String.Empty, Guid.Empty);
		...
		// so we need to change the way that we manage users. 
		/*
		public static DataTable ExchangeUsers(HttpApplicationState Application, StringBuilder sbErrors)
		{
		...
		*/
		...
		public static ExchangeService CreateExchangeService(HttpApplicationState Application, string sEXCHANGE_ALIAS, string sEXCHANGE_EMAIL, string sMAIL_SMTPUSER, string sMAIL_SMTPPASS, Guid gEXCHANGE_ID)
		{
			...
			if ( !Sql.IsEmptyString(sOAUTH_CLIENT_ID) )
			{
				...
				Office365AccessToken token = ActiveDirectory.Office365RefreshAccessToken(Application, sOAuthDirectoryTenatID, sOAUTH_CLIENT_ID, sOAUTH_CLIENT_SECRET, gEXCHANGE_ID, false);
		...
		public static ExchangeService CreateExchangeService(ExchangeSync.UserSync User)
		{
			...
			return CreateExchangeService(User.Context.Application, User.EXCHANGE_ALIAS, User.EXCHANGE_EMAIL, User.MAIL_SMTPUSER, User.MAIL_SMTPPASS, gEXCHANGE_ID);
		}
		...
		public static DataTable GetMessage(ExchangeSync.UserSync User, string sUNIQUE_ID)
		{
			...
			if ( item.ItemClass == "IPM.Note" )
				dt = GetMessage(User.Context, service, sUNIQUE_ID);
		...
		public static DataTable GetMessage(HttpContext Context, ExchangeService service, string sUNIQUE_ID)
		{
			...
			using ( MemoryStream mem = new MemoryStream(message.MimeContent.Content) )
			{
				...
				if ( email != null )
				{
					MimeUtils.CreateMessageRecord(Context, dt, email, message.Size);
				}
			}
#else
		...
		{
			ExchangeService service = CreateExchangeService(Context.Application, String.Empty, String.Empty, String.Empty, String.Empty, gMAILBOX_ID);
		...
		{
			ExchangeService service = CreateExchangeService(Context.Application, String.Empty, String.Empty, sUSERNAME, sPASSWORD, gMAILBOX_ID);
								...
								foreach ( Attachment attach in email.Attachments )
								{
									...
									if ( attach is FileAttachment && bIsInline )
									{
										...
										if ( file.Content != null )
										{
											...
											Crm.EmailImages.LoadFile(gIMAGE_ID, file.Content, trn);
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( Sql.IsEmptyGuid(gEMAIL_ID) )
					{
						...
						if ( item.ItemClass == "IPM.Note" )
							gEMAIL_ID = ImportMessage(User.Context, Session, service, con, sPARENT_TYPE, gPARENT_ID, User.EXCHANGE_ALIAS, User.USER_ID, gASSIGNED_USER_ID, gTEAM_ID, sTEAM_SET_LIST, sREMOTE_KEY);
						else if ( item.ItemClass == "IPM.Post" )
							gEMAIL_ID = ImportPost(User.Context, Session, service, con, sPARENT_TYPE, gPARENT_ID, User.EXCHANGE_ALIAS, User.USER_ID, gASSIGNED_USER_ID, gTEAM_ID, sTEAM_SET_LIST, sREMOTE_KEY);
						else
		...
		public static Guid ImportInboundEmail(HttpContext Context, IDbConnection con, Guid gMAILBOX_ID, string sINTENT, Guid gGROUP_ID, Guid gGROUP_TEAM_ID, string sUNIQUE_ID, string sUNIQUE_MESSAGE_ID)
		{
			...
			if ( bLoadSuccessful )
			{
				try
				{
					...
					long   lUploadMaxSize = Sql.ToLong(Context.Application["CONFIG.upload_maxsize"]);
