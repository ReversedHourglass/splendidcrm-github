...
namespace SplendidCRM
{
	public class GoogleApps
	{
		...
		public static BaseClientService.Initializer GetUserCredentialInitializer(HttpApplicationState Application, Guid gUSER_ID, string sScope)
		{
			...
			Google.Apis.Auth.OAuth2.Responses.TokenResponse token = SplendidCRM.GoogleApps.RefreshAccessToken(Application, gUSER_ID, false);
			token.Scope = sScope;
		...
		public static bool TestAccessToken(HttpApplicationState Application, Guid gUSER_ID, StringBuilder sbErrors)
		{
			...
			try
			{
				Google.Apis.Services.BaseClientService.Initializer initializer = SplendidCRM.GoogleApps.GetUserCredentialInitializer(Application, gUSER_ID, GmailService.Scope.GmailReadonly);
		...
		{
			SplendidMailGmail client = new SplendidMailGmail(Application, gOAUTH_TOKEN_ID);
		...
		private static void UpdateFolderTreeNodeCounts(GmailService service, XmlNode xFolder)
		{
		...
		public static void UpdateFolderTreeNodeCounts(HttpApplicationState Application, Guid gUSER_ID, XmlNode xFolder)
		{
			Google.Apis.Services.BaseClientService.Initializer initializer = SplendidCRM.GoogleApps.GetUserCredentialInitializer(Application, gUSER_ID, GmailService.Scope.GmailReadonly);
			GmailService service = new GmailService(initializer);
		...
		private static void GetFolderTreeFromResults(GmailService service, XmlNode xParent, Label fld)
		{
		...
		public static XmlDocument GetFolderTree(HttpApplicationState Application, Guid gUSER_ID, ref string sGOOGLEAPPS_USERNAME, ref string sInboxFolderId)
		{
		...
		public static void GetFolderCount(HttpApplicationState Application, Guid gUSER_ID, string sFOLDER_ID, ref int nTotalCount, ref int nUnreadCount)
		{
			Google.Apis.Services.BaseClientService.Initializer initializer = SplendidCRM.GoogleApps.GetUserCredentialInitializer(Application, gUSER_ID, GmailService.Scope.GmailReadonly);
			GmailService service = new GmailService(initializer);
		...
		public static void DeleteMessage(HttpContext Context, Guid gUSER_ID, string sUNIQUE_ID)
		{
			DataTable dt = MimeUtils.CreateMessageTable();
			Google.Apis.Services.BaseClientService.Initializer initializer = SplendidCRM.GoogleApps.GetUserCredentialInitializer(Context.Application, gUSER_ID, GmailService.Scope.GmailReadonly);
			GmailService service = new GmailService(initializer);
			
			service.Users.Messages.Delete("me", sUNIQUE_ID).Execute();
		}

		public static DataTable GetMessage(HttpContext Context, Guid gUSER_ID, string sUNIQUE_ID)
		{
			DataTable dt = MimeUtils.CreateMessageTable();
			Google.Apis.Services.BaseClientService.Initializer initializer = SplendidCRM.GoogleApps.GetUserCredentialInitializer(Context.Application, gUSER_ID, GmailService.Scope.GmailReadonly);
			GmailService service = new GmailService(initializer);
			...
			using ( MemoryStream stm = new MemoryStream(by) )
			{
				...
				if ( email != null )
				{
					...
					DataRow row = MimeUtils.CreateMessageRecord(Context, dt, email, dSize);
		...
		public static byte[] GetAttachmentData(HttpContext Context, Guid gUSER_ID, string sUNIQUE_ID, int nATTACHMENT_ID, ref string sFILENAME, ref string sCONTENT_TYPE)
		{
		...
		public static DataTable GetFolderMessages(HttpContext Context, Guid gUSER_ID, string sFOLDER_ID, int nPageSize, int nPageOffset, string sSortColumn, string sSortOrder)
		{
			DataTable dt = MimeUtils.CreateMessageTable();
			Google.Apis.Services.BaseClientService.Initializer initializer = SplendidCRM.GoogleApps.GetUserCredentialInitializer(Context.Application, gUSER_ID, GmailService.Scope.GmailReadonly);
			GmailService service = new GmailService(initializer);
		...
		public static Guid ImportMessage(HttpContext Context, Guid gUSER_ID, string sPARENT_TYPE, Guid gPARENT_ID, Guid gASSIGNED_USER_ID, Guid gTEAM_ID, string sTEAM_SET_LIST, string sUNIQUE_ID)
		{
