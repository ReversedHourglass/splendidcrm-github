...
namespace SplendidCRM
{
	public class GoogleSync
	{
		...
		public class GoogleWebhook
		{
			...
			public GoogleWebhook(string sChannelID, string sChannelExpiration, string sResourceState, string sMessageNumber, string sResourceID, string sResourceURI, string sChannelToken)
			{
			...
			public static void AddWebhook(HttpContext Context, string sChannelId, string sChannelExpiration, string sResourceState, string sMessageNumber, string sResourceId, string sResourceURI, string sChannelToken)
			{
				try
				{
					...
					using ( IDbConnection con = dbf.CreateConnection() )
					{
						...
						if ( Sql.IsEmptyString(sRESOURCE_ID) )
						{
							...
							Google.Apis.Services.BaseClientService.Initializer initializer = SplendidCRM.GoogleApps.GetUserCredentialInitializer(Application, gUSER_ID, CalendarService.Scope.Calendar);
							CalendarService service = new CalendarService(initializer);
							StopChannel(Context, service, sChannelId, sResourceId, gUSER_ID);
							throw(new Exception("Could not find channel " + gCHANNEL_ID.ToString()));
						}
						else
						{
							GoogleSync.GoogleWebhook evt = new GoogleSync.GoogleWebhook(sChannelId, sChannelExpiration, sResourceState, sMessageNumber, sResourceId, sResourceURI, sChannelToken);
							lock ( queueWebhooks )
							{
								queueWebhooks.Enqueue(evt);
							}
						}
					}
				}
				catch(Exception ex)
				{
					...
					SyncError.SystemMessage(Context, "Error", new StackTrace(true).GetFrame(0), sMESSAGE);
					SplendidError.SystemMessage(Context, "Error", new StackTrace(true).GetFrame(0), sMESSAGE);
					throw(new Exception(sMESSAGE));
				}
			}

			// https://developers.google.com/google-apps/calendar/v3/push
			public static void CreateChannel(HttpContext Context, CalendarService service, Guid gUSER_ID, string sNAME, string sResourceId, string sCALENDAR_ID)
			{
				...
				try
				{
					...
					using ( IDbConnection con = dbf.CreateConnection() )
					{
						...
						if ( !Sql.IsEmptyGuid(gCHANNEL_ID) && dtTOKEN_EXPIRES_AT != DateTime.MinValue && DateTime.Now.AddHours(6) > dtTOKEN_EXPIRES_AT )
						{
							...
							GoogleWebhook.StopChannel(Context, service, gCHANNEL_ID.ToString(), sResourceId, gUSER_ID);
						...
						if ( Sql.IsEmptyGuid(gCHANNEL_ID) )
						{
							...
							//body.Address     = Crm.Config.SiteURL(Context.Application) + "GoogleOAuth/Google_Webhook.aspx";
							body.Address     = Sql.ToString(Context.Application["CONFIG.GoogleApps.PushNotificationURL"]);
							body.Type        = "web_hook";
			...
			public static void StopChannel(HttpContext Context, CalendarService service, string sChannelId, string sResourceId, Guid gUSER_ID)
			{
				try
				{
			...
			// https://developers.google.com/google-apps/calendar/v3/push
			public static void ProcessNotification(HttpContext Context, string sChannelId, string sChannelExpiration, string sResourceState, string sMessageNumber, string sResourceId, string sResourceURI, string sChannelToken)
			{
					...
					using ( IDbConnection con = dbf.CreateConnection() )
					{
						...
						if ( !Sql.IsEmptyString(sRESOURCE_ID) )
						{
							...
							if ( sResourceURI.StartsWith("https://www.googleapis.com/calendar/v3") )
							{
								...
								GoogleSync.ImportAppointment(Context, Session, service, con, gUSER_ID, appointment, sCALENDAR_ID, sbErrors);
							}
						}
						else
						{
			...
			public static void ProcessAllNotifications(HttpContext Context)
			{
				try
				{
					while ( queueWebhooks.Count > 0 )
					{
						...
						GoogleWebhook.ProcessNotification(Context, evt.ChannelID, evt.ChannelExpiration, evt.ResourceState, evt.MessageNumber, evt.ResourceID, evt.ResourceURI, evt.ChannelToken);
					}
				}
				catch(Exception ex)
				{
		...
		public static void SyncAllUsers(Object sender)
		{
			...
			bool bGoogleAppsEnabled = Sql.ToBoolean(Context.Application["CONFIG.GoogleApps.Enabled"]);
			if ( !GoogleSync.bInsideSyncAll && bGoogleAppsEnabled )
			{
				...
				try
				{
					...
					GoogleWebhook.ProcessAllNotifications(Context);
					using ( DataTable dt = new DataTable() )
					{
						...
						foreach ( DataRow row in dt.Rows )
						{
							...
							StringBuilder sbErrors = new StringBuilder();
							GoogleSync.UserSync User = new GoogleSync.UserSync(Context, bGOOGLEAPPS_SYNC_CONTACTS, bGOOGLEAPPS_SYNC_CALENDAR, gUSER_ID, false);
							Sync(User, sbErrors);
						}
					}
				}
		...
		public class UserSync
		{
			...
			public UserSync(HttpContext Context, bool bGOOGLEAPPS_SYNC_CONTACTS, bool bGOOGLEAPPS_SYNC_CALENDAR, Guid gUSER_ID, bool bSyncAll)
			{
			...
			public void Start()
			{
				...
				bool bVERBOSE_STATUS = Sql.ToBoolean(Context.Application["CONFIG.GoogleApps.VerboseStatus"]);
				if ( bVERBOSE_STATUS )
					SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "UserSync:  Begin " + USER_ID.ToString() + " at " + dtStart.ToString());
				StringBuilder sbErrors = new StringBuilder();
				...
				if ( bVERBOSE_STATUS )
					SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "UserSync:  End "   + USER_ID.ToString() + " at " + dtEnd.ToString() + ". Elapse time " + ts.Minutes.ToString() + " minutes " + ts.Seconds.ToString() + " seconds.");
			}
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						using ( IDataReader rdr = cmd.ExecuteReader(CommandBehavior.SingleRow) )
						{
							if ( rdr.Read() )
							{
								...
								User = new GoogleSync.UserSync(Context, bGOOGLEAPPS_SYNC_CONTACTS, bGOOGLEAPPS_SYNC_CALENDAR, gUSER_ID, bSyncAll);
							}
						}
					}
				}
		...
		public static void Sync(GoogleSync.UserSync User, StringBuilder sbErrors)
		{
			...
			if ( !arrProcessing.Contains(User.USER_ID) )
			{
				...
				try
				{
					...
					//Guid gINBOUND_EMAIL_KEY = Sql.ToGuid(User.Context.Application["CONFIG.InboundEmailKey"]);
					//Guid gINBOUND_EMAIL_IV  = Sql.ToGuid(User.Context.Application["CONFIG.InboundEmailIV" ]);
					...
					{
						Google.Apis.Services.BaseClientService.Initializer initializer = SplendidCRM.GoogleApps.GetUserCredentialInitializer(Application, User.USER_ID, ContactsService.Scope.Contacts);
						ContactsService service = new ContactsService(initializer);
						
						GoogleSync.SyncContacts(User.Context, Session, service, User.USER_ID, User.SyncAll, sbErrors);
					}
					if ( User.GOOGLEAPPS_SYNC_CALENDAR )
					{
						Google.Apis.Services.BaseClientService.Initializer initializer = SplendidCRM.GoogleApps.GetUserCredentialInitializer(Application, User.USER_ID, CalendarService.Scope.Calendar);
						CalendarService service = new CalendarService(initializer);
						GoogleSync.SyncAppointments(User.Context, Session, service, User.USER_ID, User.SyncAll, sbErrors);
					}
				}
				finally
				{
		...
		private static bool SetGoogleContact(Contact contact, DataRow row, StringBuilder sbChanges)
		{
			bool bChanged = false;
		...
		public static void SyncContacts(HttpContext Context, ExchangeSession Session, ContactsService service, Guid gUSER_ID, bool bSyncAll, StringBuilder sbErrors)
		{
			bool bVERBOSE_STATUS = Sql.ToBoolean(Context.Application["CONFIG.GoogleApps.VerboseStatus"]);
			if ( bVERBOSE_STATUS )
				SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncContacts: " + gUSER_ID.ToString());
					...
					do
					{
						...
						if ( contacts.Feed.Items != null && contacts.Feed.Items.Count > 0 )
						{
							if ( contacts.Feed.Items.Count > 0 )
								SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncContacts: " + contacts.Feed.Items.Count.ToString() + " contacts to retrieve from " + gUSER_ID.ToString());
							foreach ( Contact contact in contacts.Feed.Items )
							{
								...
								if ( bGroupFound )
									GoogleSync.ImportContact(Context, Session, service, con, gUSER_ID, sSplendidGroupURI, contact, sbErrors);
							}
						}
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								da.Fill(dt);
								if ( dt.Rows.Count > 0 )
									SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncContacts: " + dt.Rows.Count.ToString() + " contacts to send to " + gUSER_ID.ToString());
								foreach ( DataRow row in dt.Rows )
								{
									...
									try
									{
										...
										{
											if ( bVERBOSE_STATUS )
												SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncContacts: Sending new contact " + Sql.ToString(row["FIRST_NAME"]) + " " + Sql.ToString(row["LAST_NAME"]) + " to " + gUSER_ID.ToString());
											ContactEntry entry = new ContactEntry();
											GoogleSync.SetGoogleContact(entry.Entry, row, sbChanges);
										...
										{
											if ( bVERBOSE_STATUS )
												SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncContacts: Binding contact " + Sql.ToString(row["FIRST_NAME"]) + " " + Sql.ToString(row["LAST_NAME"]) + " to " + gUSER_ID.ToString());
											...
											{
												bool bChanged = GoogleSync.SetGoogleContact(contact, row, sbChanges);
												if ( bChanged )
												{
													if ( bVERBOSE_STATUS )
														SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncContacts: Sending contact " + Sql.ToString(row["FIRST_NAME"]) + " " + Sql.ToString(row["LAST_NAME"]) + " to " + gUSER_ID.ToString());
													ContactEntry entry = new ContactEntry();
													entry.Entry = contact;
										...
										{
											if ( bVERBOSE_STATUS )
												SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncContacts: Unsyncing contact " + Sql.ToString(row["FIRST_NAME"]) + " " + Sql.ToString(row["LAST_NAME"]) + " for " + gUSER_ID.ToString());
											using ( IDbTransaction trn = Sql.BeginTransaction(con) )
											{
												try
												{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								da.Fill(dt);
								if ( dt.Rows.Count > 0 )
									SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncContacts: " + dt.Rows.Count.ToString() + " contacts to remove from group " + gUSER_ID.ToString());
								foreach ( DataRow row in dt.Rows )
								{
									...
									{
										if ( bVERBOSE_STATUS )
											SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncContacts: Deleting sync contact " + Sql.ToString(row["FIRST_NAME"]) + " " + Sql.ToString(row["LAST_NAME"]) + " from " + gUSER_ID.ToString());
										using ( IDbTransaction trn = Sql.BeginTransaction(con) )
										{
											try
											{
		...
		public static bool BuildCONTACTS_Update(HttpApplicationState Application, ExchangeSession Session, IDbCommand spCONTACTS_Update, DataRow row, Contact contact, Guid gUSER_ID, Guid gTEAM_ID, Guid gASSIGNED_USER_ID, bool bCreateAccount, IDbTransaction trn)
		{
			bool bChanged = false;
		...
		public static void ImportContact(HttpContext Context, ExchangeSession Session, ContactsService service, IDbConnection con, Guid gUSER_ID, string sSplendidGroupURI, Contact contact, StringBuilder sbErrors)
		{
			HttpApplicationState Application = Context.Application;
					...
					using ( DataTable dt = new DataTable() )
					{
						...
						if ( sSYNC_ACTION == "remote new" || sSYNC_ACTION == "remote changed" )
						{
							using ( IDbTransaction trn = Sql.BeginTransaction(con) )
							{
								...
								{
									if ( bVERBOSE_STATUS )
										SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "ImportContact: Retrieving contact " + sContactName + " from " + gUSER_ID.ToString());
						...
						else if ( (sSYNC_ACTION == "local changed" || sSYNC_ACTION == "local new") && !Sql.IsEmptyGuid(gID) )
						{
							...
							if ( dt.Rows.Count > 0 )
							{
								...
								{
									if ( bVERBOSE_STATUS )
										SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "ImportContact: Syncing contact " + Sql.ToString(row["FIRST_NAME"]) + " " + Sql.ToString(row["LAST_NAME"]) + " to " + gUSER_ID.ToString());
									bool bChanged = GoogleSync.SetGoogleContact(contact, row, sbChanges);
									if ( bChanged )
									{
										ContactEntry entry = new ContactEntry();
						...
						else if ( sSYNC_ACTION == "local deleted" )
						{
							...
							{
								if ( bVERBOSE_STATUS )
									SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "ImportContact: Deleting " + sContactName + " from " + gUSER_ID.ToString());
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									try
									{
						...
						{
							if ( bVERBOSE_STATUS )
								SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "ImportContact: Unsyncing contact " + sContactName + " for " + gUSER_ID.ToString());
							using ( IDbTransaction trn = Sql.BeginTransaction(con) )
							{
								try
								{
		...
		private static bool SetGoogleAppointment(HttpContext Context, Event appointment, DataRow row, DataTable dtAppointmentEmails, StringBuilder sbChanges, bool bDISABLE_PARTICIPANTS, Guid gUSER_ID)
		{
			bool bChanged = false;
			// 09/14/2015 Paul. Google requires the timezone. 
			string sTIME_ZONE = "America/New_York";
			Guid gTIMEZONE = Sql.ToGuid(Context.Application["CONFIG.default_timezone"]);
			TimeZone T10z = Context.Application["TIMEZONE." + gTIMEZONE.ToString()] as SplendidCRM.TimeZone;
			...
			else
			{
				...
				//GoogleUitls.FixAppointmentTimes(Context, appointment);
		...
		public static void SyncAppointments(HttpContext Context, ExchangeSession Session, CalendarService service, Guid gUSER_ID, bool bSyncAll, StringBuilder sbErrors)
		{
			HttpApplicationState Application = Context.Application;
			// 03/23/2013 Paul.  Add the ability to disable participants. 
			...
			bool bVERBOSE_STATUS       = Sql.ToBoolean(Application["CONFIG.GoogleApps.VerboseStatus"      ]);
			if ( bVERBOSE_STATUS )
				SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncAppointments: " + gUSER_ID.ToString());
			// 02/20/2013 Paul.  Reduced the number of days to go back. 
			int  nAPPOINTMENT_AGE_DAYS = Sql.ToInteger(Context.Application["CONFIG.GoogleApps.AppointmentAgeDays"]);
			...
			try
			{
				...
				bool   bPushNotifications   = Sql.ToBoolean(Context.Application["CONFIG.GoogleApps.PushNotifications"  ]);
				string sPushNotificationURL = Sql.ToString (Context.Application["CONFIG.GoogleApps.PushNotificationURL"]);
					...
					do
					{
						...
						if ( events.Items != null && events.Items.Count > 0 )
						{
							if ( events.Items.Count > 0 )
								SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncAppointments: " + events.Items.Count.ToString() + " appointments to retrieve from " + gUSER_ID.ToString());
							foreach ( Event appointment in events.Items )
							{
								...
								{
									GoogleWebhook.CreateChannel(Context, service, gUSER_ID, "Google.Channel", appointment.Id, sCALENDAR_ID);
								}
							}
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								da.Fill(dt);
								if ( dt.Rows.Count > 0 )
									SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncAppointments: " + dt.Rows.Count.ToString() + " appointments to send to " + gUSER_ID.ToString());
								foreach ( DataRow row in dt.Rows )
								{
									Guid     gID                             = Sql.ToGuid    (row["ID"                           ]);
									Guid     gASSIGNED_USER_ID               = Sql.ToGuid    (row["ASSIGNED_USER_ID"             ]);
									...
									try
									{
										...
										{
											if ( bVERBOSE_STATUS )
												SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncAppointments: Sending new appointment " + Sql.ToString(row["NAME"]) + " " + Sql.ToString(row["DATE_START"]) + " to " + gUSER_ID.ToString());
											appointment = new Event();
											DataTable dtAppointmentEmails = AppointmentEmails(Application, con, Sql.ToGuid(row["ID"]));
											// 03/23/2013 Paul.  Add the ability to disable participants. 
											GoogleSync.SetGoogleAppointment(Context, appointment, row, dtAppointmentEmails, sbChanges, bDISABLE_PARTICIPANTS, gUSER_ID);
										...
										{
											if ( bVERBOSE_STATUS )
												SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncAppointments: Binding appointment " + Sql.ToString(row["NAME"]) + " " + Sql.ToString(row["DATE_START"]) + " to " + gUSER_ID.ToString());
											...
											if ( sSYNC_ACTION == "local changed" )
											{
												...
												DataTable dtAppointmentEmails = AppointmentEmails(Application, con, Sql.ToGuid(row["ID"]));
												// 03/23/2013 Paul.  Add the ability to disable participants. 
												bool bChanged = GoogleSync.SetGoogleAppointment(Context, appointment, row, dtAppointmentEmails, sbChanges, bDISABLE_PARTICIPANTS, gUSER_ID);
												if ( bChanged )
												{
													if ( bVERBOSE_STATUS )
														SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncAppointments: Sending appointment " + Sql.ToString(row["NAME"]) + " " + Sql.ToString(row["DATE_START"]) + " to " + gUSER_ID.ToString());
										...
										{
											if ( bVERBOSE_STATUS )
												SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncAppointments: Unsyncing appointment " + Sql.ToString(row["NAME"]) + " " + Sql.ToString(row["DATE_START"]) + " for " + gUSER_ID.ToString());
											using ( IDbTransaction trn = Sql.BeginTransaction(con) )
											{
												try
												{
		...
		// 01/24/2012 Paul.  Only update the record if it has changed.  This is to prevent an endless loop caused by sync operations. 
		public static bool BuildAPPOINTMENTS_Update(HttpApplicationState Application, ExchangeSession Session, IDbCommand spAPPOINTMENTS_Update, DataRow row, Event appointment, Guid gUSER_ID, Guid gTEAM_ID, Guid gASSIGNED_USER_ID)
		{
			bool bChanged = false;
			// 03/25/2010 Paul.  We start with the existing record values so that we can apply ACL Field Security rules. 
			if ( row != null && row.Table != null )
		...
		public static void ImportAppointment(HttpContext Context, ExchangeSession Session, CalendarService service, IDbConnection con, Guid gUSER_ID, Event appointment, string sCALENDAR_ID, StringBuilder sbErrors)
		{
			HttpApplicationState Application = Context.Application;
					...
					using ( DataTable dt = new DataTable() )
					{
						...
						if ( sSYNC_ACTION == "remote new" || sSYNC_ACTION == "remote changed" )
						{
							...
							using ( IDbTransaction trn = Sql.BeginTransaction(con) )
							{
								...
								{
									if ( bVERBOSE_STATUS )
										SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncAppointments: Retrieving appointment " + Sql.ToString(appointment.Summary) + " " + Sql.ToString(dtDATE_START) + " from " + gUSER_ID.ToString());
						...
						else if ( (sSYNC_ACTION == "local changed" || sSYNC_ACTION == "local new") && !Sql.IsEmptyGuid(gID) )
						{
							...
							if ( dt.Rows.Count > 0 )
							{
								...
								{
									if ( bVERBOSE_STATUS )
										SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncAppointments: Syncing appointment " + Sql.ToString(row["NAME"]) + " " + Sql.ToString(row["DATE_START"]) + " to " + gUSER_ID.ToString());
									// 03/28/2010 Paul.  The EWS Managed API requires one field to change in order to update the record. 
									// 07/18/2010 Paul.  Move Google Sync functions to a separate class. 
									DataTable dtAppointmentEmails = AppointmentEmails(Application, con, Sql.ToGuid(row["ID"]));
									// 03/23/2013 Paul.  Add the ability to disable participants. 
						...
						else if ( sSYNC_ACTION == "local deleted" )
						{
							...
							{
								if ( bVERBOSE_STATUS )
									SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncAppointments: Deleting " + Sql.ToString(appointment.Summary) + " " + Sql.ToString(dtDATE_START) + " from " + gUSER_ID.ToString());
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									try
									{
						...
						else if ( sSYNC_ACTION == "remote deleted" && !Sql.IsEmptyGuid(gID) )
						{
							if ( bVERBOSE_STATUS )
							{
								if ( appointment.OriginalStartTime.DateTime.HasValue )
									SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncAppointments: Unsyncing appointment " + Sql.ToString(appointment.OriginalStartTime.DateTime.Value) + " for " + gUSER_ID.ToString());
								else
									SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncAppointments: Unsyncing appointment " + Sql.ToString(appointment.OriginalStartTime.Date) + " for " + gUSER_ID.ToString());
							}
							using ( IDbTransaction trn = Sql.BeginTransaction(con) )
							{
								try
