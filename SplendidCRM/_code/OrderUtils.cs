...
namespace SplendidCRM
{
	public class OrderUtils
	{
		...
		public static byte[] CreateSignatureImage(Signature_2DLineGraphic lineGraphic)
		{
			byte[] png = null;
		...
		public static Guid GetSignatureReportID(string sDETAIL_NAME)
		{
		...
		public static void CreateSignedPDF(HttpContext Context, L10N L10n, TimeZone T10n, string sModuleName, Guid gID, string sSignature, out Guid gIMAGE_ID, out Guid gNOTE_ID, out Guid gNOTE_ATTACHMENT_ID, out Guid gREPORT_ID)
		{
			OrderUtils.Signature_2DLineGraphic lineGraphic = new JavaScriptSerializer().Deserialize<OrderUtils.Signature_2DLineGraphic>(sSignature);
			if ( lineGraphic.lines.Count == 0 )
				throw(new Exception(L10n.Term("Orders.ERR_SIGNATURE_NOT_PROVIDED")));

			byte[] bySignature       = OrderUtils.CreateSignatureImage(lineGraphic);
			string sTABLE_NAME       = Crm.Modules.TableName(Context.Application, sModuleName);
			...
			string sNOTE_NAME        = Crm.Modules.ItemName(Context.Application, sModuleName, gID) + " " + L10n.Term("Orders.LBL_SIGNED"   ) + " " + DateTime.Now.ToString();
			string sIMAGE_NAME       = Crm.Modules.ItemName(Context.Application, sModuleName, gID) + " " + L10n.Term("Orders.LBL_SIGNATURE") + " " + DateTime.Now.ToString();
			sIMAGE_NAME              = sIMAGE_NAME.Replace('\\', '_').Replace(':' , '_');
			gIMAGE_ID                = Guid.Empty;
			gNOTE_ID                 = Guid.Empty;
			gNOTE_ATTACHMENT_ID      = Guid.Empty;
			gREPORT_ID               = Sql.ToGuid(lineGraphic.REPORT_ID);
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				if ( dtReport.Rows.Count > 0 )
				{
					...
					string sFILE_EXT      ;
					string sPrimaryKey = Crm.Modules.SingularTableName(Crm.Modules.TableName(sModuleName)) + "_ID";
					...
					byte[] byContent = SplendidCRM.Reports.AttachmentView.Render(Context, dictParameters, L10n, T10n, gREPORT_ID, sRDL, "PDF", sModuleName, Guid.Empty, out sFILE_MIME_TYPE, out sFILE_EXT);
