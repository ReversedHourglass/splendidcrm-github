...
namespace SplendidCRM
{
	...
	public class PayPalCache
	{
		...
		public static string PayPalAPIUsername(HttpApplicationState Application)
		{
			return Sql.ToString(Application["CONFIG.PayPal.APIUsername"]);
		}

		public static string PayPalAPIPassword(HttpApplicationState Application)
		{
			return Sql.ToString(Application["CONFIG.PayPal.APIPassword"]);
		}

		public static string PayPalX509PrivateKey(HttpApplicationState Application)
		{
			return Sql.ToString(Application["CONFIG.PayPal.X509PrivateKey"]);
		}

		public static string PayPalX509Certificate(HttpApplicationState Application)
		{
			return Sql.ToString(Application["CONFIG.PayPal.X509Certificate"]);
		}

		// 08/16/2015 Paul.  Add CARD_TOKEN for use with PayPal REST API. 
		public static string PayPalClientID(HttpApplicationState Application)
		{
			return Sql.ToString(Application["CONFIG.PayPal.ClientID"]);
		}

		public static string PayPalClientSecret(HttpApplicationState Application)
		{
			return Sql.ToString(Application["CONFIG.PayPal.ClientSecret"]);
		}

		public static string PayPalWebServiceURL(HttpApplicationState Application)
		{
		...
		public static string PayPalAuthorizationURL(HttpApplicationState Application)
		{
			if ( Sql.ToBoolean(Application["CONFIG.PayPal.Sandbox"]) )
		...
		// 11/15/2009 Paul.  We need a version of the certificate function that accepts the application. 
		public static PayPalAPI CreatePayPalAPI(HttpApplicationState Application)
		{
			string sPayPalAPIUsername     = PayPalAPIUsername    (Application);
			string sPayPalAPIPassword     = PayPalAPIPassword    (Application);
			string sPayPalX509PrivateKey  = PayPalX509PrivateKey (Application);
			string sPayPalX509Certificate = PayPalX509Certificate(Application);
			string sPayPalWebServiceURL   = PayPalWebServiceURL  (Application);
			PayPalAPI api = null;
			try
			{
				// 11/15/2009 Paul.  It is better to get the cache from the Runtime object instead of the context. 
				System.Web.Caching.Cache Cache = HttpRuntime.Cache;
				byte[] byPkcs12 = Cache.Get("PayPal.PKCS12") as byte[];
				if ( byPkcs12 == null )
				{
					byPkcs12 = PayPalUtils.MakeCertPKCS12(sPayPalX509PrivateKey, sPayPalX509Certificate, String.Empty);
					Cache.Insert("PayPal.PKCS12", byPkcs12, null, DateTime.Now.AddMinutes(5), System.Web.Caching.Cache.NoSlidingExpiration);
				}
				api = new PayPalAPI(sPayPalAPIUsername, sPayPalAPIPassword, byPkcs12, String.Empty, sPayPalWebServiceURL);
			}
			catch(Exception ex)
			{
				throw (new Exception("Invalid PayPal X.509 Certificate Private Key", ex));
			}
			return api;
		}

		public static void ClearTransaction(string sTransactionID)
		{
			// 11/15/2009 Paul.  It is better to get the cache from the Runtime object instead of the context. 
			HttpRuntime.Cache.Remove("PayPal.Transaction." + sTransactionID);
		}

		public static DataSet Transaction(HttpApplicationState Application, string sTransactionID)
		{
			// 11/15/2009 Paul.  It is better to get the cache from the Runtime object instead of the context. 
			System.Web.Caching.Cache Cache = HttpRuntime.Cache;
			...
			if ( ds == null )
			{
				...
				Cache.Insert("PayPal.Transaction." + sTransactionID, ds, null, PayPalCache.DefaultCacheExpiration(), System.Web.Caching.Cache.NoSlidingExpiration);
			}
			return ds;
		}

		public static StringDictionary Countries()
		{
			// 11/15/2009 Paul.  It is better to get the cache from the Runtime object instead of the context. 
			System.Web.Caching.Cache Cache = HttpRuntime.Cache;
			...
			if ( dict == null )
			{
				...
				Cache.Insert("PayPal.Contries", dict, null, DateTime.Now.AddYears(1), System.Web.Caching.Cache.NoSlidingExpiration);
			}
			return dict;
		}

		// 09/13/2013 Paul.  PayTrace requires 2-character state codes. 
		public static StringDictionary States()
		{
			System.Web.Caching.Cache Cache = HttpRuntime.Cache;
			...
			if ( dict == null )
			{
				...
				// 09/13/2013 Paul.  Keep this list in memory for a long time. 
				Cache.Insert("PayPal.States", dict, null, DateTime.Now.AddYears(1), System.Web.Caching.Cache.NoSlidingExpiration);
			}
			return dict;
		}
	}
