...
namespace SplendidCRM
{
	...
	public class PayPalRest
	{
		...
		{
			// 07/31/2018 Paul.  As of June 30, 2018, the PayPal system requires TLS 1.2. 
			...
			if ( !Sql.IsEmptyString(sCARD_TOKEN) )
			{
				...
				catch(Exception ex)
				{
					...
					SplendidError.SystemMessage(Application, "Error", new StackTrace(true).GetFrame(0), "Delete Stored Card " + sCARD_TOKEN + ": " + ex.Message);
				}
		...
		public static string Charge(HttpApplicationState Application, Guid gCURRENCY_ID, Guid gCONTACT_ID, Guid gINVOICE_ID, Guid gPAYMENT_ID, Guid gCREDIT_CARD_ID)
		{
			// 07/31/2018 Paul.  As of June 30, 2018, the PayPal system requires TLS 1.2. 
			// https://stackoverflow.com/questions/28286086/default-securityprotocol-in-net-4-5
			// https://github.com/paypal/PayPal-NET-SDK/issues/142
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					using ( IDataReader rdr = cmd.ExecuteReader(CommandBehavior.SingleRow) )
					{
						if ( rdr.Read() )
						{
							...
							if ( C10n.ISO4217 == SplendidDefaults.BaseCurrencyISO(Application) )
							{
					...
					foreach ( Transaction transaction in payment.transactions )
					{
						foreach ( RelatedResources resource in transaction.related_resources )
						{
							if ( resource.sale != null )
							{
								...
								Refund result = resource.sale.Refund(apiContext, refund);
		...
		public static DataSet PaymentSearch(HttpApplicationState Application, DateTime dtStartDate, DateTime dtEndDate, string sEmail)
		{
			// 07/31/2018 Paul.  As of June 30, 2018, the PayPal system requires TLS 1.2. 
			// https://stackoverflow.com/questions/28286086/default-securityprotocol-in-net-4-5
			// https://github.com/paypal/PayPal-NET-SDK/issues/142
		...
		public static DataSet PaymentDetails(HttpApplicationState Application, string sPaymentId)
		{
			// 07/31/2018 Paul.  As of June 30, 2018, the PayPal system requires TLS 1.2. 
			// https://stackoverflow.com/questions/28286086/default-securityprotocol-in-net-4-5
			// https://github.com/paypal/PayPal-NET-SDK/issues/142
