...
namespace SplendidCRM
{
	...
	{
		private static string CleanseData(string sValue)
		{
		...
		private static string PayTraceRequest(Dictionary<string, string> dictParams, string sLineItems)
		{
		...
		public static string ValidateLogin(HttpApplicationState Application, string sUserName, string sPassword)
		{
...
//#endif
		...
		public static string Charge(HttpApplicationState Application, Guid gCURRENCY_ID, Guid gINVOICE_ID, Guid gPAYMENT_ID, Guid gCREDIT_CARD_ID, string sClientIP, string sDESCRIPTION)
		{
			Guid gCREDIT_CARD_KEY = Sql.ToGuid(Application["CONFIG.CreditCardKey"]);
			Guid gCREDIT_CARD_IV  = Sql.ToGuid(Application["CONFIG.CreditCardIV" ]);
			StringDictionary dictCountries = PayPalCache.Countries();
...
//#endif
				...
				try
				{
					if ( CARD_TYPE == "Visa" || CARD_TYPE == "MasterCard" )
					{
						...
						{
							sErrorMessage = dictResponse["ERROR"];
							SplendidError.SystemMessage(Application, "Warning", new StackTrace(true).GetFrame(0), "Failed to add PayTrace Level 3 data: " + sErrorMessage);
						}
						// http://help.paytrace.com/api-response-codes
						if ( dictResponse.ContainsKey("RESPONSE") )
						{
							...
							if ( !(sRESPONSE.StartsWith("170.") || sRESPONSE.StartsWith("171.")) )
								SplendidError.SystemMessage(Application, "Warning", new StackTrace(true).GetFrame(0), "Failed to add PayTrace Level 3 data: " + sRESPONSE);
//#if DEBUG
//							Debug.WriteLine(sRESPONSE);
//#endif
						}
				...
				{
					SplendidError.SystemMessage(Application, "Warning", new StackTrace(true).GetFrame(0), "Failed to add PayTrace Level 3 data: " + ex.Message);
				}
			}
			return sSTATUS;
		}

		public static string Refund(HttpApplicationState Application, string sTRANSACTION_NUMBER)
		{
			Guid gCREDIT_CARD_KEY = Sql.ToGuid(Application["CONFIG.CreditCardKey"]);
			Guid gCREDIT_CARD_IV  = Sql.ToGuid(Application["CONFIG.CreditCardIV" ]);
...
//#endif
		...
		public static string Refund(HttpApplicationState Application, Guid gPAYMENT_ID, Guid gCURRENCY_ID, Guid gACCOUNT_ID, Guid gCREDIT_CARD_ID, string sINVOICE_NUMBER, Decimal dAMOUNT, string sTRANSACTION_NUMBER)
		{
			Guid gCREDIT_CARD_KEY = Sql.ToGuid(Application["CONFIG.CreditCardKey"]);
			Guid gCREDIT_CARD_IV  = Sql.ToGuid(Application["CONFIG.CreditCardIV" ]);
...
//#endif
		...
		public static DataSet Transactions(HttpApplicationState Application, DateTime dtStartDate, DateTime dtEndDate, string sTransactionType, string sSearchText, string sTransactionID)
		{
			Guid gCREDIT_CARD_KEY = Sql.ToGuid(Application["CONFIG.CreditCardKey"]);
			Guid gCREDIT_CARD_IV  = Sql.ToGuid(Application["CONFIG.CreditCardIV" ]);
...
//#endif
		...
		public static DataTable CustomerProfiles(HttpApplicationState Application, string sCustomerID)
		{
			Guid gCREDIT_CARD_KEY = Sql.ToGuid(Application["CONFIG.CreditCardKey"]);
			Guid gCREDIT_CARD_IV  = Sql.ToGuid(Application["CONFIG.CreditCardIV" ]);
...
//#endif
		...
		public static DataSet Transaction(HttpApplicationState Application, string sTransactionID)
		{
			...
			if ( ds == null )
			{
				ds = PayTraceUtils.Transactions(Application, DateTime.MinValue, DateTime.MinValue, String.Empty, String.Empty, sTransactionID);
				...
				if ( dt.Rows.Count > 0 )
				{
					...
					{
						//DataTable dtCustomers = CustomerProfiles(Application, sCustomerID);
						//ds.Tables.Add(dtCustomers);
					}
				}
				Cache.Insert("PayTrace.Transaction." + sTransactionID, ds, null, DateTime.Now.AddMinutes(1), System.Web.Caching.Cache.NoSlidingExpiration);
			}
			return ds;
		}
