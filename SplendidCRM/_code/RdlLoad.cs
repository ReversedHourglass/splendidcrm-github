...
namespace SplendidCRM
{
	...
	public partial class RdlUtil
	{
		...
		public static void LocalLoadReportDefinition(HttpContext Context, Dictionary<string, string> dictParameters, SplendidControl ctlParameterView, L10N L10n, TimeZone T10n, ReportViewer rdlViewer, Guid gREPORT_ID, string sRDL, string sMODULE_NAME, Guid gSCHEDULED_USER_ID, out string sReportSQL)
		{
			DataSet ds = new DataSet();
			// 03/24/2016 Paul.  We need an alternate way to provide parameters to render a report with a signature. 
			RdlDocument rdl= LocalLoadReportDefinition(Context, dictParameters, ctlParameterView, L10n, T10n, ds, sRDL, sMODULE_NAME, gSCHEDULED_USER_ID, out sReportSQL, false, null);

			// 10/06/2012 Paul.  Capture and log errors. 
			rdlViewer.ReportError += new ReportErrorEventHandler(rdlViewer_ReportError);
			...
			*/
			...
			foreach ( XmlNode xSubreport in nlSubreports )
			{
				...
				if ( !Sql.IsEmptyGuid(gSUBREPORT_ID) )
				{
					...
					Context.Application[gREPORT_ID.ToString() + ".Subreport." + sSubReportName] = gSUBREPORT_ID;
					...
					if ( dtReport != null && dtReport.Rows.Count > 0 )
					{
						...
						// 03/24/2016 Paul.  We need an alternate way to provide parameters to render a report with a signature. 
						RdlDocument rdlSubreport = LocalLoadReportDefinition(Context, null, null, L10n, T10n, dsSubreport, sSUBREPORT_RDL, sSUBREPORT_MODULE_NAME, gSCHEDULED_USER_ID, out sSubReportSQL, false, null);
			...
			if ( nlSubreports.Count > 0 )
			{
				...
				rdlViewer.LocalReport.SubreportProcessing += delegate(object sender, SubreportProcessingEventArgs e)
				{
					string sSubReportName = e.ReportPath;
					if ( sSubReportName.StartsWith("/") )
						sSubReportName = sSubReportName.Substring(1);
					
					Guid gSUBREPORT_ID = Sql.ToGuid(Context.Application[gREPORT_ID.ToString() + ".Subreport." + sSubReportName]);
					if ( !Sql.IsEmptyGuid(gSUBREPORT_ID) )
					{
						//e.DataSourceNames;
						e.DataSources.Clear();
						...
						if ( dtReport != null && dtReport.Rows.Count > 0 )
						{
							...
							string sSubReportSQL = String.Empty;
							// 03/24/2016 Paul.  We need an alternate way to provide parameters to render a report with a signature. 
							RdlDocument rdlSubreport = LocalLoadReportDefinition(Context, null, null, L10n, T10n, dsSubreport, sSUBREPORT_RDL, sSUBREPORT_MODULE_NAME, gSCHEDULED_USER_ID, out sSubReportSQL, false, e);
		...
		// 10/06/2012 Paul.  Capture and log errors. 
		private static void rdlViewer_ReportError(object sender, ReportErrorEventArgs e)
		{
			try
			{
#if DEBUG
		...
		// 03/24/2016 Paul.  We need an alternate way to provide parameters to render a report with a signature. 
		public static RdlDocument LocalLoadReportDefinition(HttpContext Context, Dictionary<string, string> dictParameters, SplendidControl ctlParameterView, L10N L10n, TimeZone T10n, DataSet ds, string sRDL, string sMODULE_NAME, Guid gSCHEDULED_USER_ID, out string sReportSQL, bool bChart, SubreportProcessingEventArgs eSubreportArgs)
		{
			HttpApplicationState Application = Context.Application;
			HttpRequest          Request     = Context.Request;
			ExchangeSession      Session     = null;
			sReportSQL = String.Empty;
			...
			{
				Session = ExchangeSecurity.LoadUserACL(Application, gSCHEDULED_USER_ID);
			}
			...
			foreach ( XmlNode xReportParameter in nlReportParameters )
			{
				...
				else if ( Request != null )
				{
					...
					try
					{
						...
						string[] arrVALUES = Request.QueryString.GetValues(sName);
						if ( arrVALUES == null )
							arrVALUES = Request.Form.GetValues(sName);
						if ( arrVALUES != null )
						{
				...
				if ( sValue.StartsWith("=") )
				{
					...
					RuleValidation validation = new RuleValidation(typeof(ReportParameterThis), typeProvider);
					...
					ReportParameterThis value = new ReportParameterThis("");
			...
			foreach ( XmlNode xHyperLinks in nlHyperLinks )
			{
				...
				{
					xHyperLinks.InnerText = sInnerText.Replace("~/", Request.Url.Scheme + "://" + Request.Url.Host + Sql.ToString(Application["rootURL"]));
				}
			}
						...
						using ( IDbTransaction trn = Sql.BeginTransaction(con) )
						{
							try
							{
								using ( IDbCommand cmd = con.CreateCommand() )
								{
									...
									if ( bChart )
										rdl.BuildChartCommand(Context, xDataSet, cmd);
									else
										rdl.BuildCommand(Context, xDataSet, cmd);
