...
namespace Spring.Social.ConstantContact
{
	public class ConstantContactSync
	{
		...
		public static bool ConstantContactEnabled(HttpApplicationState Application)
		{
		...
		public static Guid ConstantContactUserID(HttpApplicationState Application)
		{
		...
		public static Spring.Social.ConstantContact.Api.IConstantContact CreateApi(HttpApplicationState Application)
		{
		...
		public static bool ValidateConstantContact(HttpApplicationState Application, string sOAuthPortalID, string sOAuthClientID, string sOAuthClientSecret, string sOAuthAccessToken, StringBuilder sbErrors)
		{
		...
		public static bool ValidateConstantContact(HttpApplicationState Application, StringBuilder sbErrors)
		{
			bool bValidSource = false;
			try
			{
				Spring.Social.ConstantContact.Api.IConstantContact constantContact = CreateApi(Application);
		...
		public static void Sync(HttpContext Context, bool bSyncAll)
		{
			...
			bool bConstantContactEnabled = ConstantContactEnabled(Application);
			if ( !bInsideSyncAll && bConstantContactEnabled )
			{
				...
				try
				{
					...
					Guid gConstantContact_USER_ID = ConstantContactSync.ConstantContactUserID(Context.Application);
					ConstantContact.UserSync User = new ConstantContact.UserSync(Context, gConstantContact_USER_ID, bSyncAll);
					Sync(User, sbErrors);
				}
				catch(Exception ex)
				{
		...
		public static void Sync(ConstantContact.UserSync User, StringBuilder sbErrors)
		{
			...
			if ( bVERBOSE_STATUS )
				SyncError.SystemMessage(User.Context, "Warning", new StackTrace(true).GetFrame(0), "ConstantContactSync.Sync Start.");
			
			Spring.Social.ConstantContact.Api.IConstantContact constantContact = ConstantContactSync.CreateApi(Application);
			if ( constantContact != null )
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					try
					{
						...
						ConstantContact.ProspectList prospectList = new ConstantContact.ProspectList(constantContact);
						if ( !bInsideContacts )
						{
							try
							{
						...
						{
							ConstantContact.Contact contact = new ConstantContact.Contact(constantContact);
							if ( !bInsideContacts )
							{
								try
								{
						...
						{
							ConstantContact.Lead lead = new ConstantContact.Lead(constantContact);
							if ( !bInsideContacts )
							{
								try
								{
					...
					{
						if ( bVERBOSE_STATUS )
							SyncError.SystemMessage(User.Context, "Warning", new StackTrace(true).GetFrame(0), "ConstantContactSync.Sync Done.");
					}
				}
			}
		}

		public static void Sync(SplendidCRM.DbProviderFactory dbf, IDbConnection con, ConstantContact.UserSync User, ConstantContact.HObject qo, StringBuilder sbErrors)
		{
			...
			if ( bVERBOSE_STATUS )
				SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.ConstantContactTableName + ".Sync: " + sDIRECTION);
			try
			{
				...
				if ( bVERBOSE_STATUS )
					SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.ConstantContactTableName + ".Sync: Query took " + (DateTime.Now - dtStartSelect).Seconds.ToString() + " seconds. Using last modified " + dtStartModifiedDate.ToString());
				if ( lst.Count > 0 )
					SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.ConstantContactTableName + ".Sync: " + lst.Count.ToString() + " " + qo.ConstantContactTableName + " to import.");
					...
					using ( DbDataAdapter da = dbf.CreateDataAdapter() )
					{
						...
						using ( DataTable dt = new DataTable() )
						{
							...
							if ( dt.Rows.Count > 0 && !qo.IsReadOnly )
								SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.ConstantContactTableName + ".Sync: " + dt.Rows.Count.ToString() + " " + qo.CRMModuleName + " to send.");
							...
							for ( int i = 0; i < dt.Rows.Count && !qo.IsReadOnly; i++ )
							{
								...
								try
								{
									if ( sSYNC_ACTION == "local new" || Sql.IsEmptyString(sSYNC_REMOTE_KEY) )
									{
										...
										{
											if ( bVERBOSE_STATUS )
												SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.ConstantContactTableName + ".Sync: Sending new " + qo.ConstantContactTableName + " (" + i.ToString() + ") " + Sql.ToString(row["NAME"]) + ".");
									...
									{
										if ( bVERBOSE_STATUS )
											SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.ConstantContactTableName + ".Sync: Binding " + qo.ConstantContactTableName + " (" + i.ToString() + ") " + Sql.ToString(row["NAME"]) + ".");
										...
										if ( sSYNC_ACTION == "local changed" )
										{
											...
											if (sDIRECTION == "bi-directional" || sDIRECTION == "from crm only" )
											{
												...
												if ( bVERBOSE_STATUS )
													SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.ConstantContactTableName + ".Sync: Sending " + qo.ConstantContactTableName + " (" + i.ToString() + ") " + Sql.ToString(row["NAME"]) + ". " + sbChanges.ToString());
												qo.Update();
											}
										}
									}
									...
									else if ( sSYNC_ACTION == "remote deleted" || sSYNC_ACTION == "remote unsync" )
									{
										...
										{
											if ( bVERBOSE_STATUS )
												SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.ConstantContactTableName + ".Sync: Deleting " + qo.CRMTableName + " " + Sql.ToString(row["NAME"]) + ".");
											using ( IDbTransaction trn = Sql.BeginTransaction(con) )
											{
												try
												{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								da.Fill(dt);
								if ( dt.Rows.Count > 0 )
									SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.ConstantContactTableName + ".Sync: " + dt.Rows.Count.ToString() + " " + qo.CRMModuleName + " to delete.");
								for ( int i = 0; i < dt.Rows.Count; i++ )
								{
									...
									string sSYNC_REMOTE_KEY = Sql.ToString(row["SYNC_REMOTE_KEY"]);
									if ( bVERBOSE_STATUS )
										SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.ConstantContactTableName + ".Sync: Deleting ConstantContact (" + sSYNC_REMOTE_KEY + "), CRM " + qo.CRMModuleName + "(" + gID.ToString() + ").");
