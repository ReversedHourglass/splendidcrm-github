...
namespace Spring.Social.GetResponse
{
	public class GetResponseSync
	{
		...
		public static bool GetResponseEnabled(HttpApplicationState Application)
		{
		...
		public static Guid GetResponseUserID(HttpApplicationState Application)
		{
		...
		public static Spring.Social.GetResponse.Api.IGetResponse CreateApi(HttpApplicationState Application)
		{
		...
		public static bool ValidateGetResponse(HttpApplicationState Application, string sSecretApiKey, StringBuilder sbErrors)
		{
		...
		public static bool ValidateGetResponse(HttpApplicationState Application, StringBuilder sbErrors)
		{
			bool bValidSource = false;
			try
			{
				Spring.Social.GetResponse.Api.IGetResponse getResponse = CreateApi(Application);
				getResponse.ContactOperations.GetModified(DateTime.Today);
			}
			catch(Exception ex)
			{
		...
		public static void Sync(HttpContext Context, bool bSyncAll)
		{
			...
			bool bGetResponseEnabled = GetResponseEnabled(Application);
			if ( !bInsideSyncAll && bGetResponseEnabled )
			{
				...
				try
				{
					...
					Guid gGetResponse_USER_ID = GetResponseSync.GetResponseUserID(Context.Application);
					GetResponse.UserSync User = new GetResponse.UserSync(Context, gGetResponse_USER_ID, bSyncAll);
					Sync(User, sbErrors);
				}
				catch(Exception ex)
				{
		...
		public static void Sync(GetResponse.UserSync User, StringBuilder sbErrors)
		{
			...
			if ( bVERBOSE_STATUS )
				SyncError.SystemMessage(User.Context, "Warning", new StackTrace(true).GetFrame(0), "GetResponseSync.Sync Start.");
			
			Spring.Social.GetResponse.Api.IGetResponse getResponse = GetResponseSync.CreateApi(Application);
			if ( getResponse != null )
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					{
						if ( bVERBOSE_STATUS )
							SyncError.SystemMessage(User.Context, "Warning", new StackTrace(true).GetFrame(0), "GetResponseSync.Sync Done.");
					}
				}
			}
		}

		public static void Sync(SplendidCRM.DbProviderFactory dbf, IDbConnection con, GetResponse.UserSync User, GetResponse.HObject qo, StringBuilder sbErrors)
		{
			...
			if ( bVERBOSE_STATUS )
				SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.GetResponseTableName + ".Sync: " + sDIRECTION);
			try
			{
				...
				if ( bVERBOSE_STATUS )
					SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.GetResponseTableName + ".Sync: Query took " + (DateTime.Now - dtStartSelect).Seconds.ToString() + " seconds. Using last modified " + dtStartModifiedDate.ToString());
				if ( lst.Count > 0 )
					SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.GetResponseTableName + ".Sync: " + lst.Count.ToString() + " " + qo.GetResponseTableName + " to import.");
					...
					using ( DbDataAdapter da = dbf.CreateDataAdapter() )
					{
						...
						using ( DataTable dt = new DataTable() )
						{
							...
							if ( dt.Rows.Count > 0 && !qo.IsReadOnly )
								SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.GetResponseTableName + ".Sync: " + dt.Rows.Count.ToString() + " " + qo.CRMModuleName + " to send.");
							...
							for ( int i = 0; i < dt.Rows.Count && !qo.IsReadOnly; i++ )
							{
								...
								try
								{
									if ( sSYNC_ACTION == "local new" || Sql.IsEmptyString(sSYNC_REMOTE_KEY) )
									{
										...
										{
											if ( bVERBOSE_STATUS )
												SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.GetResponseTableName + ".Sync: Sending new " + qo.GetResponseTableName + " (" + i.ToString() + ") " + Sql.ToString(row["NAME"]) + ".");
									...
									{
										if ( bVERBOSE_STATUS )
											SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.GetResponseTableName + ".Sync: Binding " + qo.GetResponseTableName + " (" + i.ToString() + ") " + Sql.ToString(row["NAME"]) + ".");
										...
										if ( sSYNC_ACTION == "local changed" )
										{
											...
											if (sDIRECTION == "bi-directional" || sDIRECTION == "from crm only" )
											{
												...
												if ( bVERBOSE_STATUS )
													SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.GetResponseTableName + ".Sync: Sending " + qo.GetResponseTableName + " (" + i.ToString() + ") " + Sql.ToString(row["NAME"]) + ". " + sbChanges.ToString());
												qo.Update();
											}
										}
									}
									...
									else if ( sSYNC_ACTION == "remote deleted" || sSYNC_ACTION == "remote unsync" )
									{
										...
										{
											if ( bVERBOSE_STATUS )
												SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.GetResponseTableName + ".Sync: Deleting " + qo.CRMTableName + " " + Sql.ToString(row["NAME"]) + ".");
											using ( IDbTransaction trn = Sql.BeginTransaction(con) )
											{
												try
												{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								da.Fill(dt);
								if ( dt.Rows.Count > 0 )
									SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.GetResponseTableName + ".Sync: " + dt.Rows.Count.ToString() + " " + qo.CRMModuleName + " to delete.");
								for ( int i = 0; i < dt.Rows.Count; i++ )
								{
									...
									string sSYNC_REMOTE_KEY = Sql.ToString(row["SYNC_REMOTE_KEY"]);
									if ( bVERBOSE_STATUS )
										SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.GetResponseTableName + ".Sync: Deleting GetResponse (" + sSYNC_REMOTE_KEY + "), CRM " + qo.CRMModuleName + "(" + gID.ToString() + ").");
