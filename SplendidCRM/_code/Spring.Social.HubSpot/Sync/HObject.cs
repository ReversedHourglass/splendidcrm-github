...
namespace Spring.Social.HubSpot
{
	public class HObject
	{
		...
		public HObject(Spring.Social.HubSpot.Api.IHubSpot hubSpot, string sHubSpotTableName, string sHubSpotTableSort, string sCRMModuleName, string sCRMTableName, string sCRMTableSort, bool bCRMAssignedUser)
		{
					...
					using ( DataTable dt = new DataTable() )
					{
						...
						if ( sSYNC_ACTION == "remote new" || sSYNC_ACTION == "remote changed" )
						{
							...
							if (sDIRECTION == "bi-directional" || sDIRECTION == "to crm only" )
							{
								...
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									...
									{
										if ( bVERBOSE_STATUS )
											SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), this.HubSpotTableName + ".Import: Retrieving " + this.HubSpotTableName + " " + sName + ".");
								...
								if ( bChanged && bVERBOSE_STATUS )
									SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), this.HubSpotTableName + ".Import: Received " + this.HubSpotTableName + " " + sName + ". " + sbChanges.ToString());
							}
						}
						else if ( (sSYNC_ACTION == "local changed" || sSYNC_ACTION == "local new") && !Sql.IsEmptyGuid(this.LOCAL_ID) )
						{
							...
							if ( dt.Rows.Count > 0 )
							{
								...
								{
									if ( bVERBOSE_STATUS )
										SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), this.HubSpotTableName + ".Import: Syncing " + this.HubSpotTableName + " " + Sql.ToString(row["NAME"]) + ".");
									bool bChanged = false;
									// 05/18/2012 Paul.  Allow control of sync direction. 
						...
						else if ( sSYNC_ACTION == "local deleted" )
						{
							...
							if ( bVERBOSE_STATUS )
								SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), this.HubSpotTableName + ".Import: Deleting " + sName + ".");
							using ( IDbTransaction trn = Sql.BeginTransaction(con) )
							{
								try
								{
						...
						else if ( sSYNC_ACTION == "remote deleted" && !Sql.IsEmptyGuid(this.LOCAL_ID) )
						{
							...
							{
								if ( bVERBOSE_STATUS )
									SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), this.HubSpotTableName + ".Import: Deleting " + this.CRMTableName + " " + sName + ".");
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									try
									{
