...
namespace Spring.Social.HubSpot
{
	public class HubSpotSync
	{
		...
		public static bool HubSpotEnabled(HttpApplicationState Application)
		{
		...
		public static Guid HubSpotUserID(HttpApplicationState Application)
		{
		...
		public static Spring.Social.HubSpot.Api.IHubSpot CreateApi(HttpApplicationState Application)
		{
		...
		public static bool ValidateHubSpot(HttpApplicationState Application, string sOAuthPortalID, string sOAuthClientID, string sOAuthClientSecret, string sOAuthAccessToken, StringBuilder sbErrors)
		{
		...
		public static bool ValidateHubSpot(HttpApplicationState Application, StringBuilder sbErrors)
		{
			bool bValidSource = false;
			try
			{
				Spring.Social.HubSpot.Api.IHubSpot hubSpot = CreateApi(Application);
		...
		public static void Sync(HttpContext Context, bool bSyncAll)
		{
			...
			bool bHubSpotEnabled = HubSpotEnabled(Application);
			if ( !bInsideSyncAll && bHubSpotEnabled )
			{
				...
				try
				{
					...
					{
						Guid gHubSpot_USER_ID = HubSpotSync.HubSpotUserID(Context.Application);
						HubSpot.UserSync User = new HubSpot.UserSync(Context, gHubSpot_USER_ID, bSyncAll);
						Sync(User, sbErrors);
					}
					else
					{
						SyncError.SystemMessage(Context, "Error", new StackTrace(true).GetFrame(0), "Failed to refresh HubSpot Access Token: " + sbErrors.ToString());
					}
				}
				catch(Exception ex)
				{
		...
		public static void Sync(HubSpot.UserSync User, StringBuilder sbErrors)
		{
			...
			if ( bVERBOSE_STATUS )
				SyncError.SystemMessage(User.Context, "Warning", new StackTrace(true).GetFrame(0), "HubSpotSync.Sync Start.");
			
			Spring.Social.HubSpot.Api.IHubSpot hubSpot = HubSpotSync.CreateApi(Application);
			if ( hubSpot != null )
			{
				SplendidCRM.DbProviderFactory dbf = SplendidCRM.DbProviderFactories.GetFactory(Application);
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					try
					{
						...
						{
							HubSpot.Company company = new HubSpot.Company(hubSpot);
							if ( !bInsideCompanies )
							{
								try
								{
							...
							{
								HubSpot.Contact contact = new HubSpot.Contact(hubSpot, dtCompanies);
								if ( !bInsideContacts )
								{
									try
									{
						...
						{
							HubSpot.Lead lead = new HubSpot.Lead(hubSpot);
							if ( !bInsideContacts )
							{
								try
								{
					...
					{
						if ( bVERBOSE_STATUS )
							SyncError.SystemMessage(User.Context, "Warning", new StackTrace(true).GetFrame(0), "HubSpotSync.Sync Done.");
					}
				}
			}
		}

		public static void Sync(SplendidCRM.DbProviderFactory dbf, IDbConnection con, HubSpot.UserSync User, HubSpot.HObject qo, StringBuilder sbErrors)
		{
			...
			if ( bVERBOSE_STATUS )
				SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.HubSpotTableName + ".Sync: " + sDIRECTION);
			try
			{
				string sSQL = String.Empty;
				...
				if ( bVERBOSE_STATUS )
					SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.HubSpotTableName + ".Sync: Query took " + (DateTime.Now - dtStartSelect).Seconds.ToString() + " seconds. Using last modified " + dtStartModifiedDate.ToString());
				if ( lst.Count > 0 )
					SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.HubSpotTableName + ".Sync: " + lst.Count.ToString() + " " + qo.HubSpotTableName + " to import.");
					...
					using ( DbDataAdapter da = dbf.CreateDataAdapter() )
					{
						...
						using ( DataTable dt = new DataTable() )
						{
							...
							if ( dt.Rows.Count > 0 && !qo.IsReadOnly )
								SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.HubSpotTableName + ".Sync: " + dt.Rows.Count.ToString() + " " + qo.CRMModuleName + " to send.");
							...
							for ( int i = 0; i < dt.Rows.Count && !qo.IsReadOnly; i++ )
							{
								...
								try
								{
									if ( sSYNC_ACTION == "local new" || Sql.IsEmptyString(sSYNC_REMOTE_KEY) )
									{
										...
										{
											if ( bVERBOSE_STATUS )
												SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.HubSpotTableName + ".Sync: Sending new " + qo.HubSpotTableName + " (" + i.ToString() + ") " + Sql.ToString(row["NAME"]) + ".");
									...
									{
										if ( bVERBOSE_STATUS )
											SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.HubSpotTableName + ".Sync: Binding " + qo.HubSpotTableName + " (" + i.ToString() + ") " + Sql.ToString(row["NAME"]) + ".");
										...
										if ( sSYNC_ACTION == "local changed" )
										{
											...
											if (sDIRECTION == "bi-directional" || sDIRECTION == "from crm only" )
											{
												...
												if ( bVERBOSE_STATUS )
													SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.HubSpotTableName + ".Sync: Sending " + qo.HubSpotTableName + " (" + i.ToString() + ") " + Sql.ToString(row["NAME"]) + ". " + sbChanges.ToString());
												qo.Update();
											}
										}
									}
									...
									else if ( sSYNC_ACTION == "remote deleted" || sSYNC_ACTION == "remote unsync" )
									{
										...
										{
											if ( bVERBOSE_STATUS )
												SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.HubSpotTableName + ".Sync: Deleting " + qo.CRMTableName + " " + Sql.ToString(row["NAME"]) + ".");
											using ( IDbTransaction trn = Sql.BeginTransaction(con) )
											{
												try
												{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								da.Fill(dt);
								if ( dt.Rows.Count > 0 )
									SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.HubSpotTableName + ".Sync: " + dt.Rows.Count.ToString() + " " + qo.CRMModuleName + " to delete.");
								for ( int i = 0; i < dt.Rows.Count; i++ )
								{
									...
									string sSYNC_REMOTE_KEY = Sql.ToString(row["SYNC_REMOTE_KEY"]);
									if ( bVERBOSE_STATUS )
										SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.HubSpotTableName + ".Sync: Deleting HubSpot (" + sSYNC_REMOTE_KEY + "), CRM " + qo.CRMModuleName + "(" + gID.ToString() + ").");
