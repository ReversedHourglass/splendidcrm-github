...
namespace Spring.Social.Marketo.Connect
{
	public class MarketoOAuth2Template : OAuth2Template
	{
		...
		{
			NameValueCollection response = this.RestTemplate.GetForObject<NameValueCollection>(accessTokenUrl, request);
		...
		{
			return this.RestTemplate.GetForObjectAsync<NameValueCollection>(accessTokenUrl, request,
				r =>
				{
					if (r.Error == null)
					{
