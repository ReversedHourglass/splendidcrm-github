...
namespace Spring.Social.Marketo
{
	public class MarketoSync
	{
		...
		public static bool MarketoEnabled(HttpApplicationState Application)
		{
		...
		public static Guid MarketoUserID(HttpApplicationState Application)
		{
		...
		public static Spring.Social.Marketo.Api.IMarketo CreateApi(HttpApplicationState Application)
		{
		...
		public static bool AuthorizeMarketo(HttpApplicationState Application, string sMarketoClientID, string sMarketoClientSecret, string sMarketoEndpointURL, string sMarketoIdentityURL, ref string sOAuthAccessToken, ref string sOAuthExpiresAt, ref string sOAuthScope, StringBuilder sbErrors)
		{
			bool bSuccess = false;
			try
			{
		...
		public static bool RefreshAccessToken(HttpApplicationState Application, StringBuilder sbErrors)
		{
			bool bSuccess = false;
			string sMarketoEndpointURL  = Sql.ToString(Application["CONFIG.Marketo.EndpointURL"     ]);
			string sMarketoIdentityURL  = Sql.ToString(Application["CONFIG.Marketo.IdentityURL"     ]);
		...
		public static bool ValidateMarketo(HttpApplicationState Application, string sMarketoClientID, string sMarketoClientSecret, string sMarketoEndpointURL, string sMarketoIdentityURL, string sOAuthAccessToken, StringBuilder sbErrors)
		{
		...
		public static bool ValidateMarketo(HttpApplicationState Application, StringBuilder sbErrors)
		{
			bool bValidSource = false;
			try
			{
				Spring.Social.Marketo.Api.IMarketo Marketo = CreateApi(Application);
		...
		public static void Sync(HttpContext Context, bool bSyncAll)
		{
			...
			bool bMarketoEnabled = MarketoEnabled(Application);
			if ( !bInsideSyncAll && bMarketoEnabled )
			{
				...
				try
				{
					...
					if ( sbErrors.Length == 0 )
					{
						Guid gMarketo_USER_ID = MarketoSync.MarketoUserID(Context.Application);
						Marketo.UserSync User = new Marketo.UserSync(Context, gMarketo_USER_ID, bSyncAll);
						Sync(User, sbErrors);
					}
					else
					{
						SyncError.SystemMessage(Context, "Error", new StackTrace(true).GetFrame(0), "Failed to refresh Marketo Access Token: " + sbErrors.ToString());
					}
				}
				catch(Exception ex)
				{
		...
		public static string MarketoFields(HttpApplicationState Application, Spring.Social.Marketo.Api.IMarketo marketo)
		{
		...
		public static void Sync(Marketo.UserSync User, StringBuilder sbErrors)
		{
			...
			if ( bVERBOSE_STATUS )
				SyncError.SystemMessage(User.Context, "Warning", new StackTrace(true).GetFrame(0), "MarketoSync.Sync Start.");
			
			Spring.Social.Marketo.Api.IMarketo marketo = MarketoSync.CreateApi(Application);
			if ( marketo != null )
			{
				SplendidCRM.DbProviderFactory dbf = SplendidCRM.DbProviderFactories.GetFactory(Application);
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					try
					{
						...
						string sMarketoFields = MarketoSync.MarketoFields(Application, marketo);
						// 05/19/2015 Paul.  Marketo returns time as local to (GMT-08:00) Pacific Time (US & Canada). 
						if ( sSYNC_MODULES == "Contacts" )
						{
							Marketo.Contact contact = new Marketo.Contact(marketo, sMarketoFields);
							if ( !bInsideContacts )
							{
								try
								{
						...
						{
							Marketo.Lead lead = new Marketo.Lead(marketo, sMarketoFields);
							if ( !bInsideContacts )
							{
								try
								{
					...
					{
						if ( bVERBOSE_STATUS )
							SyncError.SystemMessage(User.Context, "Warning", new StackTrace(true).GetFrame(0), "MarketoSync.Sync Done.");
					}
				}
			}
		}

		public static void Sync(SplendidCRM.DbProviderFactory dbf, IDbConnection con, Marketo.UserSync User, Marketo.MObject qo, StringBuilder sbErrors)
		{
			...
			if ( bVERBOSE_STATUS )
				SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.MarketoTableName + ".Sync: " + sDIRECTION);
			try
			{
				...
				if ( bVERBOSE_STATUS )
					SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.MarketoTableName + ".Sync: Query took " + (DateTime.Now - dtStartSelect).Seconds.ToString() + " seconds. Using last modified " + dtStartModifiedDate.ToString());
				if ( lst.Count > 0 )
					SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.MarketoTableName + ".Sync: " + lst.Count.ToString() + " " + qo.MarketoTableName + " to import.");
					...
					using ( DbDataAdapter da = dbf.CreateDataAdapter() )
					{
						...
						using ( DataTable dt = new DataTable() )
						{
							...
							if ( dt.Rows.Count > 0 && !qo.IsReadOnly )
								SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.MarketoTableName + ".Sync: " + dt.Rows.Count.ToString() + " " + qo.CRMModuleName + " to send.");
							...
							for ( int i = 0; i < dt.Rows.Count && !qo.IsReadOnly; i++ )
							{
								...
								try
								{
									if ( sSYNC_ACTION == "local new" || Sql.IsEmptyString(sSYNC_REMOTE_KEY) )
									{
										...
										{
											if ( bVERBOSE_STATUS )
												SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.MarketoTableName + ".Sync: Sending new " + qo.MarketoTableName + " (" + i.ToString() + ") " + Sql.ToString(row["NAME"]) + ".");
									...
									{
										if ( bVERBOSE_STATUS )
											SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.MarketoTableName + ".Sync: Binding " + qo.MarketoTableName + " (" + i.ToString() + ") " + Sql.ToString(row["NAME"]) + ".");
										...
										if ( sSYNC_ACTION == "local changed" )
										{
											...
											if (sDIRECTION == "bi-directional" || sDIRECTION == "from crm only" )
											{
												...
												if ( bVERBOSE_STATUS )
													SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.MarketoTableName + ".Sync: Sending " + qo.MarketoTableName + " (" + i.ToString() + ") " + Sql.ToString(row["NAME"]) + ". " + sbChanges.ToString());
												qo.Update();
											}
										}
									}
									...
									else if ( sSYNC_ACTION == "remote deleted" || sSYNC_ACTION == "remote unsync" )
									{
										...
										{
											if ( bVERBOSE_STATUS )
												SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.MarketoTableName + ".Sync: Deleting " + qo.CRMTableName + " " + Sql.ToString(row["NAME"]) + ".");
											using ( IDbTransaction trn = Sql.BeginTransaction(con) )
											{
												try
												{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								da.Fill(dt);
								if ( dt.Rows.Count > 0 )
									SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.MarketoTableName + ".Sync: " + dt.Rows.Count.ToString() + " " + qo.CRMModuleName + " to delete.");
								for ( int i = 0; i < dt.Rows.Count; i++ )
								{
									...
									string sSYNC_REMOTE_KEY = Sql.ToString(row["SYNC_REMOTE_KEY"]);
									if ( bVERBOSE_STATUS )
										SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.MarketoTableName + ".Sync: Deleting Marketo (" + sSYNC_REMOTE_KEY + "), CRM " + qo.CRMModuleName + "(" + gID.ToString() + ").");
