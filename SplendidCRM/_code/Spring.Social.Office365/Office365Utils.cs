...
namespace SplendidCRM
{
	...
	{
		public static Dictionary<Guid, string> dictInboundSubscriptions = new Dictionary<Guid, string>();
		...
		{
			SplendidMailOffice365 client = new SplendidMailOffice365(Application, gUSER_ID);
		...
		public static bool ValidateExchange(HttpApplicationState Application, string sOAuthDirectoryTenatID, string sOAuthClientID, string sOAuthClientSecret, Guid gUSER_ID, string sMAILBOX, StringBuilder sbErrors)
		{
			...
			try
			{
				Office365AccessToken token = SplendidCRM.ActiveDirectory.Office365RefreshAccessToken(Application, sOAuthDirectoryTenatID, sOAuthClientID, sOAuthClientSecret, gUSER_ID, false);
		...
		public static void GetMessage(HttpContext Context, Guid gMAILBOX_ID, string sUNIQUE_ID, ref string sNAME, ref string sFROM_ADDR, ref bool bIS_READ, ref int nSIZE)
		{
			...
			Office365AccessToken token = ActiveDirectory.Office365RefreshAccessToken(Context.Application, sOAuthDirectoryTenatID, sOAUTH_CLIENT_ID, sOAUTH_CLIENT_SECRET, gMAILBOX_ID, false);
		...
		public static DataTable GetFolderMessages(HttpContext Context, string sUSERNAME, string sPASSWORD, Guid gMAILBOX_ID, string sMAILBOX, bool bONLY_SINCE, string sEXCHANGE_WATERMARK)
		{
			...
			// 02/04/2023 Paul.  Directory Tenant is now required for single tenant app registrations. 
			string sOAuthDirectoryTenatID = Sql.ToString(Context.Application["CONFIG.Exchange.DirectoryTenantID"]);
			int    nPageSize            = Sql.ToInteger(Context.Application["CONFIG.Exchange.Messages.PageSize"]);
			if ( nPageSize <= 0 )
				nPageSize = 100;
		...
		public static string NormalizeInternetAddressName(Spring.Social.Office365.Api.Recipient recipient)
		{
		...
		public static void BuildAddressList(Spring.Social.Office365.Api.Recipient addr, StringBuilder sbTO_ADDRS, StringBuilder sbTO_ADDRS_NAMES, StringBuilder sbTO_ADDRS_EMAILS)
		{
		...
		public static Guid FindTargetTrackerKey(Spring.Social.Office365.Api.Message email, string sHtmlBody, string sTextBody)
		{
		...
		public static string EmbedInlineImages(Spring.Social.Office365.Api.Message email, string sDESCRIPTION_HTML)
		{
									...
									if ( !bInline )
									{
										...
										if ( file.ContentBytes != null )
										{
											...
											if ( (lUploadMaxSize == 0) || (lFileSize <= lUploadMaxSize) )
											{
												...
												Crm.NoteAttachments.LoadFile(gNOTE_ATTACHMENT_ID, file.ContentBytes, trn);
											}
										}
									}
								}
