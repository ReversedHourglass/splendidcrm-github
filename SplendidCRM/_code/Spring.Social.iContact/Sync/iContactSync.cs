...
namespace Spring.Social.iContact
{
	public class iContactSync
	{
		...
		public static bool iContactEnabled(HttpApplicationState Application)
		{
		...
		public static Guid iContactUserID(HttpApplicationState Application)
		{
		...
		public static Spring.Social.iContact.Api.IiContact CreateApi(HttpApplicationState Application)
		{
			string sApiAppId               = Sql.ToString(Application["CONFIG.iContact.ApiAppId"              ]);
			string sApiUsername            = Sql.ToString(Application["CONFIG.iContact.ApiUsername"           ]);
			string sApiPassword            = Sql.ToString(Application["CONFIG.iContact.ApiPassword"           ]);
		...
		public static bool ValidateiContact(HttpApplicationState Application, string sApiAppId, string sApiUsername, string sApiPassword, string siContactAccountId, string siContactClientFolderId, StringBuilder sbErrors)
		{
		...
		public static bool GetAccount(HttpApplicationState Application, string sApiAppId, string sApiUsername, string sApiPassword, ref string siContactAccountId, ref string siContactClientFolderId, StringBuilder sbErrors)
		{
			bool bValidSource = false;
			try
			{
		...
		public static bool ValidateiContact(HttpApplicationState Application, StringBuilder sbErrors)
		{
			bool bValidSource = false;
			try
			{
				Spring.Social.iContact.Api.IiContact icontact = CreateApi(Application);
				icontact.ContactOperations.Validate();
			}
			catch(Exception ex)
			{
		...
		public static void Sync(HttpContext Context, bool bSyncAll)
		{
			...
			bool biContactEnabled = iContactEnabled(Application);
			if ( !bInsideSyncAll && biContactEnabled )
			{
				...
				try
				{
					...
					Guid giContact_USER_ID = iContactSync.iContactUserID(Context.Application);
					iContact.UserSync User = new iContact.UserSync(Context, giContact_USER_ID, bSyncAll);
					Sync(User, sbErrors);
				}
				catch(Exception ex)
				{
		...
		public static void Sync(iContact.UserSync User, StringBuilder sbErrors)
		{
			...
			if ( bVERBOSE_STATUS )
				SyncError.SystemMessage(User.Context, "Warning", new StackTrace(true).GetFrame(0), "iContactSync.Sync Start.");
			
			Spring.Social.iContact.Api.IiContact icontact = iContactSync.CreateApi(Application);
			if ( icontact != null )
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					{
						if ( bVERBOSE_STATUS )
							SyncError.SystemMessage(User.Context, "Warning", new StackTrace(true).GetFrame(0), "iContactSync.Sync Done.");
					}
				}
			}
		}

		public static void Sync(SplendidCRM.DbProviderFactory dbf, IDbConnection con, iContact.UserSync User, iContact.HObject qo, StringBuilder sbErrors)
		{
			...
			if ( bVERBOSE_STATUS )
				SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.iContactTableName + ".Sync: " + sDIRECTION);
			try
			{
				...
				//if ( bVERBOSE_STATUS )
				//	SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.iContactTableName + ".Sync: Query took " + (DateTime.Now - dtStartSelect).Seconds.ToString() + " seconds. Using last modified " + dtStartModifiedDate.ToString());
				//if ( lst.Count > 0 )
				//	SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.iContactTableName + ".Sync: " + lst.Count.ToString() + " " + qo.iContactTableName + " to import.");
					...
					using ( DbDataAdapter da = dbf.CreateDataAdapter() )
					{
						...
						using ( DataTable dt = new DataTable() )
						{
							...
							if ( dt.Rows.Count > 0 && !qo.IsReadOnly )
								SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.iContactTableName + ".Sync: " + dt.Rows.Count.ToString() + " " + qo.CRMModuleName + " to send.");
							...
							for ( int i = 0; i < dt.Rows.Count && !qo.IsReadOnly; i++ )
							{
								...
								try
								{
									if ( sSYNC_ACTION == "local new" || Sql.IsEmptyString(sSYNC_REMOTE_KEY) )
									{
										...
										{
											if ( bVERBOSE_STATUS )
												SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.iContactTableName + ".Sync: Sending new " + qo.iContactTableName + " (" + i.ToString() + ") " + Sql.ToString(row["NAME"]) + ".");
									...
									{
										if ( bVERBOSE_STATUS )
											SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.iContactTableName + ".Sync: Binding " + qo.iContactTableName + " (" + i.ToString() + ") " + Sql.ToString(row["NAME"]) + ".");
										...
										if ( sSYNC_ACTION == "local changed" )
										{
											...
											if (sDIRECTION == "bi-directional" || sDIRECTION == "from crm only" )
											{
												...
												if ( bVERBOSE_STATUS )
													SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.iContactTableName + ".Sync: Sending " + qo.iContactTableName + " (" + i.ToString() + ") " + Sql.ToString(row["NAME"]) + ". " + sbChanges.ToString());
												qo.Update();
											}
										}
									}
									...
									else if ( sSYNC_ACTION == "remote deleted" || sSYNC_ACTION == "remote unsync" )
									{
										...
										{
											if ( bVERBOSE_STATUS )
												SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.iContactTableName + ".Sync: Deleting " + qo.CRMTableName + " " + Sql.ToString(row["NAME"]) + ".");
											using ( IDbTransaction trn = Sql.BeginTransaction(con) )
											{
												try
												{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								da.Fill(dt);
								if ( dt.Rows.Count > 0 )
									SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.iContactTableName + ".Sync: " + dt.Rows.Count.ToString() + " " + qo.CRMModuleName + " to delete.");
								for ( int i = 0; i < dt.Rows.Count; i++ )
								{
									...
									string sSYNC_REMOTE_KEY = Sql.ToString(row["SYNC_REMOTE_KEY"]);
									if ( bVERBOSE_STATUS )
										SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), qo.iContactTableName + ".Sync: Deleting iContact (" + sSYNC_REMOTE_KEY + "), CRM " + qo.CRMModuleName + "(" + gID.ToString() + ").");
