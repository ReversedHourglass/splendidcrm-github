...
namespace SplendidCRM
{
	...
	public class iCloudSync
	{
		...
		public static void GetUserCredentials(HttpApplicationState Application, Guid gUSER_ID, ref string sICLOUD_USERNAME, ref string sICLOUD_PASSWORD, ref string sICLOUD_CTAG_CONTACTS, ref string sICLOUD_CTAG_CALENDAR)
		{
		...
		public static DataTable CreateContactsTable(HttpApplicationState Application, ref IDbCommand spCONTACTS_Update)
		{
		...
		public static DataTable CreateAppointmentsTable(HttpApplicationState Application, ref IDbCommand spAPPOINTMENTS_Update)
		{
			DataTable dt = new DataTable();
			dt.Columns.Add("UID"             , typeof(String  ));
			dt.Columns.Add("NAME"            , typeof(String  ));
		...
		public static DataTable CreateParticipantsTable()
		{
			DataTable dt = new DataTable();
		...
		public static string GenerateAppleClientSecret(HttpApplicationState Application)
		{
				...
				if ( identity != null )
				{
					...
					using ( IDbConnection con = dbf.CreateConnection() )
					{
						...
						try
						{
							...
							sRequestBody += "&redirect_uri="  + HttpUtility.UrlEncode("https://" + Request.Url.Host + Sql.ToString(Application["rootURL"]) + "OAuth/iCloudLanding.aspx");
		...
		public static void SyncAllUsers(Object sender)
		{
			...
			bool bICLOUDEnabled = Sql.ToBoolean(Context.Application["CONFIG.iCloud.Enabled"]);
				...
				try
				{
					using ( DataTable dt = new DataTable() )
					{
						...
						foreach ( DataRow row in dt.Rows )
						{
							...
							iCloudSync.UserSync User = new iCloudSync.UserSync(Context, bICLOUD_SYNC_CONTACTS, bICLOUD_SYNC_CALENDAR, sICLOUD_USERNAME, sICLOUD_PASSWORD, gUSER_ID, sICLOUD_CTAG_CONTACTS, sICLOUD_CTAG_CALENDAR, false);
							Sync(User, sbErrors);
						}
					}
				}
		...
		public class UserSync
		{
			...
			public UserSync(HttpContext Context, bool bICLOUD_SYNC_CONTACTS, bool bICLOUD_SYNC_CALENDAR, string sICLOUD_USERNAME, string sICLOUD_PASSWORD, Guid gUSER_ID, string sICLOUD_CTAG_CONTACTS, string sICLOUD_CTAG_CALENDAR, bool bSyncAll)
			{
			...
			public void Start()
			{
				...
				bool bVERBOSE_STATUS = Sql.ToBoolean(Context.Application["CONFIG.iCloud.VerboseStatus"]);
				if ( bVERBOSE_STATUS )
					SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "UserSync:  Begin " + ICLOUD_USERNAME + ", " + USER_ID.ToString() + " at " + dtStart.ToString());
				...
				if ( bVERBOSE_STATUS )
					SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "UserSync:  End "   + ICLOUD_USERNAME + ", " + USER_ID.ToString() + " at " + dtEnd.ToString() + ". Elapse time " + ts.Minutes.ToString() + " minutes " + ts.Seconds.ToString() + " seconds.");
			}
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						using ( IDataReader row = cmd.ExecuteReader(CommandBehavior.SingleRow) )
						{
							if ( row.Read() )
							{
								...
								string sICLOUD_CTAG_CONTACTS = Sql.ToString (row["ICLOUD_CTAG_CONTACTS"]);
								string sICLOUD_CTAG_CALENDAR = Sql.ToString (row["ICLOUD_CTAG_CALENDAR"]);
								User = new iCloudSync.UserSync(Context, bICLOUD_SYNC_CONTACTS, bICLOUD_SYNC_CALENDAR, sICLOUD_USERNAME, sICLOUD_PASSWORD, gUSER_ID, sICLOUD_CTAG_CONTACTS, sICLOUD_CTAG_CALENDAR, bSyncAll);
							}
						}
					}
				}
		...
		public static void Sync(iCloudSync.UserSync User, StringBuilder sbErrors)
		{
			...
			if ( !arrProcessing.Contains(User.USER_ID) )
			{
				...
				try
				{
					...
					{
						ContactsService service = new ContactsService(User.Context.Application);
						// 01/20/2012 Paul.  Use the SplendidCRM unique key as the iCloud MmeDeviceID. 
						service.setUserCredentials(User.ICLOUD_USERNAME, User.ICLOUD_PASSWORD, User.ICLOUD_CTAG_CONTACTS, User.ICLOUD_CTAG_CALENDAR, Sql.ToString(User.Context.Application["CONFIG.unique_key"]));
						service.QueryClientLoginToken(false);
						
						iCloudSync.SyncContacts(User.Context, Session, service, User.ICLOUD_USERNAME, User.USER_ID, User.SyncAll, sbErrors);
					}
					if ( User.ICLOUD_SYNC_CALENDAR )
					{
						CalendarService service = new CalendarService(User.Context.Application);
						// 01/20/2012 Paul.  Use the SplendidCRM unique key as the iCloud MmeDeviceID. 
						service.setUserCredentials(User.ICLOUD_USERNAME, User.ICLOUD_PASSWORD, User.ICLOUD_CTAG_CONTACTS, User.ICLOUD_CTAG_CALENDAR, Sql.ToString(User.Context.Application["CONFIG.unique_key"]));
						service.QueryClientLoginToken(false);
						
						iCloudSync.SyncAppointments(User.Context, Session, service, User.ICLOUD_USERNAME, User.USER_ID, User.SyncAll, sbErrors);
					}
				}
				finally
				{
		...
		public static bool SetiCloudContact(ContactEntry contact, DataRow row, StringBuilder sbChanges)
		{
			bool bChanged = false;
		...
		public static void SyncContacts(HttpContext Context, ExchangeSession Session, ContactsService service, string sICLOUD_USERNAME, Guid gUSER_ID, bool bSyncAll, StringBuilder sbErrors)
		{
			bool bVERBOSE_STATUS = Sql.ToBoolean(Context.Application["CONFIG.iCloud.VerboseStatus"]);
			if ( bVERBOSE_STATUS )
				SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncContacts: " + sICLOUD_USERNAME + " to " + gUSER_ID.ToString());
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				try
				{
					...
					if ( fContactResults.Count > 0 )
						SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncContacts: " + fContactResults.Count.ToString() + " contacts to retrieve from " + sICLOUD_USERNAME);
					foreach ( ContactEntry contact in fContactResults )
					{
						...
						iCloudSync.ImportContact(Context, Session, service, con, sICLOUD_USERNAME, gUSER_ID, contact, sbErrors);
					}
					using ( IDbTransaction trn = Sql.BeginTransaction(con) )
					{
						try
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								da.Fill(dt);
								if ( dt.Rows.Count > 0 )
									SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncContacts: " + dt.Rows.Count.ToString() + " contacts to send to " + sICLOUD_USERNAME);
								foreach ( DataRow row in dt.Rows )
								{
									...
									try
									{
										...
										{
											if ( bVERBOSE_STATUS )
												SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncContacts: Sending new contact " + Sql.ToString(row["FIRST_NAME"]) + " " + Sql.ToString(row["LAST_NAME"]) + " to " + sICLOUD_USERNAME);
											contact = new ContactEntry();
											iCloudSync.SetiCloudContact(contact, row, sbChanges);
										...
										{
											if ( bVERBOSE_STATUS )
												SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncContacts: Binding contact " + Sql.ToString(row["FIRST_NAME"]) + " " + Sql.ToString(row["LAST_NAME"]) + " to " + sICLOUD_USERNAME);
											...
											{
												bool bChanged = iCloudSync.SetiCloudContact(contact, row, sbChanges);
												if ( bChanged )
												{
													if ( bVERBOSE_STATUS )
														SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncContacts: Sending contact " + Sql.ToString(row["FIRST_NAME"]) + " " + Sql.ToString(row["LAST_NAME"]) + " to " + sICLOUD_USERNAME);
										...
										{
											if ( bVERBOSE_STATUS )
												SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncContacts: Unsyncing contact " + Sql.ToString(row["FIRST_NAME"]) + " " + Sql.ToString(row["LAST_NAME"]) + " for " + sICLOUD_USERNAME);
											using ( IDbTransaction trn = Sql.BeginTransaction(con) )
											{
												try
												{
					...
					//			da.Fill(dt);
					//			if ( dt.Rows.Count > 0 )
					//				SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncContacts: " + dt.Rows.Count.ToString() + " contacts to remove from group " + sICLOUD_USERNAME);
					//			foreach ( DataRow row in dt.Rows )
					//			{
					...
					//				{
					//					if ( bVERBOSE_STATUS )
					//						SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncContacts: Deleting sync contact " + Sql.ToString(row["FIRST_NAME"]) + " " + Sql.ToString(row["LAST_NAME"]) + " from " + sICLOUD_USERNAME);
					//					using ( IDbTransaction trn = Sql.BeginTransaction(con) )
					//					{
					//						try
					//						{
		...
		public static bool BuildCONTACTS_Update(HttpApplicationState Application, ExchangeSession Session, IDbCommand spCONTACTS_Update, DataRow row, ContactEntry contact, Guid gUSER_ID, Guid gTEAM_ID, Guid gASSIGNED_USER_ID, bool bCreateAccount, IDbTransaction trn)
		{
			bool bChanged = false;
		...
		// 01/24/2012 Paul.  We need to have a way to force an update as we cannot rely upon the iCloud embedded revision date. 
		public static void ImportContact(HttpContext Context, ExchangeSession Session, ContactsService service, IDbConnection con, string sICLOUD_USERNAME, Guid gUSER_ID, ContactEntry contact, StringBuilder sbErrors)
		{
			HttpApplicationState Application = Context.Application;
					...
					using ( DataTable dt = new DataTable() )
					{
						...
						if ( sSYNC_ACTION == "remote new" || sSYNC_ACTION == "remote changed" )
						{
							...
							using ( IDbTransaction trn = Sql.BeginTransaction(con) )
							{
								...
								{
									if ( bVERBOSE_STATUS )
										SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "ImportContact: Retrieving contact " + sContactName + " from " + sICLOUD_USERNAME);
						...
						else if ( (sSYNC_ACTION == "local changed" || sSYNC_ACTION == "local new") && !Sql.IsEmptyGuid(gID) )
						{
							...
							if ( dt.Rows.Count > 0 )
							{
								...
								{
									if ( bVERBOSE_STATUS )
										SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "ImportContact: Syncing contact " + Sql.ToString(row["FIRST_NAME"]) + " " + Sql.ToString(row["LAST_NAME"]) + " to " + sICLOUD_USERNAME);
									bool bChanged = iCloudSync.SetiCloudContact(contact, row, sbChanges);
									if ( bChanged )
									{
										service.Update(contact);
						...
						else if ( sSYNC_ACTION == "local deleted" )
						{
							...
							{
								if ( bVERBOSE_STATUS )
									SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "ImportContact: Deleting " + sContactName + " from " + sICLOUD_USERNAME);
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									try
									{
						...
						{
							if ( bVERBOSE_STATUS )
								SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "ImportContact: Unsyncing contact " + sContactName + " for " + sICLOUD_USERNAME);
							using ( IDbTransaction trn = Sql.BeginTransaction(con) )
							{
								try
								{
		...
		public static DataTable AppointmentEmails(HttpApplicationState Application, IDbConnection con, Guid gID, Guid gUSER_ID)
		{
		...
		public static bool SetiCloudAppointment(AppointmentEntry appointment, DataRow row, DataTable dtAppointmentEmails, string sICLOUD_USERNAME, StringBuilder sbChanges, bool bDISABLE_PARTICIPANTS)
		{
			bool bChanged = false;
		...
		public static void SyncAppointments(HttpContext Context, ExchangeSession Session, CalendarService service, string sICLOUD_USERNAME, Guid gUSER_ID, bool bSyncAll, StringBuilder sbErrors)
		{
			// 03/23/2013 Paul.  Add the ability to disable participants. 
			bool bDISABLE_PARTICIPANTS = Sql.ToBoolean(Context.Application["CONFIG.iCloud.DisableParticipants"]);
			bool bVERBOSE_STATUS       = Sql.ToBoolean(Context.Application["CONFIG.iCloud.VerboseStatus"      ]);
			if ( bVERBOSE_STATUS )
				SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncAppointments: " + sICLOUD_USERNAME + " to " + gUSER_ID.ToString());
			// 02/20/2013 Paul.  Reduced the number of days to go back. 
			int  nAPPOINTMENT_AGE_DAYS = Sql.ToInteger(Context.Application["CONFIG.iCloud.AppointmentAgeDays"]);
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				try
				{
					...
					if ( fEventResults.Count > 0 )
						SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncAppointments: " + fEventResults.Count.ToString() + " appointments to retrieve from " + sICLOUD_USERNAME);
					foreach (AppointmentEntry appointment in fEventResults )
					{
						// 01/24/2012 Paul.  We need to have a way to force an update as we cannot rely upon the iCloud embedded revision date. 
						// If the bSyncAll flag is not set, then the contact has changed remotely, regardless of the embedded date. 
						iCloudSync.ImportAppointment(Context, Session, service, con, sICLOUD_USERNAME, gUSER_ID, appointment, sbErrors);
					}
					using ( IDbTransaction trn = Sql.BeginTransaction(con) )
					{
						try
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								da.Fill(dt);
								if ( dt.Rows.Count > 0 )
									SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncAppointments: " + dt.Rows.Count.ToString() + " appointments to send to " + sICLOUD_USERNAME);
								foreach ( DataRow row in dt.Rows )
								{
									Guid     gID                             = Sql.ToGuid    (row["ID"                           ]);
									Guid     gASSIGNED_USER_ID               = Sql.ToGuid    (row["ASSIGNED_USER_ID"             ]);
									...
									try
									{
										...
										{
											if ( bVERBOSE_STATUS )
												SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncAppointments: Sending new appointment " + Sql.ToString(row["NAME"]) + " " + Sql.ToString(row["DATE_START"]) + " to " + sICLOUD_USERNAME);
											...
											// 03/23/2013 Paul.  Add the ability to disable participants. 
											iCloudSync.SetiCloudAppointment(appointment, row, dtAppointmentEmails, sICLOUD_USERNAME, sbChanges, bDISABLE_PARTICIPANTS);
										...
										{
											if ( bVERBOSE_STATUS )
												SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncAppointments: Binding appointment " + Sql.ToString(row["NAME"]) + " " + Sql.ToString(row["DATE_START"]) + " to " + sICLOUD_USERNAME);
											...
											if ( sSYNC_ACTION == "local changed" )
											{
												...
												DataTable dtAppointmentEmails = AppointmentEmails(Application, con, Sql.ToGuid(row["ID"]), gUSER_ID);
												// 03/23/2013 Paul.  Add the ability to disable participants. 
												bool bChanged = iCloudSync.SetiCloudAppointment(appointment, row, dtAppointmentEmails, sICLOUD_USERNAME, sbChanges, bDISABLE_PARTICIPANTS);
												if ( bChanged )
												{
													if ( bVERBOSE_STATUS )
														SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncAppointments: Sending appointment " + Sql.ToString(row["NAME"]) + " " + Sql.ToString(row["DATE_START"]) + " to " + sICLOUD_USERNAME);
										...
										{
											if ( bVERBOSE_STATUS )
												SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncAppointments: Unsyncing appointment " + Sql.ToString(row["NAME"]) + " " + Sql.ToString(row["DATE_START"]) + " for " + sICLOUD_USERNAME);
											using ( IDbTransaction trn = Sql.BeginTransaction(con) )
											{
												try
												{
		...
		// 01/24/2012 Paul.  Only update the record if it has changed.  This is to prevent an endless loop caused by sync operations. 
		public static bool BuildAPPOINTMENTS_Update(HttpApplicationState Application, ExchangeSession Session, IDbCommand spAPPOINTMENTS_Update, DataRow row, AppointmentEntry appointment, Guid gUSER_ID, Guid gTEAM_ID, Guid gASSIGNED_USER_ID)
		{
			bool bChanged = false;
			// 03/25/2010 Paul.  We start with the existing record values so that we can apply ACL Field Security rules. 
			if ( row != null && row.Table != null )
		...
		// 01/24/2012 Paul.  We need to have a way to force an update as we cannot rely upon the iCloud embedded revision date. 
		public static void ImportAppointment(HttpContext Context, ExchangeSession Session, CalendarService service, IDbConnection con, string sICLOUD_USERNAME, Guid gUSER_ID, AppointmentEntry appointment, StringBuilder sbErrors)
		{
			HttpApplicationState Application = Context.Application;
					...
					using ( DataTable dt = new DataTable() )
					{
						...
						if ( sSYNC_ACTION == "remote new" || sSYNC_ACTION == "remote changed" )
						{
							...
							using ( IDbTransaction trn = Sql.BeginTransaction(con) )
							{
								...
								{
									if ( bVERBOSE_STATUS )
										SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncAppointments: Retrieving appointment " + Sql.ToString(appointment.Title) + " " + Sql.ToString(appointment.Times[0].StartTime) + " from " + sICLOUD_USERNAME);
						...
						else if ( (sSYNC_ACTION == "local changed" || sSYNC_ACTION == "local new") && !Sql.IsEmptyGuid(gID) )
						{
							...
							if ( dt.Rows.Count > 0 )
							{
								...
								{
									if ( bVERBOSE_STATUS )
										SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncAppointments: Syncing appointment " + Sql.ToString(row["NAME"]) + " " + Sql.ToString(row["DATE_START"]) + " to " + sICLOUD_USERNAME);
									// 03/28/2010 Paul.  The EWS Managed API requires one field to change in order to update the record. 
									// 07/18/2010 Paul.  Move iCloud Sync functions to a separate class. 
									DataTable dtAppointmentEmails = AppointmentEmails(Application, con, Sql.ToGuid(row["ID"]), gUSER_ID);
									// 03/23/2013 Paul.  Add the ability to disable participants. 
						...
						else if ( sSYNC_ACTION == "local deleted" )
						{
							...
							{
								if ( bVERBOSE_STATUS )
									SyncError.SystemMessage(Context, "Warning", new StackTrace(true).GetFrame(0), "SyncAppointments: Deleting " + Sql.ToString(appointment.Title) + " " + Sql.ToString(appointment.Times[0].StartTime) + " from " + sICLOUD_USERNAME);
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									try
									{
