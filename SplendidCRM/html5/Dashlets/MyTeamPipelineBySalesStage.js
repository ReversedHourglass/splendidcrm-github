			...
			bgPage.AuthenticatedMethod(function(status, message)
			{
				if ( status == 1 )
				{
					if ( !Sql.IsEmptyString(sSEARCH_FILTER) )
						sSEARCH_FILTER += ' and ';
					sSEARCH_FILTER += "TEAM_ID = \'" + Security.USER_ID() + "\'";
					...
					bgPage.ListView_LoadTableWithAggregate(sTABLE_NAME, sORDER_BY, sSELECT, sFILTER, sGROUP_BY, sAGGREGATE, function(status, message)
					{
						if ( status == 1 )
						{
							...
							sSELECT         = 'SALES_STAGE, LIST_ORDER, TEAM_ID, USER_NAME';
							sGROUP_BY       = 'SALES_STAGE, LIST_ORDER, TEAM_ID, USER_NAME';
							...
							bgPage.ListView_LoadTableWithAggregate(sTABLE_NAME, sORDER_BY, sSELECT, sFILTER, sGROUP_BY, sAGGREGATE, function(status, message)
							{
								if ( status == 1 )
								{
									...
									for ( var i = 0; i < dt.length; i++ )
									{
										...
										var gTEAM_ID  = Sql.ToGuid   (row["TEAM_ID" ]);
										var sUSER_NAME         = Sql.ToString (row["USER_NAME"        ]);
									...
									try
									{
										...
										$('#' + sLayoutPanel + '_divChartHTML5').bind('jqplotDataClick', function(ev, seriesIndex, pointIndex, data)
										{
											...
											//window.location.href = sREMOTE_SERVER + 'Opportunities/default.aspx?SALES_STAGE=' + escape(sSALES_STAGE) + '&TEAM_ID=' + escape(sTEAM_ID);
										});
